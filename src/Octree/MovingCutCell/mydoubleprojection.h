...
We also define default boundary conditions for the velocity update
$A_f$ that correspond to either (i) a no slip boundary condition for a
fixed embedded boundary or (ii) an embedded boundary moving at a
...
event begin_timestep (i++)
{
  Af = new face vector;
#if EMBED
  Af.n[embed] = dirichlet (0);
  Af.t[embed] = dirichlet (0);
#if dimension == 3
  Af.r[embed] = dirichlet (0);
#endif // dimension = 3
#if TREE
  Af.x.refine = refine_face;
  foreach_dimension()
    Af.x.prolongation = refine_embed_face_x;
#endif // TREE
#endif // EMBED
}
...
{
  foreach_face()
    Af.x[] += (face_value (u.x, 0) + dt*a.x[]);

  /**
  We multiply *Af* by *fm* in a separate loop to avoid inconsitencies
  in face fractions when using moving embedded boundaries. */
