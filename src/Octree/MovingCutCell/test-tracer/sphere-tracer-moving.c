/**
# Flow past a moving sphere at different Reynolds number and Peclet numbers $Pe$

This test case is the moving embedded boundaries counterpart of the
test case [sphere-tracer.c]().
...
We solve here the 3D Navier-Stokes equations and add the sphere using
...
The sphere's position is advanced to time $t + \Delta t$. */
...
event adapt (i++)
{
  adapt_wavelet ({cs,u,f}, (double[]) {1.e-2,(cmax),(cmax),(cmax),1.e-2},
  		 maxlevel = (lmax), minlevel = (1));
...
event logfile (i++; t <= 11.*(tref))
{
  ...
  fprintf (stderr, "%g %g %d %g %g %d %d %d %d %d %d %g %g %g %g %g %g %g %g %g %g\n",
	   (Re), (Pe),
	   i, t/(tref), dt/(tref),
...
event fields (t = {1.*(tref), 5.*(tref), 10.*(tref)})
{
  ...
  for (double x = -(L0)/2. + step/2.; x < (L0)/2. - step/2.; x += step)
    fprintf (fpx, "%g %g %g\n", (x - p_p.x)/(d),
	     ((x - p_p.x) <= -(d)/2. || (x - p_p.x) >= (d)/2. ? interpolate (f,    x, p_p.y, p_p.z) : 0.),
	     ((x - p_p.x) <= -(d)/2. || (x - p_p.x) >= (d)/2. ? interpolate (gf.x, x, p_p.y, p_p.z) : 0.));  
  fclose (fpx);
  ...
  for (double y = -(L0)/2. + step/2.; y < (L0)/2. - step/2.; y += step)
    fprintf (fpy, "%g %g %g\n", (y - p_p.y)/(d),
	     ((y - p_p.y) <= -(d)/2. || (y - p_p.y) >= (d)/2. ? interpolate (f,    p_p.x, y, p_p.z) : 0.),
	     ((y - p_p.y) <= -(d)/2. || (y - p_p.y) >= (d)/2. ? interpolate (gf.y, p_p.x, y, p_p.z) : 0.));  
  fclose (fpy);

  // Centerline x=0, y=0
  char namez[80];
  sprintf (namez, "fields-z-%.0f-pid-%d.dat", t/(tref), pid());
  FILE * fpz = fopen (namez, "w");

  for (double z = -(L0)/2. + step/2.; z < (L0)/2. - step/2.; z += step)
    fprintf (fpz, "%g %g %g\n", (z - p_p.z)/(d),
	     ((z - p_p.z) <= -(d)/2. || (z - p_p.z) >= (d)/2. ? interpolate (f,    p_p.x, p_p.y, z) : 0.),
	     ((z - p_p.z) <= -(d)/2. || (z - p_p.z) >= (d)/2. ? interpolate (gf.z, p_p.x, p_p.y, z) : 0.));  
  fclose (fpz);
}
...
![Vorticity](sphere-tracer-moving/vorticity.mp4)(loop)

![Tracer field](sphere-tracer-moving/scalar.mp4)(loop)
...
set xrange [0:11]
set yrange [1.2:3]
plot '../sphere-tracer/log' u 4:16 w l lw 2 lc rgb "blue" t "fixed", \
     'log'                    u 4:16 w l lw 2 lc rgb "red"  t "moving"
~~~

#### Scalar field *f*

We plot the distribution along the centerlines $y=0$, $x=0$ and $z=0$
of the scalar field *f*.

~~~gnuplot Scalar field *f* along the centerline $y=0$, $z=0$
reset
set terminal svg font ",16"
set key top right spacing 1.1
set xlabel 'x/d'
...
set arrow from  0.5, graph 0 to 0.5, graph 1 nohead
plot '< cat ../sphere-tracer/fields-x-1-pid-*'  u 1:2 w l lw 2 lc rgb "black" notitle, \
     '< cat ../sphere-tracer/fields-x-5-pid-*'  u 1:2 w l lw 2 lc rgb "black" notitle, \
     '< cat ../sphere-tracer/fields-x-10-pid-*' u 1:2 w l lw 2 lc rgb "black" notitle, \
...
~~~gnuplot Scalar field *f* along the centerline $x=0$, $z=0$
set xlabel 'y/d'
set xrange [-10:10]
plot '< cat ../sphere-tracer/fields-y-1-pid-*'  u 1:2 w l lw 2 lc rgb "black" notitle, \
     '< cat ../sphere-tracer/fields-y-5-pid-*'  u 1:2 w l lw 2 lc rgb "black" notitle, \
     '< cat ../sphere-tracer/fields-y-10-pid-*' u 1:2 w l lw 2 lc rgb "black" notitle, \
...
~~~gnuplot Scalar field *f* along the centerline $x=0$, $y=0$
set xlabel 'z/d'
set xrange [-10:10]
plot '< cat ../sphere-tracer/fields-z-1-pid-*'  u 1:2 w l lw 2 lc rgb "black" notitle, \
     '< cat ../sphere-tracer/fields-z-5-pid-*'  u 1:2 w l lw 2 lc rgb "black" notitle, \
     '< cat ../sphere-tracer/fields-z-10-pid-*' u 1:2 w l lw 2 lc rgb "black" notitle, \
     '< cat fields-z-1-pid-*'  u 1:2 w l lw 2 lc rgb "blue"      t 't=1', \
     '< cat fields-z-5-pid-*'  u 1:2 w l lw 2 lc rgb "red"       t 't=5', \
     '< cat fields-z-10-pid-*' u 1:2 w l lw 2 lc rgb "sea-green" t 't=10'
~~~

#### Gradient of the scalar field *gf*

We plot the distribution along the centerlines $y=0$, $x=0$ and $z=0$
of the scalar field *gf*.

~~~gnuplot Gradient of the scalar field *gf.x* along the centerline $y=0$, $z=0$
reset
set terminal svg font ",16"
set key top right spacing 1.1
set xlabel 'x/d'
...
set arrow from -0.5, graph 0 to -0.5, graph 1 nohead
set arrow from  0.5, graph 0 to 0.5, graph 1 nohead
plot '< cat ../sphere-tracer/fields-x-1-pid-*'  u 1:3 w l lw 2 lc rgb "black" notitle, \
     '< cat ../sphere-tracer/fields-x-5-pid-*'  u 1:3 w l lw 2 lc rgb "black" notitle, \
     '< cat ../sphere-tracer/fields-x-10-pid-*' u 1:3 w l lw 2 lc rgb "black" notitle, \
...
~~~gnuplot Gradient of the scalar field *gf.y* along the centerline $x=0$, $z=0$
set xlabel 'y/d'
set ylabel 'gf.y'
plot '< cat ../sphere-tracer/fields-y-1-pid-*'  u 1:3 w l lw 2 lc rgb "black" notitle, \
     '< cat ../sphere-tracer/fields-y-5-pid-*'  u 1:3 w l lw 2 lc rgb "black" notitle, \
     '< cat ../sphere-tracer/fields-y-10-pid-*' u 1:3 w l lw 2 lc rgb "black" notitle, \
...
~~~gnuplot Gradient of the scalar field *gf.z* along the centerline $x=0$, $y=0$
set xlabel 'z/d'
set ylabel 'gf.z'
plot '< cat ../sphere-tracer/fields-z-1-pid-*'  u 1:3 w l lw 2 lc rgb "black" notitle, \
     '< cat ../sphere-tracer/fields-z-5-pid-*'  u 1:3 w l lw 2 lc rgb "black" notitle, \
     '< cat ../sphere-tracer/fields-z-10-pid-*' u 1:3 w l lw 2 lc rgb "black" notitle, \
     '< cat fields-z-1-pid-*'  u 1:3 w l lw 2 lc rgb "blue"      t 't=1', \
     '< cat fields-z-5-pid-*'  u 1:3 w l lw 2 lc rgb "red"       t 't=5', \
     '< cat fields-z-10-pid-*' u 1:3 w l lw 2 lc rgb "sea-green" t 't=10'
~~~
*/
