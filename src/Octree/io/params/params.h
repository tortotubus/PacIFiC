...
#define input_file_register_option(group_name, option_variable, type_name)     \
  _input_file_register_option((group_name), #option_variable,                  \
                              (void *)&(option_variable), (type_name))

#define input_file_register_option_named(group_name, option_name,              \
                                         option_variable, type_name)           \
  _input_file_register_option((group_name), option_name,                       \
                              (void *)&(option_variable), (type_name))
 ...
 * @param option_variable (int, double, bool, or string) The variable holding
 * the option value
...
void _input_file_register_option(const char *group_name,
                                 const char *option_name, void *value_ptr,
...
static int _input_group_find_option(const InputGroup *group,
...
void _input_file_register_group(const char *group_name) {
  ...
  InputGroup *groups = (InputGroup *)realloc(
      input_file.input_groups, (size_t)new_count * sizeof(InputGroup));
  if (!groups)
    return;
 ...
 * @param value_ptr The pointer to the variable where the option's value should
 * be applied
...
void _input_file_register_option(const char *group_name,
  ...
  ParamValue *option_values = (ParamValue *)realloc(
      g->option_values, (size_t)new_count * sizeof(ParamValue));
  if (!option_values)
    return;
...
static int _input_group_find_option(const InputGroup *group,
  ...
  for (int oi = 0; oi < group->option_count; oi++) {
    if (group->option_names[oi] &&
        strcmp(group->option_names[oi], option_name) == 0)
      return oi;
  }
  return -1;
}
