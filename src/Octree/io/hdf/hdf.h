...
void HDF_write_attribute(const char *attribute_name, const void *data,
                         const hid_t dtype_id, const hid_t group_id,
                         const hsize_t dims[], HDF *hdf);
void HDF_write_scalar_attribute(const char *attribute_name, const void *data,
                                const hid_t dtype_id, const hid_t group_id,
                                HDF *hdf);
void HDF_modify_scalar_attribute(const char *attribute_name, const void *data,
                                 const hid_t dtype_id, const hid_t group_id,
                                 HDF *hdf);
void HDF_read_scalar_attribute(const char *attribute_name, void *data,
                               const hid_t group_id, HDF *hdf);
void HDF_write_type_attribute(const char *type_name, const hid_t group_id,
                              HDF *hdf);
void HDF_read_dataset(const char *dataset_name, void **data,
                      const hid_t dtype_id, const hid_t group_id,
                      const int rank, hsize_t dims[], hsize_t max_dims[],
                      HDF *hdf);
void HDF_read_dataset_dims(const char *dataset_name, const hid_t dtype_id,
                           const hid_t group_id, const int rank, hsize_t dims[],
                           hsize_t max_dims[], HDF *hdf);
void HDF_write_dataset(const char *dataset_name, const void *data,
                       const hid_t dtype_id, const hid_t group_id,
                       const int rank, const hsize_t dims[], HDF *hdf);
void HDF_write_chunked_dataset(const char *dataset_name, const void *data,
                               const hid_t dtype_id, const hid_t group_id,
                               const int rank, const hsize_t dims[],
                               const hsize_t max_dims[],
                               const hsize_t chunk_dims[], HDF *hdf);
void HDF_append_chunked_dataset(const char *dataset_name, const void *data,
                                const hid_t dtype_id, const hid_t group_id,
                                const int rank, const hsize_t dims[], HDF *hdf);
void HDF_write_compressed_dataset(const char *dataset_name, const void *data,
                                  const hid_t dtype_id, const hid_t group_id,
                                  const int rank, const hsize_t dims[],
                                  const hsize_t max_dims[],
                                  const hsize_t chunk_dims[],
                                  unsigned int compression_level, HDF *hdf);
void HDF_collective_write_dataset(const char *dataset_name, const void *data,
                                  const hid_t dtype_id, const hid_t group_id,
                                  const int rank, const hsize_t dims[],
                                  const hsize_t local_size[],
                                  const hsize_t local_offset[], HDF *hdf);
void HDF_collective_write_chunked_dataset(
    ...
    const hsize_t local_size[], const hsize_t local_offset[], HDF *hdf);
void HDF_collective_append_chunked_dataset(
    const char *dataset_name, const void *data, const hid_t dtype_id,
    const hid_t group_id, const int rank, const hsize_t dims[],
    const hsize_t local_size[], const hsize_t local_offset[], HDF *hdf);

void HDF_collective_write_compressed_dataset(
    const char *dataset_name, const void *data, const hid_t dtype_id,
    const hid_t group_id, const int rank, const hsize_t dims[],
    const hsize_t max_dims[], const hsize_t chunk_dims[],
 ...
 * @relates HDF
 */
void HDF_error(HDF *hdf) { HDF_close(hdf); }
...
void HDF_write_attribute(const char *attribute_name, const void *data,
  ...
  hdf->attr_id = H5Acreate2(group_id, attribute_name, hdf->attr_dtype_id,
                            hdf->attr_space_id, H5P_DEFAULT, H5P_DEFAULT);
...
void HDF_write_type_attribute(const char *type_name, const hid_t group_id,
  ...
  // Create the attribute
  hdf->attr_id = H5Acreate2(group_id, "Type", hdf->attr_dtype_id,
                            hdf->attr_space_id, H5P_DEFAULT, H5P_DEFAULT);
  HDF_check_object(hdf, hdf->attr_id);
...
void HDF_write_dataset(const char *dataset_name, const void *data,
  ...
  // Create the dataset
  hdf->dset_id =
      H5Dcreate2(group_id, dataset_name, hdf->dset_dtype_id, hdf->dset_space_id,
                 H5P_DEFAULT, hdf->dcpl_id, H5P_DEFAULT);
  HDF_check_object(hdf, hdf->dset_id);

  result = H5Dwrite(hdf->dset_id, hdf->dset_dtype_id, H5S_ALL, H5S_ALL,
                    H5P_DEFAULT, data);
  HDF_check_result(hdf, result);
...
void HDF_append_chunked_dataset(const char *dataset_name, const void *data,
  ...
  result = H5Sselect_hyperslab(hdf->dset_space_id, H5S_SELECT_SET, start,
                               NULL, // stride
                               count,
                               NULL); // block
...
void HDF_collective_write_dataset(const char *dataset_name, const void *data,
  ...
  result = H5Sselect_hyperslab(hdf->file_space, H5S_SELECT_SET, local_offset,
                               NULL, local_size, NULL);
  HDF_check_result(hdf, result);
...
void HDF_collective_append_chunked_dataset(
  ...
  for (int i = 1; i < rank; ++i) {
    if (dims[i] != cur_dims[i]) {
      fprintf(stderr,
              ...
              i, (unsigned long long)cur_dims[i], (unsigned long long)dims[i]);
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
  }
  ...
  result = H5Sselect_hyperslab(hdf->file_space, H5S_SELECT_SET, start,
                               NULL, // stride
                               count,
                               NULL); // block
  HDF_check_result(hdf, result);
