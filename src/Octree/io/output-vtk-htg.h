...
#include "io/vtk/vtkHDFHyperTreeGrid.h"
...
void output_hdf_htg_series(scalar *scalar_list, vector *vector_list,
                           const char *basename, int iter, double time,
                           bool overwrite);

void output_hdf_htg(scalar *scalar_list = NULL, vector *vector_list = NULL,
                    const char *basename = NULL, int iter, double time,
                    bool use_transient = false, bool overwrite, bool use_ab);
 ...
 * @brief
 */
trace void output_hdf_htg_series(scalar *scalar_list, vector *vector_list,
                                 const char *basename, int iter, double time,
                                 bool overwrite) {
  char pname[128];
