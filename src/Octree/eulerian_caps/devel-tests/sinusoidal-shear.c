...
tensor my_grad_u_from_centered[];
event logfile (i+=OUTPUT_FREQ) {
  scalar un[], divu[], divfu[], divfu_from_grad[];
  int nb_mb_cells = 0;
  double divu_mb_avg = 0., divfu_mb_avg = 0.;
  ...
  foreach() {
    ...
    divfu[] = (uf.x[1,0] - uf.x[] + uf.y[0,1] - uf.y[])/Delta;
    divfu_from_grad[] = my_grad_u.x.x[] + my_grad_u.y.y[];
    foreach_dimension() {
      my_grad_u_from_centered.x.x[] = (u.x[1,0] - u.x[-1,0])/(2.*Delta);
      my_grad_u_from_centered.x.y[] = (u.x[0,1] - u.x[0,-1])/(2.*Delta);
    }
    if (GAMMA) {
      nb_mb_cells++;
      divu_mb_avg += divu[];
      ...
      aen_avg += aen[];
      abs_J_avg += fabs(J[]-1);
      abs_G_avg_xx += fabs(G.x.x[]-1);
      abs_G_avg_xy += fabs(G.x.y[]);
      abs_G_avg_yy += fabs(G.y.y[]-1);
  ...
  if (nb_mb_cells != 0) {
    divu_mb_avg /= nb_mb_cells;
    divfu_mb_avg /= nb_mb_cells;
  ...
  fprintf (stdout, "%.17g %.17g %.17g %.17g %.17g %.17g %.17g  %.17g "
    ...
    normf(divu).max, normf(divu).avg, divu_mb_avg, normf(divfu).max,
    normf(divfu).avg, divfu_mb_avg,
    normf(divfu_from_grad).max, normf(divfu_from_grad).avg,
    normf(my_grad_u.x.x).avg, normf(my_grad_u.x.y).avg,
    normf(my_grad_u.y.x).avg, normf(my_grad_u.y.y).avg,
    normf(my_grad_u_from_centered.x.x).avg,
    normf(my_grad_u_from_centered.x.y).avg,
    normf(my_grad_u_from_centered.y.x).avg,
    normf(my_grad_u_from_centered.y.y).avg
  );
}
