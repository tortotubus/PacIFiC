...
#include "capsule-ft-distributed.h"
...
event tracer_advection(i++) {  
  ...
  repulsive_vel();
    ...
    distributed_store_pre_reduce_lagVel(i);
  #endif
  /**
  ...
  */
    ...
    distributed_lagVel_exchange_before_advection(i);
  #else
    reduce_alllagVel();
  #endif

  #if _MPI && DEBUG_AABB
    debug_store_pre_advect_pos(i);
    debug_owner_advection_dryrun(i);
  #endif

  #if _MPI && LAG_DISTRIBUTED_OWNER_ADVECTION
    distributed_prepare_owner_advection();
  #endif
  ...
  for(int i=0; i<NCAPS; i++) {
    if (CAPS(i).isactive) {
      ...
      #if _MPI && LAG_DISTRIBUTED_OWNER_ADVECTION
        advect_this_capsule = distributed_capsule_is_owned_here(&CAPS(i));
      #endif
      if (advect_this_capsule) {
        advect_lagMesh(&CAPS(i));
        #if _MPI && LAG_DISTRIBUTED_OWNER_ADVECTION
          distributed_mark_capsule_advected(i);
        #endif
      }
  ...
  #if _MPI && LAG_DISTRIBUTED_OWNER_ADVECTION
    distributed_geometry_exchange_after_advection(i);
    distributed_lifecycle_after_advection(i);
  #endif

  debug_print_distributed_aabb_state(i);
...
event cleanup (t = end) {
  #if _MPI
    free_distributed_capsule_mpi_storage();
  #endif
