...
event movie(t+=0.002) {
  ...
  output_hdf_pd((Pscalar[]){pradius, {-1}},(Pvector[]){ppos, pvel, {{-1}}},"one_cap");
#endif
...
event output_and_dump_caps (i+=OUTPUT_FREQ_PV_DUMP) {
    ...
    pv_output_ascii(nb_dump); // dump capsules in vtk
      ...
      foreach() {
        if (cm[] > 1.e-20)
          viscosity_plot[] = mup + (muc - mup)*I[];
        else
          viscosity_plot[] = 0.;
      }
      boundary({viscosity_plot});

      scalar * list = {p, I, viscosity_plot};
      vector * vlist = {u, forcing};
