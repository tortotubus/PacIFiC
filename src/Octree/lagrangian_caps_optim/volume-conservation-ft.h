...
*/
trace
coord compute_alpha_m(CapsuleMesh* mesh, int m) {
  coord alpha = {0., 0., 0.};

  for(int i=0; i<mesh->nodes[m].nb_triangles; i++) {
    int tid = mesh->nodes[m].triangle_ids[i];
    int local_id = 0;
    while (LAG_TRI_TOPO(mesh, tid)->node_ids[local_id] != m && local_id < 3) 
      local_id++;
    int nids[2];  // `nids` for "neighbor ids"
    nids[0] = LAG_TRI_TOPO(mesh, tid)->node_ids[(local_id + 1)%3];
    nids[1] = LAG_TRI_TOPO(mesh, tid)->node_ids[(local_id + 2)%3];
...
*/
trace
void enforce_optimal_volume_conservation(CapsuleMesh* mesh) {
  ...
  */
  ...
  for(int j=0; j<mesh->nlt; j++) {
    ...
    for(int k=0; k<3; k++) 
      tn[k] = LAG_TRI_TOPO(mesh, j)->node_ids[k];
    for(int k=0; k<3; k++) {
