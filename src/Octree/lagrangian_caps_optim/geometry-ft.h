...
*/
double edge_length(CapsuleMesh *mesh, int i) {
  double length = 0.;
  int v1, v2;
  v1 = LAG_EDGE_TOPO(mesh, i)->node_ids[0];
  v2 = LAG_EDGE_TOPO(mesh, i)->node_ids[1];
  foreach_dimension() {
...
```force``` is set to ```true```, the edges' lengths are computed no matter the
value of ```updated_stretches```.
*/
...
void compute_lengths(_compute_lengths p) {
  CapsuleMesh *mesh = p.mesh;
  bool force = (p.force) ? p.force : false;
  if (force || !mesh->updated_stretches) {
    for (int i = 0; i < mesh->nle; i++)
      LAG_EDGE_STATE(mesh, i)->length = edge_length(mesh, i);
    mesh->updated_stretches = true;
  }
}

/** In 3D simulations, the function below assumes that the Lagrangian mesh
contains the origin and is convex, and swaps the order of the nodes in order to
compute an outward normal vector. This only need to be performed at the creation
of the mesh since the outward property of the normal vectors won't change
through the simulation.
*/
void comp_initial_area_normals(CapsuleMesh *mesh) {
  for (int i = 0; i < mesh->nlt; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      nid[j] = LAG_TRI_TOPO(mesh, i)->node_ids[j];
      foreach_dimension() centroid.x += mesh->nodes[nid[j]].pos.x / 3;
    }
    coord normal, e[2];
    for (int j = 0; j < 2; j++)
      foreach_dimension() e[j].x = GENERAL_1DIST(mesh->nodes[nid[0]].pos.x,
    ...
    if (dp < 0) {
      LAG_TRI_TOPO(mesh, i)->node_ids[1] = nid[2];
      LAG_TRI_TOPO(mesh, i)->node_ids[2] = nid[1];
      foreach_dimension() normal.x *= -1;
    }
    foreach_dimension() {
      LAG_TRI_STATE(mesh, i)->centroid.x = centroid.x;
      LAG_TRI_STATE(mesh, i)->normal.x = normal.x / norm;
    }
    LAG_TRI_STATE(mesh, i)->area = norm / 2;
  }
}

/** The two functions below compute the outward normal vector to all the
triangles of the mesh, for 3D simulations. */
void comp_triangle_area_normal(CapsuleMesh *mesh, int i) {
  int nid[3]; // node ids
  for (int j = 0; j < 3; j++)
    nid[j] = LAG_TRI_TOPO(mesh, i)->node_ids[j];
  ...
  foreach_dimension() LAG_TRI_STATE(mesh, i)->centroid.x = 0.;

  // for(int j=0; j<3; j++) {
  //   foreach_dimension() {
  //     LAG_TRI_STATE(mesh, i)->centroid.x +=
  ...
  for (int j = 0; j < 3; j++) {
    foreach_dimension() {
      LAG_TRI_STATE(mesh, i)->centroid.x +=
          mesh->centroid.x +
          GENERAL_1DIST(mesh->nodes[nid[j]].pos.x, mesh->centroid.x);
    }
  }
  foreach_dimension() LAG_TRI_STATE(mesh, i)->centroid.x /= 3.;
  ////ggd

  coord origin = {X0 + L0 / 2, Y0 + L0 / 2, Z0 + L0 / 2};
  foreach_dimension() {
    if (fabs(LAG_TRI_STATE(mesh, i)->centroid.x - origin.x) > L0 / 2.) {
      if (LAG_TRI_STATE(mesh, i)->centroid.x - origin.x > 0)
        LAG_TRI_STATE(mesh, i)->centroid.x -= L0;
      else
        LAG_TRI_STATE(mesh, i)->centroid.x += L0;
    }
  }
  coord normal, e[2];
  for (int j = 0; j < 2; j++)
    foreach_dimension() e[j].x =
        GENERAL_1DIST(mesh->nodes[nid[0]].pos.x, mesh->nodes[nid[j + 1]].pos.x);
  foreach_dimension() normal.x = e[0].y * e[1].z - e[0].z * e[1].y;
  double norm = sqrt(sq(normal.x) + sq(normal.y) + sq(normal.z));
  foreach_dimension() LAG_TRI_STATE(mesh, i)->normal.x = normal.x / norm;
  LAG_TRI_STATE(mesh, i)->area = norm / 2.;
}

void comp_triangle_area_normals(CapsuleMesh *mesh) {
  for (int i = 0; i < mesh->nlt; i++)
    comp_triangle_area_normal(mesh, i);
}

/**
Capsule nodes are stored in an unwrapped material coordinate system. The IBM
layer wraps IB node positions when locating Eulerian support for coupling.
These legacy correction hooks are kept for callers that need cache invalidation,
but they no longer mutate capsule coordinates.
*/
...
void correct_lag_pos(CapsuleMesh *mesh) {
  if (!mesh)
    return;
...
caps structure.
*/
void comp_centroid(CapsuleMesh *mesh) {
  coord origin = {X0 + L0 / 2, Y0 + L0 / 2, Z0 + L0 / 2};
  ...
  for (int i = 0; i < mesh->nln; i++)
    foreach_dimension() {
      ...
      mesh->centroid.x +=
          (tentative_pos < origin.x - L0 / 2)
              ? tentative_pos + L0
              : ((tentative_pos > origin.x + L0 / 2) ? tentative_pos - L0
                                                     : tentative_pos);
    }
  foreach_dimension() mesh->centroid.x =
      mesh->centroid.x / mesh->nln + mesh->nodes[0].pos.x;
  correct_node_pos(&mesh->centroid);
}

trace void comp_volume(CapsuleMesh *mesh) {
  coord origin = {X0 + L0 / 2, Y0 + L0 / 2, Z0 + L0 / 2};
  comp_centroid(mesh);
  double volume = 0;
  for (int i = 0; i < mesh->nlt; i++) {
    ...
    for (int j = 0; j < 3; j++)
      foreach_dimension() {
        double tentative_pos =
            mesh->nodes[LAG_TRI_TOPO(mesh, i)->node_ids[j]].pos.x -
            mesh->centroid.x;
        nodes[j].x =
            (tentative_pos < origin.x - L0 / 2)
                ? tentative_pos + L0
    ...
    for (int j = 0; j < 3; j++) {
      coord cross_product;
      foreach_dimension() cross_product.x =
          nodes[(j + 1) % 3].y * nodes[(j + 2) % 3].z -
...
void compute_taylor_factor(CapsuleMesh *mesh, double *Taylor_deform,
                           double *Inclin_angle, coord *rs, double *TDmaxmin,
                           double *TDang) {
  ...
  for (int i = 0; i < mesh->nlt; i++) {
    double rn = 0.;
    double rSquared =
        sq(GENERAL_1DIST(LAG_TRI_STATE(mesh, i)->centroid.x,
                         mesh->centroid.x)) +
        sq(GENERAL_1DIST(LAG_TRI_STATE(mesh, i)->centroid.y,
                         mesh->centroid.y)) +
        sq(GENERAL_1DIST(LAG_TRI_STATE(mesh, i)->centroid.z, mesh->centroid.z));
    coord tri_vec = {0};

    foreach_dimension() tri_vec.x =
        GENERAL_1DIST(LAG_TRI_STATE(mesh, i)->centroid.x, mesh->centroid.x);
    foreach_dimension() rn += tri_vec.x * LAG_TRI_STATE(mesh, i)->normal.x;
    // rn = fabs(rn); // In case of centroid ouside of capsule, we change the
    // sign of rn

    // Compute the components of inertia tensor
    Ixx += LAG_TRI_STATE(mesh, i)->area / 5.0 * rn *
           (rSquared - tri_vec.x * tri_vec.x);
    Iyy += LAG_TRI_STATE(mesh, i)->area / 5.0 * rn *
           (rSquared - tri_vec.y * tri_vec.y);
    Izz += LAG_TRI_STATE(mesh, i)->area / 5.0 * rn *
           (rSquared - tri_vec.z * tri_vec.z);
    Ixy +=
        LAG_TRI_STATE(mesh, i)->area / 5.0 * rn * (0. - tri_vec.x * tri_vec.y);
    Ixz +=
        LAG_TRI_STATE(mesh, i)->area / 5.0 * rn * (0. - tri_vec.x * tri_vec.z);
    Iyz +=
        LAG_TRI_STATE(mesh, i)->area / 5.0 * rn * (0. - tri_vec.y * tri_vec.z);
  }
  ...
  dd = (Ixx * Iyz * Iyz + Iyy * Ixz * Ixz + Izz * Ixy * Ixy -
        2 * Ixy * Ixz * Iyz - Ixx * Iyy * Izz);
  ...
  *Inclin_angle = atan2(Evy, Evx);
  ...
  for (int i = 0; i < mesh->nln; i++) {
    ...
    if (rad > rmax) {
      rmax = rad;
      *TDang = (fabs(projx) < 1.e-14) ? (projy > 0. ? pi / 2. : 3 * pi / 2.)
                                      : fmod(atan2(projy, projx) + pi, pi);
    }
    if (rad < rmin)
      rmin = rad;
  }
...
trace void comp_capsule_geodynamics(CapsuleMesh *mesh) {
...
/*Compute the cell size in the grid*/
  ...
  for (int i = 0; i < mesh->nln; i++) {
    foreach_dimension() center_vel.x += mesh->nodes[i].lagVel.x / mesh->nln;
  }
  ...
  for (int i = 0; i < mesh->nln; i++) {
    double tentative_radius =
        sqrt(GENERAL_SQNORM(mesh->nodes[i].pos, mesh->centroid));

    max_radius =
        (tentative_radius > max_radius) ? tentative_radius : max_radius;
    min_radius =
        (tentative_radius < min_radius) ? tentative_radius : min_radius;

    foreach_dimension() angvel.x +=
        (GENERAL_1DIST(mesh->nodes[i].pos.y, mesh->centroid.y) *
             (mesh->nodes[i].lagVel.z - center_vel.z) -
         GENERAL_1DIST(mesh->nodes[i].pos.z, mesh->centroid.z) *
             (mesh->nodes[i].lagVel.y - center_vel.y)) /
        tentative_radius / tentative_radius;
  }
  ...
  foreach_dimension() mesh->ang_vel.x = angvel.x / mesh->nln;
}
...
void comp_normals(CapsuleMesh *mesh) {
  if (!mesh->updated_normals) {
    ...
    for (int i = 0; i < mesh->nln; i++) {
      ...
      for (int j = 0; j < mesh->nodes[i].nb_triangles; j++) {
        ...
        double dist = 0.;
        foreach_dimension() dist +=
            sq(mesh->nodes[i].pos.x - LAG_TRI_STATE(mesh, tid)->centroid.x);
        dist = sqrt(dist);
        sw += dist;
        foreach_dimension() mesh->nodes[i].normal.x +=
            LAG_TRI_STATE(mesh, tid)->normal.x * dist;
      }
      foreach_dimension() mesh->nodes[i].normal.x /= sw;
...
*/
bool is_neighbor_ft(CapsuleMesh *mesh, int i, int j) {
  for (int k = 0; k < mesh->nodes[i].nb_neighbors; k++) {
    if (mesh->nodes[i].neighbor_ids[k] == j)
      return true;
  }
  return false;
}
...
j */
bool edge_exists(CapsuleMesh *mesh, int j, int k) {
  for (int i = 0; i < mesh->nle; i++) {
    if ((LAG_EDGE_TOPO(mesh, i)->node_ids[0] == j &&
         LAG_EDGE_TOPO(mesh, i)->node_ids[1] == k) ||
        (LAG_EDGE_TOPO(mesh, i)->node_ids[0] == k &&
         LAG_EDGE_TOPO(mesh, i)->node_ids[1] == j))
      return true;
  }
  return false;
}
...
boundary. */
bool is_edge_across_periodic(CapsuleMesh *mesh, int i) {
  int n[2];
  for (int k = 0; k < 2; k++)
    n[k] = LAG_EDGE_TOPO(mesh, i)->node_ids[k];
...
bool write_edge(_write_edge p) {
  ...
  if (!overwrite && edge_exists(mesh, j, k))
    return false;
  else {
    LAG_EDGE_TOPO(mesh, i)->node_ids[0] = j;
    LAG_EDGE_TOPO(mesh, i)->node_ids[1] = k;
    for (int ii = 0; ii < 2; ii++)
      LAG_EDGE_TOPO(mesh, i)->triangle_ids[ii] = -1;
    if (new_mesh) {
...
don't exist yet). */
void new_edge(CapsuleMesh *mesh, int i, int j) {
  int eid = mesh->nle; // id of the new edge
  int nodes[2];
  nodes[0] = i;
  nodes[1] = j;
  for (int k = 0; k < 2; k++) {
    LAG_EDGE_TOPO(mesh, eid)->node_ids[k] = nodes[k];
    ...
    LAG_EDGE_TOPO(mesh, eid)->triangle_ids[k] = -1;
  }
  mesh->nle++;
}
...
at its midpoint. */
void split_edge(CapsuleMesh *mesh, int i) {
  int nid[2];
  for (int j = 0; j < 2; j++)
    nid[j] = LAG_EDGE_TOPO(mesh, i)->node_ids[j];

  /** Create new node */
  foreach_dimension() mesh->nodes[mesh->nln].pos.x =
  ...
  write_edge((_write_edge){
      .mesh = mesh, .i = i, .j = nid[0], .k = mesh->nln, .overwrite = true});
  write_edge(
      (_write_edge){.mesh = mesh, .i = mesh->nle, .j = nid[1], .k = mesh->nln});
  for (int j = 0; j < 2; j++) {
    LAG_EDGE_TOPO(mesh, i)->triangle_ids[j] = -1;
    LAG_EDGE_TOPO(mesh, mesh->nle)->triangle_ids[j] = -1;
  }
...
bool triangle_exists(CapsuleMesh *mesh, int i, int j, int k) {
  for (int t = 0; t < mesh->nlt; t++) {
    for (int a = 0; a < 3; a++) {
      if (LAG_TRI_TOPO(mesh, t)->node_ids[a] == i) {
        for (int b = 0; b < 3; b++) {
          if (b != a && LAG_TRI_TOPO(mesh, t)->node_ids[b] == j) {
            for (int c = 0; c < 3; c++) {
              if (c != a && c != b && LAG_TRI_TOPO(mesh, t)->node_ids[c] == k) {
                return true;
              }
            }
          }
...
boundary. */
bool is_triangle_across_periodic(CapsuleMesh *mesh, int i) {
  int e[3];
  for (int k = 0; k < 3; k++)
    e[k] = LAG_TRI_TOPO(mesh, i)->edge_ids[k];
...
bool write_triangle(_write_triangle p) {
  ...
  int k = p.k;
  bool overwrite = (p.overwrite) ? p.overwrite : false;
  if (!overwrite && triangle_exists(mesh, i, j, k))
    return false;
  else {
    LAG_TRI_TOPO(mesh, tid)->node_ids[0] = i;
    LAG_TRI_TOPO(mesh, tid)->node_ids[1] = j;
    LAG_TRI_TOPO(mesh, tid)->node_ids[2] = k;
    int c = 0;
    for (int a = 0; a < 3; a++) {
      int va = LAG_TRI_TOPO(mesh, tid)->node_ids[a];
      int b = (a + 1) % 3;
      int vb = LAG_TRI_TOPO(mesh, tid)->node_ids[b];
      for (int m = 0; m < mesh->nodes[va].nb_neighbors; m++) {
        for (int n = 0; n < mesh->nodes[vb].nb_neighbors; n++) {
          if (mesh->nodes[va].edge_ids[m] == mesh->nodes[vb].edge_ids[n]) {
            LAG_TRI_TOPO(mesh, tid)->edge_ids[c] = mesh->nodes[va].edge_ids[m];
            c++;
            int edge_id = mesh->nodes[va].edge_ids[m];
            int p =
                (LAG_EDGE_TOPO(mesh, edge_id)->triangle_ids[0] == -1) ? 0 : 1;
            LAG_EDGE_TOPO(mesh, edge_id)->triangle_ids[p] = tid;
          }
        }
      }
    }
...
void new_triangle(CapsuleMesh *mesh, int i, int j, int k, int prev_tid) {
  ...
  int tid = mesh->nlt;
  for (int k = 0; k < 3; k++) {
    LAG_TRI_TOPO(mesh, tid)->node_ids[k] = nodes[k];
    ...
    for (int l = 0; l < mesh->nodes[nodes[k]].nb_neighbors; l++) {
      ...
      int cn = (LAG_EDGE_TOPO(mesh, ce)->node_ids[0] == nodes[k])
                   ? LAG_EDGE_TOPO(mesh, ce)->node_ids[1]
                   : LAG_EDGE_TOPO(mesh, ce)->node_ids[0];
      if (cn == nodes[(k + 1) % 3])
        break;
    }
    /** Then, we add this edge to the list of edges of our new triangle */
    LAG_TRI_TOPO(mesh, tid)->edge_ids[k] = ce;
    /** And we have to update the triangle id of the edge */
    replaced_tid = false;
    for (int l = 0; l < 2; l++)
      if (LAG_EDGE_TOPO(mesh, ce)->triangle_ids[l] == prev_tid) {
        LAG_EDGE_TOPO(mesh, ce)->triangle_ids[l] = tid;
        replaced_tid = true;
        break;
      }
    if (!replaced_tid && LAG_EDGE_TOPO(mesh, ce)->triangle_ids[0] != tid) {
      for (int l = 0; l < 2; l++)
        if (LAG_EDGE_TOPO(mesh, ce)->triangle_ids[l] == -1) {
          LAG_EDGE_TOPO(mesh, ce)->triangle_ids[l] = tid;
          replaced_tid = true;
          break;
        }
...
void overwrite_triangle(CapsuleMesh *mesh, int tid, int i, int j, int k) {
  int nodes[3];
  nodes[0] = i;
  nodes[1] = j;
  nodes[2] = k;
  for (int i = 0; i < 3; i++) {
    ...
    for (int j = 0; j < mesh->nodes[nodes[i]].nb_neighbors; j++) {
      ...
      int cn = (LAG_EDGE_TOPO(mesh, eid)->node_ids[0] == nodes[i])
                   ? LAG_EDGE_TOPO(mesh, eid)->node_ids[1]
                   : LAG_EDGE_TOPO(mesh, eid)->node_ids[0];
      if (cn == nodes[(i + 1) % 3])
        break;
    }
    /** 2.2 Add this edge to the triangle's list of edges */
    LAG_TRI_TOPO(mesh, tid)->edge_ids[i] = eid;
    /** 2.3 Add the triangle id to the edge's list of triangles */
    int index = (LAG_EDGE_TOPO(mesh, eid)->triangle_ids[0] > -1 &&
                 LAG_EDGE_TOPO(mesh, eid)->triangle_ids[0] != tid)
                    ? 1
                    : 0;
    LAG_EDGE_TOPO(mesh, eid)->triangle_ids[index] = tid;
  }
}

/** The function below returns true if node j is a vertex of triangle i */
bool is_triangle_vertex(CapsuleMesh *mesh, int i, int j) {
  for (int k = 0; k < 3; k++) {
    if (LAG_TRI_TOPO(mesh, i)->node_ids[k] == j)
      return true;
  }
  return false;
}
...
*/
double comp_angle(CapNode *n1, CapNode *n2, CapNode *n3) {
  double theta = 0.;
  foreach_dimension() {
...
*/
bool is_obtuse_node(CapsuleMesh *mesh, int tid, int i) {
  CapNode *n[3];
  // n[0] is the node to check if obtuse in the triangle tid
  n[0] = &(mesh->nodes[i]);
  int count_pts = 1;
  for (int j = 0; j < 3; j++) {
    if (LAG_TRI_TOPO(mesh, tid)->node_ids[j] != i) {
      n[count_pts] = &(mesh->nodes[LAG_TRI_TOPO(mesh, tid)->node_ids[j]]);
      count_pts++;
    }
  }
  ...
  if (comp_angle(n[0], n[1], n[2]) > pi / 2.)
    return true;
  else
    return false;
}
...
three angles.
*/
bool is_obtuse_triangle(CapsuleMesh *mesh, int tid) {
  CapNode *n[3];
  for (int j = 0; j < 3; j++)
    n[j] = &(mesh->nodes[LAG_TRI_TOPO(mesh, tid)->node_ids[j]]);
  if (comp_angle(n[0], n[1], n[2]) > pi / 2.)
    return true;
  else if (comp_angle(n[1], n[2], n[0]) > pi / 2.)
    return true;
  else if (comp_angle(n[2], n[0], n[1]) > pi / 2.)
    return true;
  else
    return false;
}
...
void refine_mesh(CapsuleMesh *mesh) {
  ...
  for (int i = 0; i < cnt; i++) {
    ...
    for (int j = 0; j < 3; j++) {
      int edge_id = LAG_TRI_TOPO(mesh, i)->edge_ids[j];
      if (LAG_EDGE_TOPO(mesh, edge_id)->triangle_ids[1] > -1) {
        split_edge(mesh, edge_id);
        mid_ids[j] = mesh->nln - 1;
      } else {
        mid_ids[j] = is_triangle_vertex(
                         mesh, i, LAG_EDGE_TOPO(mesh, edge_id)->node_ids[0])
                         ? LAG_EDGE_TOPO(mesh, edge_id)->node_ids[1]
                         : LAG_EDGE_TOPO(mesh, edge_id)->node_ids[0];
      }
    }
    ...
    for (int j = 0; j < 3; j++) {
      ...
      for (int k = 0; k < 3; k++) { // Loop over the current triangle vertices
        corner_id = LAG_TRI_TOPO(mesh, i)->node_ids[k];
...
*/
foreach_dimension() double periodic_friendly_cross_product_x(coord a, coord b,
                                                             coord ref) {
  coord nodes[2];
...
trace double compute_node_area(CapsuleMesh *mesh, int i) {
  double area = 0.;
  for (int j = 0; j < mesh->nodes[i].nb_triangles; j++) {
    int tid = mesh->nodes[i].triangle_ids[j];
    if (is_obtuse_triangle(mesh, tid)) {
      area += (is_obtuse_node(mesh, tid, i))
                  ? LAG_TRI_STATE(mesh, tid)->area / 2
                  : LAG_TRI_STATE(mesh, tid)->area / 4;
    } else {
      double voronoi_area = 0.;
      for (int k = 0; k < 3; k++) {
        int nid = LAG_TRI_TOPO(mesh, tid)->node_ids[k];
        if (nid != i) {
          int eid = -1; // eid for "edge id", connecting i and nid
          for (int l = 0; l < 3; l++) {
            int teid =
                LAG_TRI_TOPO(mesh, tid)->edge_ids[l]; // temporary edge id
            if ((LAG_EDGE_TOPO(mesh, teid)->node_ids[0] == i ||
                 LAG_EDGE_TOPO(mesh, teid)->node_ids[1] == i) &&
                (LAG_EDGE_TOPO(mesh, teid)->node_ids[0] == nid ||
                 LAG_EDGE_TOPO(mesh, teid)->node_ids[1] == nid))
              eid = teid;
          }
          ...
          for (int l = 0; l < 2; l++) {
            ...
            int tneid = LAG_EDGE_TOPO(mesh, eid)->triangle_ids[l];
            for (int m = 0; m < 3; m++)
              if (LAG_TRI_TOPO(mesh, tneid)->node_ids[m] != i &&
                  LAG_TRI_TOPO(mesh, tneid)->node_ids[m] != nid)
                onid[l] = LAG_TRI_TOPO(mesh, tneid)->node_ids[m];
          }
          ...
          foreach_dimension() edge_length +=
              sq(GENERAL_1DIST(mesh->nodes[i].pos.x, mesh->nodes[nid].pos.x));
