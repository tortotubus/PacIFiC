...
# Layal Jbara
#
# This file, Train_Ternary_Lift.py, contains functions for training a Ternary regression Lift model.
...
#####################################################################################################
"""
Train_Ternary_Lift.py contains functions for training a Ternary regression Lift model.
"""
#####################################################################################################
...
def data_train(qfold, N_2, N_3, N_4, B_S):
    ...
    log_dir = f"../results/logs/Lift/Ternary_q_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}"
    log_path = f"../results/logs/Lift/Ternary_log_q_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.txt"
    cp_filepath = f'../results/Checkpoints/Lift/CP-Ternary_model_fold_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.h5'
    ...
    X_train_tri       = np.load(f'../results/Tensors/Lift/X_train_tri_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')
    X_test_tri        = np.load(f'../results/Tensors/Lift/X_test_tri_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')
    ...
    model = Model(inputs=[input_list_tri]+[input_list_bi], outputs=output_layer)
    ...
    if os.path.exists(cp_filepath):
        ...
        history = new_model.fit(
            [X_train_list_tri]+[X_train_list_bi], Y_train,
    ...
    # Saving Model
    model.save(f"../results/Models/Lift/Ternary_model_fold_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.h5")

    # ========================== Training Set Results ==========================
    start_time = time.perf_counter()
    Y_pred = model.predict([X_train_list_tri]+[X_train_list_bi])
    ...
    start_time = time.perf_counter()
    Y_pred_test = model.predict([X_test_list_tri]+[X_test_list_bi])
    end_time = time.perf_counter()
