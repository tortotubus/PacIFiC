...
# Layal Jbara
#
# This file, Preprocess_Lift.py, contains functions that preprocess raw data, generate features for higher-order interactions, 
# apply symmetry-based augmentation, and prepare labels for training a Lift Hierarchical Physics-Informed Neural Network (PINN). 
# It also handles shuffling and concatenation of data to improve model generalization.
#####################################################################################################

"""
Preprocess_Lift.py contains functions for preprocessing data, generating features, applying symmetry augmentation,
and preparing labels for training a Lift Hierarchical PINN.
"""
...
def dataset_process(csv_file, N_2, N_3, N_4):
    ...
    csv_file : str
        ...
        positions and Lift coefficients for multiple particles.
    ...
    X_in_bi : numpy.ndarray
        ...
        It includes pairwise particle distances and corresponding Lift coefficients.

    X_in_tri : numpy.ndarray
        The ternary interaction data, reshaped into an array with dimensions suitable for training models.
        It includes triplet particle distances and corresponding Lift coefficients.
    ...
    X_in_quat : numpy.ndarray
        ...
        It includes quadruplet particle distances and corresponding Lift coefficients, excluding the last column.

    Y_in : numpy.ndarray
        The target variable (Lift coefficient) for the quaternary interactions, reshaped into an array.
    ...
    Lift coefficients. The output arrays are reshaped for compatibility with machine learning models.
    ...
    the corresponding Lift coefficients.
    """
    ...
    for i in range(1, N_2 + 1):
        column_names.extend([f'x{i}-x0', f'y{i}-y0', f'Cl0-{i}'])
    ...
    for combo in combinations(range(1, N_3 + 1), 2):
        column_names += [f'x{combo[0]}-x0', f'y{combo[0]}-y0', f'x{combo[1]}-x0', f'y{combo[1]}-y0', f'Cl0-{combo[0]}-{combo[1]}']
    tri_data = dataset[column_names].values

    # Quaternary contributions
    column_names = []
    for combo in combinations(range(1, N_4 + 1), 3):
        column_names += [f'x{combo[0]}-x0', f'y{combo[0]}-y0', f'x{combo[1]}-x0', f'y{combo[1]}-y0', f'x{combo[2]}-x0', f'y{combo[2]}-y0', f'Cl0-{combo[0]}-{combo[1]}-{combo[2]}']
    column_names.append('Cl0')
...
def cdav(phi, a=3.23, b=7.37):
    """
    Calculates Lift coefficient as a function of volume fraction (phi).

    This function computes the Lift coefficient based on a given volume fraction (phi) using a 
    ...
    a : float, optional, default=3.23
        A constant parameter for the Lift coefficient formula. The default value is 3.23.

    b : float, optional, default=7.37
        A constant parameter for the Lift coefficient formula. The default value is 7.37.

    Returns:
    --------
    float
        The calculated Lift coefficient (C_d) based on the input volume fraction (phi).

    Notes:
    ------
    The function assumes that the volume fraction `phi` is within a valid range (0, 1), 
    and both `a` and `b` are used as the empirical constants in the Lift coefficient equation.
    """
    return a * (1 - phi) ** (-b)
    ...
    for train_index, test_index in kf.split(X_in_quat):
        ...
        if q == qfold:
            ...
            for dataset, data in {
                ...
                np.save(f'../results/Tensors/Lift/{dataset}_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}', data)

def main():
    """
    Main function that handles command-line argument parsing and invokes the `create_data` function 
    to preprocess the dataset for Lift prediction model training, based on the specified fold number 
    and interaction types.
    """
     
    parser = argparse.ArgumentParser(description="Preprocess dataset for Lift prediction model.")
