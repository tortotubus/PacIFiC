...
# Layal Jbara
#
# This file, Train_Ternary_Drag.py, contains functions for training a Ternary regression drag model.
...
#####################################################################################################
"""
Train_Ternary_Drag.py contains functions for training a Ternary regression drag model.
"""
#####################################################################################################
...
def data_train(qfold, N_2, N_3, N_4, B_S, Cds=2.74931):
    ...
    num_nghb_2 = comb(N_2, 1)  # Ternary neighbors
    ...
    log_dir = f"../results/logs/Drag/Ternary_q_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}"
    log_path = f"../results/logs/Drag/Ternary_log_q_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.txt"
    cp_filepath = f'../results/Checkpoints/Drag/CP-Ternary_model_fold_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.h5'
    ...
    # Load Ternary training and testing datasets
    ...
    concat = Concatenate()( [Lambda(summation, name=f'tri_summation{j}')(out_list_tri[j]) for j in range(num_nghb_3)]+
    ...
    model = Model(inputs=[input_list_tri]+[input_list_bi]+[input_constant], outputs=output_layer_with_constant)
    ...
    if os.path.exists(cp_filepath):
        ...
        history = new_model.fit(
            [X_train_list_tri]+[X_train_list_bi]+[X_in_c_train], Y_train,
    ...
    # Saving Model
    model.save(f"../results/Models/Drag/Ternary_model_fold_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.h5")

    # ========================== Training Set Results ==========================
    start_time = time.perf_counter()
    Y_pred = model.predict([X_train_list_tri]+[X_train_list_bi]+[X_in_c_train])
    ...
    start_time = time.perf_counter()
    Y_pred_test = model.predict([X_test_list_tri]+[X_test_list_bi]+[X_in_c_test])
    end_time = time.perf_counter()
...
def main():
    ...
    parser.add_argument('--N_2', type=int, required=True, help='Number of Ternary neighbours')
