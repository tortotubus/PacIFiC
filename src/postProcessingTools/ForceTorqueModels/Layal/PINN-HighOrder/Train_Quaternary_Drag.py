...
# Layal Jbara
#
# This file, Train_Quaternary_Drag.py, contains functions for training a Quaternary regression drag model.
...
#####################################################################################################
"""
Train_Quaternary_Drag.py contains functions for training a Quaternary regression Drag model.
"""
#####################################################################################################
...
def data_train(qfold, N_2, N_3, N_4, B_S, Cds=2.74931):
    ...
    log_dir = f"../results/logs/Drag/Quaternary_q_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}"
    log_path = f"../results/logs/Drag/Quaternary_log_q_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.txt"
    cp_filepath = f'../results/Checkpoints/Drag/CP-Quaternary_model_fold_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.h5'
    ...
    X_train_quat       = np.load(f'../results/Tensors/Drag/X_train_quat_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')
    X_test_quat        = np.load(f'../results/Tensors/Drag/X_test_quat_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')

    X_train_tri       = np.load(f'../results/Tensors/Drag/X_train_tri_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')
    X_test_tri        = np.load(f'../results/Tensors/Drag/X_test_tri_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')
    ...
    model = Model(inputs=[input_list_quat]+[input_list_tri]+[input_list_bi]+[input_constant], outputs=output_layer_with_constant)
    ...
    if os.path.exists(cp_filepath):
        ...
        history = new_model.fit(
            [X_train_list_quat]+[X_train_list_tri]+[X_train_list_bi]+[X_in_c_train], Y_train,
    ...
    # Saving Model
    model.save(f"../results/Models/Drag/Quaternary_model_fold_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.h5")

    # ========================== Training Set Results ==========================
    start_time = time.perf_counter()
    Y_pred = model.predict([X_train_list_quat]+[X_train_list_tri]+[X_train_list_bi]+[X_in_c_train])
    ...
    start_time = time.perf_counter()
    Y_pred_test = model.predict([X_test_list_quat]+[X_test_list_tri]+[X_test_list_bi]+[X_in_c_test])
    end_time = time.perf_counter()
