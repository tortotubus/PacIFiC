...
# Layal Jbara
#
# This file, Train_Binary_Drag.py, contains functions for training a binary regression drag model.
...
#####################################################################################################
"""
Train_Binary_Drag.py contains functions for training a binary regression drag model.
"""
#####################################################################################################
...
def data_train(qfold, N_2, N_3, N_4, B_S, Cds=2.74931):
    ...
    log_dir = f"../results/logs/Drag/Binary_q_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}"
    log_path = f"../results/logs/Drag/Binary_log_q_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.txt"
    cp_filepath = f'../results/Checkpoints/Drag/CP-Binary_model_fold_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.h5'
    ...
    # Load binary training and testing datasets
    X_train_bi = np.load(f'../results/Tensors/Drag/X_train_bi_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')
    X_test_bi = np.load(f'../results/Tensors/Drag/X_test_bi_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')

    Y_train = np.load(f'../results/Tensors/Drag/Y_train_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')
    Y_test = np.load(f'../results/Tensors/Drag/Y_test_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')

    X_in_c_train = np.load(f'../results/Tensors/Drag/X_in_c_train_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')
    X_in_c_test = np.load(f'../results/Tensors/Drag/X_in_c_test_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')
    ...
    # Process all neighbors with unified functionality
    X_train_list_bi=[]
    input_list_bi = []
    out_list_bi = []
    ...
    concat = Concatenate()([Lambda(summation, name=f'bi_summation{j}')(out_list_bi[j]) for j in range(num_nghb_2)])
    ...
    # Define Input Layer for the Constant Input Feature
    input_constant = Input(shape=(1,), name='input_constant')
    output_layer_with_constant = Add(name='output_with_constant')([output_layer, input_constant])
    ...
    model = Model(inputs=[input_list_bi] + [input_constant], outputs=output_layer_with_constant)
    ...
    if os.path.exists(cp_filepath):
        ...
        history = new_model.fit(
            [X_train_list_bi] + [X_in_c_train], Y_train,
    ...
    # Saving Model
    model.save(f"../results/Models/Drag/Binary_model_fold_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.h5")

    # ========================== Training Set Results ==========================
    start_time = time.perf_counter()
    Y_pred = model.predict([X_train_list_bi] + [X_in_c_train])
    ...
    print(f'CPU time: {end_time - start_time:.3f} seconds for {Y_pred.shape[0]:8d} datapoints')
    ...
    start_time = time.perf_counter()
    Y_pred_test = model.predict([X_test_list_bi] + [X_in_c_test])
    end_time = time.perf_counter()
    ...
    print(f'CPU time: {end_time - start_time:.3f} seconds for {Y_pred_test.shape[0]:8d} datapoints')

    # ============================= Summary ===================================
    print('++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    print('========================================\nOverall Performance:')
    print(f'<Average Train R^2> = {np.mean(train_scores):.3f}')
    print(f'<Average Test R^2> = {np.mean(test_scores):.3f}')
    print(f'<Average Train Time> = {(Total_time_train / 5):.3f} seconds')
    print(f'<Average Test Time> = {(Total_time_test / 5):.3f} seconds')
