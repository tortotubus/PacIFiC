...
# Layal Jbara
#
# This file, Train_Binary_Lift.py, contains functions for training a binary regression lift model.
...
#####################################################################################################
"""
Train_Binary_Lift.py contains functions for training a binary regression lift model.
"""
#####################################################################################################
...
def data_train(qfold, N_2, N_3, N_4, B_S, Cds=2.74931):
    ...
    log_dir = f"../results/logs/Lift/Binary_q_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}"
    log_path = f"../results/logs/Lift/Binary_log_q_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.txt"
    cp_filepath = f'../results/Checkpoints/Lift/CP-Binary_model_fold_{qfold}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.h5'
    ...
    X_train_bi = np.load(f'../results/Tensors/Lift/X_train_bi_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')
    X_test_bi = np.load(f'../results/Tensors/Lift/X_test_bi_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')

    Y_train = np.load(f'../results/Tensors/Lift/Y_train_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')
    Y_test = np.load(f'../results/Tensors/Lift/Y_test_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}.npy')
    ...
    model = Model(inputs=[input_list_bi], outputs=output_layer)
    ...
    # Saving Model
    model.save(f"../results/Models/Lift/Binary_model_fold_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}_BS_{B_S}.h5")
    
    # ========================== Training Set Results ==========================
    start_time = time.perf_counter()
    Y_pred = model.predict([X_train_list_bi])
    ...
    start_time = time.perf_counter()
    Y_pred_test = model.predict([X_test_list_bi])
    end_time = time.perf_counter()
