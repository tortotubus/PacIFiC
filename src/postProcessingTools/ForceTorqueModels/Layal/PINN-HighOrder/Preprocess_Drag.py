...
# Layal Jbara
#
# This file, Preprocess_Drag.py, contains functions that preprocess raw data, generate features for higher-order interactions, 
# apply symmetry-based augmentation, and prepare labels for training a Drag Hierarchical Physics-Informed Neural Network (PINN). 
# It also handles shuffling and concatenation of data to improve model generalization.
#####################################################################################################

"""
Preprocess_Drag.py contains functions for preprocessing data, generating features, applying symmetry augmentation,
and preparing labels for training a Drag Hierarchical PINN.
"""
...
def dataset_process(csv_file, N_2, N_3, N_4):
    ...
    It also calculates relative distances for each particle pair and higher-order combinations, and returns these 
    transformed features along with the corresponding outputs.

    Parameters:
    -----------
    csv_file : str
        The path to the CSV file containing the raw dataset. The file should include columns for particle 
        positions and drag coefficients for multiple particles.

    N_2 : int
        The number of binary neighbouring particles. 

    N_3 : int
         The number of ternary neighbouring particles. 

    N_4 : int
         The number of quaternary neighbouring particles. 

    Returns:
    --------
    X_in_bi : numpy.ndarray
        The binary interaction data, reshaped into an array with dimensions suitable for training models.
        It includes pairwise particle distances and corresponding drag coefficients.

    X_in_tri : numpy.ndarray
        The ternary interaction data, reshaped into an array with dimensions suitable for training models.
        It includes triplet particle distances and corresponding drag coefficients.
    ...
    X_in_quat : numpy.ndarray
        ...
        It includes quadruplet particle distances and corresponding drag coefficients, excluding the last column.

    Y_in : numpy.ndarray
        The target variable (drag coefficient) for the quaternary interactions, reshaped into an array.
    
    Notes:
    ------
    The function processes a dataset to extract pairwise, triplet, and quadruplet particle interactions.
    The interaction data is created based on relative positions (differences from particle 0) and 
    drag coefficients. The output arrays are reshaped for compatibility with machine learning models.
    
    The combinations of particles for ternary and quaternary interactions are generated using the 
    ...
    for i in range(1, N_2 + 1):
        column_names.extend([f'x{i}-x0', f'y{i}-y0', f'Cd0-{i}'])
    ...
    for combo in combinations(range(1, N_3 + 1), 2):
        column_names += [f'x{combo[0]}-x0', f'y{combo[0]}-y0', f'x{combo[1]}-x0', f'y{combo[1]}-y0', f'Cd0-{combo[0]}-{combo[1]}']
    tri_data = dataset[column_names].values

    # Quaternary contributions
    column_names = []
    for combo in combinations(range(1, N_4 + 1), 3):
        column_names += [f'x{combo[0]}-x0', f'y{combo[0]}-y0', f'x{combo[1]}-x0', f'y{combo[1]}-y0', f'x{combo[2]}-x0', f'y{combo[2]}-y0', f'Cd0-{combo[0]}-{combo[1]}-{combo[2]}']
    column_names.append('Cd0')
...
def cdav(phi, a=3.23, b=7.37):
    """
    Calculates drag coefficient as a function of volume fraction (phi).

    This function computes the drag coefficient based on a given volume fraction (phi) using a 
    ...
    -----------
    phi : float
        The volume fraction of particles in the suspension. This value should lie between 0 and 1.

    a : float, optional, default=3.23
        A constant parameter for the drag coefficient formula. The default value is 3.23.

    b : float, optional, default=7.37
        A constant parameter for the drag coefficient formula. The default value is 7.37.

    Returns:
    --------
    float
        The calculated drag coefficient (C_d) based on the input volume fraction (phi).

    Notes:
    ------
    The function assumes that the volume fraction `phi` is within a valid range (0, 1), 
    and both `a` and `b` are used as the empirical constants in the drag coefficient equation.
    """
    return a * (1 - phi) ** (-b)


def create_data(qfold, N_2, N_3, N_4, Cds=2.74931):
    ...
    N_4 : int
        The number of quaternary neighbouring particles. 
    Cds : float, optional, default=2.74931
        The drag coefficient (Cds) used for scaling the data (at Re=1 for a circular cylinder)
    ...
    # Assuming dataset is a list or array of datasets and Cds is defined
    X_in_c = np.concatenate([
        np.full_like(datasets[i][3], Cds / cdav(phi)) 
        for i, phi in enumerate([0.1, 0.2, 0.3, (0.1 + 0.2) / 2, (0.1 + 0.3) / 2])
    ], axis=0)
    ...
    def modify_data(data, start, end, num_columns, factor):
        ...
        start : int
            ...
            The ending index of the data slice to modify (exclusive).
        num_columns : int
        ...
        for i in range(start, end):
            data[i, 1::num_columns] += factor
            if data is not X_in_bi:
                data[i, 3::num_columns] += factor
            if data is X_in_quat:
                data[i, 5::num_columns] += factor
    ...
    X_in_c= np.vstack((X_in_c, X_in_c))
    ...
    X_in_c = np.vstack([X_in_c] * replication_factor)
    ...
    X_in_quat, X_in_tri, X_in_bi, Y_in, X_in_c, labels = [
        data[perm] for data in [X_in_quat, X_in_tri, X_in_bi, Y_in, X_in_c, labels]
    ]
    ...
    for train_index, test_index in kf.split(X_in_quat):
        ...
        if q == qfold:
            ...
            X_in_c_train, X_in_c_test = X_in_c[train_index], X_in_c[test_index]
            ...
            for dataset, data in {
                ...
                'X_in_c_train': X_in_c_train, 'X_in_c_test': X_in_c_test,
                'labels_train': labels_train, 'labels_test': labels_test
            }.items():
                np.save(f'../results/Tensors/Drag/{dataset}_q_{q}_N2_{N_2}_N3_{N_3}_N4_{N_4}', data)

def main():
    """
    Main function that handles command-line argument parsing and invokes the `create_data` function 
    to preprocess the dataset for drag prediction model training, based on the specified fold number 
    and interaction types.
    """
     
    parser = argparse.ArgumentParser(description="Preprocess dataset for drag prediction model.")
