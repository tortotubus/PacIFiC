...
AllComponents::AllComponents()
    : m_nb_physical_particles_to_insert(0), m_wait(NULL), m_nb_particles(0),
      m_total_nb_particles(0), m_nb_active_particles(0),
      m_total_nb_active_particles(0), m_nb_active_particles_on_proc(0),
      m_total_nb_active_particles_on_all_procs(0),
      m_total_nb_physical_particles(0), m_obstacle(NULL),
      m_outputTorsorObstacles_counter(1), m_outputTorsorObstacles_frequency(0) {
...
AllComponents::~AllComponents() {
  ...
  for (particle = m_RemovedParticles.begin();
       particle != m_RemovedParticles.end(); particle++)
    delete *particle;
  ...
  for (ivp = m_ReferenceParticles.begin(); ivp != m_ReferenceParticles.end();
       ivp++)
    delete *ivp;
...
void AllComponents::AddObstacle(Obstacle *obstacle_) {
  if (!m_obstacle)
    m_obstacle = obstacle_;
  else
    m_obstacle->append(obstacle_);
}
...
void AllComponents::setKinematicsObstacleWithoutMoving(double time, double dt) {
...
Obstacle *AllComponents::getObstacles() { return (m_obstacle); }
...
Particle *AllComponents::getParticle(int id) {
  ...
  bool found = false;
  for (iter = m_ActiveParticles.begin();
       iter != m_ActiveParticles.end() && !found; iter++)
    if ((*iter)->getID() == id) {
      particle = *iter;
      found = true;
    }
...
Particle *AllComponents::getParticleToInsert(PullMode mode) {
  if (m_nb_physical_particles_to_insert) {
    ...
    if (!m_wait) {
      switch (mode) {
      case PM_ORDERED:
        while (!m_NbRemainingParticlesToInsert[type])
          type++;
        break;
...
// ----------------------------------------------------------------------------
// Returns a pointer to the map of serial clone particles */
multimap<int, Particle *> const *
AllComponents::getPeriodicCloneParticles() const {
  return (&m_PeriodicCloneParticles);
}
...
double AllComponents::getCrustThicknessMax() {
  ...
  for (particle = m_ReferenceParticles.begin();
    ...
    crustThicknessMax =
        crustThicknessMax > crustThickness ? crustThicknessMax : crustThickness;
  }
...
double AllComponents::getCrustThicknessMin() {
  ...
  for (particle = m_ReferenceParticles.begin();
    ...
    crustThickness = (*particle)->getCrustThickness();
    crustThicknessMin =
        crustThicknessMin > crustThickness ? crustThickness : crustThicknessMin;
  }
...
void AllComponents::WaitToActive(bool const &parallel) {
  ...
  if (parallel) {
    if (m_wait->getTag() == 1)
      m_ParticlesInBufferzone.push_back(m_wait);
    else if (m_wait->getTag() == 2)
      m_CloneParticles.push_back(m_wait);
  }
...
bool removeParticleFromList(list<Particle *> &pointerslist, Particle *value) {
  list<Particle *>::iterator particle;
  bool found = false;
  for (particle = pointerslist.begin();
       particle != pointerslist.end() && !found;)
    if (*particle == value) {
...
void AllComponents::read_pre2024(istream &fileSave, string const &filename,
  ...
  if (readingMode == "Hybrid")
    GrainsExec::m_readingModeHybrid = true;
...
size_t AllComponents::read(istream &fileSave, vector<Point3> *known_positions,
  ...
  if (npos) {
    Point3 P;
    if (rank == 0)
      known_positions->reserve(npos);
    for (i = 0; i < npos; i++) {
      fileSave >> P[X] >> P[Y] >> P[Z];
      if (rank == 0)
        known_positions->push_back(P);
    }
  }
...
void AllComponents::read_particles(string const &filename, size_t const &npart,
  ...
  pair<multimap<int, int>::iterator, multimap<int, int>::iterator> crange;
  ...
  pfile.erase(pos);
  pfile += "_Particles";
  pfile = GrainsExec::fullResultFileName(
      pfile, nprocs > 1 && !GrainsExec::m_ReadMPIInASingleFile);
  if (GrainsExec::m_readingModeHybrid) {
    pfile += ".bin";
    FILEin.open(pfile.c_str(), ios::in | ios::binary);
  } else
    FILEin.open(pfile.c_str(), ios::in);
...
void AllComponents::write(ostream &fileSave, string const &filename,
                          vector<Point3> const *known_positions,
  ...
  if (rank == 0) {
    ...
    fileSave << endl
             << "NumberOfParticleTypes " << m_ReferenceParticles.size() << endl;

    vector<Particle *>::const_iterator iv;
    for (iv = m_ReferenceParticles.begin(); iv != m_ReferenceParticles.end();
         iv++)
      (*iv)->write(fileSave);
  ...
  if (rank == 0) {
    ...
    for (i = 0; i < nprocs; ++i)
      fileSave << (GrainsExec::m_writingModeHybrid ? "Hybrid" : "Text") << " "
               << nbpart_proc[i] << endl;
  }
...
void AllComponents::write_singleMPIFile(ostream &fileSave,
  ...
  if (periodic && cir == CIR_ALL) {
    doNotWrite = wrapper->doNotWritePeriodicClones(m_ActiveParticles, LC);
    ...
    for (particle = m_ActiveParticles.begin();
      ...
      if (tag < 2)
        ...
        if (doNotWrite->count((*particle)->getID())) {
          bool found = false;
          pair<multimap<int, Point3>::iterator, multimap<int, Point3>::iterator>
              crange = doNotWrite->equal_range((*particle)->getID());
  ...
  if (rank == 0) {
    // Reference particles and obstacles
    fileSave << endl
             << "NumberOfParticleTypes\t" << m_ReferenceParticles.size()
             << endl;
  ...
  for (int i = 1; i < nprocs; i++)
    mpifile_offsets[i] =
        mpifile_offsets[i - 1] + out_length_per_proc[i - 1] * sizeof(char);

  MPI_File_write_at_all(file, mpifile_offsets[rank],
                        oss_particles.str().c_str(), out_length, MPI_CHAR,
                        &status);
...
void AllComponents::debug(char *s) {
  cout << s << '\n'
       << "Particles "
       << m_ActiveParticles.size() + m_nb_physical_particles_to_insert << '\n'
       << "   Actives " << m_ActiveParticles.size() << '\t' << "   Wait    "
       << m_nb_physical_particles_to_insert << endl;
}

// ----------------------------------------------------------------------------
// Output operator
ostream &operator<<(ostream &f, AllComponents const &EC) {
  f << "Number of particles on all processes = " << EC.m_total_nb_particles
    << endl;
  f << "Number of particles = "
    << EC.m_ActiveParticles.size() + EC.m_nb_physical_particles_to_insert
    << endl;
  f << "Number of active particles = " << EC.m_ActiveParticles.size() << endl;
  ...
  for (il = EC.m_ActiveParticles.begin(); il != EC.m_ActiveParticles.end();
       il++)
    f << *(*il) << endl;
...
void AllComponents::PostProcessing_start(
    double time, double dt, LinkedCell const *LC,
    AllInsertionWindows const &insert_windows, int rank, int nprocs,
    GrainsMPIWrapper const *wrapper) {
  ...
  // Message on display
  if (rank == 0)
    for (pp = m_postProcessors.begin();
         pp != m_postProcessors.end() && !written; pp++) {
...
void AllComponents::PostProcessing(double time, double dt, LinkedCell const *LC,
  ...
  // Post processing writers
  for (pp = m_postProcessors.begin(); pp != m_postProcessors.end(); pp++)
    (*pp)->PostProcessing(time, dt, &m_ActiveParticles, &m_RemovedParticles,
                          postProcessingPeriodic, &m_ReferenceParticles,
                          m_obstacle, LC, insert_windows);
...
// contact or motion
void AllComponents::PostProcessingErreurComponents(
    string const &filename, list<Component *> const &errcomposants) {
  // Post processing writers
  list<PostProcessingWriter *>::iterator pp;
  for (pp = m_postProcessors.begin(); pp != m_postProcessors.end(); pp++)
...
void AllComponents::ComputeMaxMeanVelocity(
    double &vmax, double &vmean, GrainsMPIWrapper const *wrapper) const {
  double vit;
  vmax = vmean = 0.;
  size_t ncomp = 0;
...
void AllComponents::monitorParticlesVelocity(
    double time, ofstream &fileOut, int rank,
    GrainsMPIWrapper const *wrapper) const {
...
void AllComponents::checkParaviewPostProcessing(int const &rank,
  ...
  if (rank == 0) {
    cout << GrainsExec::m_shift3
         << "Creation of the Paraview"
            " post-processing writer with the proper parameters"
         << endl;
    cout << GrainsExec::m_shift6 << "Postprocessing" << endl;
  }
  ParaviewPP =
      new ParaviewPostProcessingWriter(rank, nprocs, name_, root_, isBinary);
...
int AllComponents::getMaxParticleIDnumber() const {
  ...
  for (particle = m_ActiveParticles.begin();
       particle != m_ActiveParticles.end(); particle++)
    numeroMax =
        numeroMax < (*particle)->getID() ? (*particle)->getID() : numeroMax;
...
void AllComponents::setOutputObstaclesLoadParameters(
    string const &root_, int const &freq_, list<string> const &ObsNames) {
  m_outputTorsorObstacles_dir = root_;
  m_outputTorsorObstacles_frequency = freq_;

  for (list<string>::const_iterator il = ObsNames.begin(); il != ObsNames.end();
       il++) {
    Obstacle *pobs =
        const_cast<Obstacle *>(m_obstacle->getObstacleFromName(*il));
    if (pobs)
...
void AllComponents::outputObstaclesLoad(double time, double dt,
                                        bool enforceOutput,
                                        bool increaseCounterOnly, int rank) {
  if (!increaseCounterOnly) {
    if (m_outputTorsorObstacles_counter == 0 || enforceOutput) {
      if (rank == 0) {
        ...
        for (list<Obstacle *>::iterator obstacle =
                 m_outputTorsorObstacles.begin();
             obstacle != m_outputTorsorObstacles.end(); obstacle++) {
          ofstream OUT((m_outputTorsorObstacles_dir + "/Loading_" +
                        (*obstacle)->getName() + ".res")
                           .c_str(),
                       ios::app);
  ...
  if (!enforceOutput) {
    ...
    if (m_outputTorsorObstacles_counter == m_outputTorsorObstacles_frequency)
...
void AllComponents::initialiseOutputObstaclesLoadFiles(int rank,
  ...
  if (rank == 0) {
    if (GrainsExec::m_ReloadType == "new") {
      ...
      string cmd = "bash " + get_tools_dir().string() +
...
size_t AllComponents::getNumberParticles() const { return (m_nb_particles); }
...
void AllComponents::computeNumberParticles(GrainsMPIWrapper const *wrapper) {
  ...
  if (wrapper)
    m_total_nb_active_particles =
        wrapper->sum_UNSIGNED_INT(m_nb_active_particles);
  else
    m_total_nb_active_particles = m_nb_active_particles;

  m_nb_particles = m_nb_active_particles + m_nb_physical_particles_to_insert;

  m_total_nb_particles =
      m_total_nb_active_particles + m_nb_physical_particles_to_insert;
  ...
  if (wrapper)
    m_total_nb_active_particles_on_all_procs =
        wrapper->sum_UNSIGNED_INT(m_nb_active_particles_on_proc);
  else
    m_total_nb_active_particles_on_all_procs = m_nb_active_particles_on_proc;

  m_total_nb_physical_particles = m_total_nb_active_particles_on_all_procs +
                                  m_nb_physical_particles_to_insert;

  GrainsExec::setTotalNumberPhysicalParticles(m_total_nb_physical_particles);
}
      ...
      if (tag == 1) {
        ...
        if (GrainsExec::m_MPI_verbose) {
          ...
          oss << "   t=" << GrainsExec::doubleToString(time, FORMAT10DIGITS)
              << " Interior to Buffer (0 -> 1) Id = " << (*particle)->getID()
    ...
    case 1:
      ...
      case 0:
        ...
        if (GrainsExec::m_MPI_verbose) {
          ostringstream oss;
          oss << "   t=" << GrainsExec::doubleToString(time, FORMAT10DIGITS)
              << " Buffer to Interior (1 -> 0) Id = " << (*particle)->getID()
              << " " << *(*particle)->getPosition() << endl;
          GrainsMPIWrapper::addToMPIString(oss.str());
        }
        break;
      ...
      case 1:
        if ((*particle)->getGeoPosition() != (*particle)->getGeoPositionNm1()) {
          newBufPart->push_back(*particle);
          if (GrainsExec::m_MPI_verbose) {
            ostringstream oss;
            oss << "   t=" << GrainsExec::doubleToString(time, FORMAT10DIGITS)
                << " Buffer to Buffer (1 -> 1)   Id = " << (*particle)->getID()
                << " " << *(*particle)->getPosition() << endl;
            oss << "                From "
                << Cell::getGeoPositionName((*particle)->getGeoPositionNm1())
                << " to "
                << Cell::getGeoPositionName((*particle)->getGeoPosition())
                << endl;
            GrainsMPIWrapper::addToMPIString(oss.str());
          }
      ...
      case 2:
        ...
        if (GrainsExec::m_MPI_verbose) {
          ostringstream oss;
          oss << "   t=" << GrainsExec::doubleToString(time, FORMAT10DIGITS)
              << " Buffer to Clone (1 -> 2)    Id = " << (*particle)->getID()
              << " " << *(*particle)->getPosition() << endl;
          GrainsMPIWrapper::addToMPIString(oss.str());
        }
        break;
    ...
    case 2:
      ...
      if (tag == 1) {
        ...
        if (GrainsExec::m_MPI_verbose) {
          ostringstream oss;
          oss << "   t=" << GrainsExec::doubleToString(time, FORMAT10DIGITS)
              << " Clone to Buffer (2 -> 1)    Id = " << (*particle)->getID()
              << " " << *(*particle)->getPosition() << endl;
          GrainsMPIWrapper::addToMPIString(oss.str());
        }
      }
