...
// MPI I/O routines
void ParaviewPostProcessingWriter::writeParticles_Paraview_MPIIO_text(
  ...
  const char fmt[] = "%12.5e ";
  const char endfmt[] = "%12.5e\n";
  ...
  // Open the file
  MPI_File_open(MPI_COMM_activeProc,
                (m_ParaviewFilename_dir + "/" + partFilename).c_str(),
  ...
  for (particle = particles->begin(); particle != particles->end(); particle++)
    if ((*particle)->getActivity() == COMPUTE &&
        ((*particle)->getTag() != 2 || forceForAllTag))
      (*particle)->write_polygonsStr_PARAVIEW(connectivity, offsets, cellstype,
  ...
  for (particle = particles->begin(); particle != particles->end(); particle++)
    if ((*particle)->getActivity() == COMPUTE &&
      ...
      for (j = 0; j < nc; ++j) {
        snprintf(&normU[counter * charspernum], charspernum + 1, fmt, nu);
        ++counter;
      }
    }
  ...
  for (particle = particles->begin(); particle != particles->end(); particle++)
    if ((*particle)->getActivity() == COMPUTE &&
      ...
      for (j = 0; j < nc; ++j) {
        snprintf(&normOm[counter * charspernum], charspernum + 1, fmt, nom);
        ++counter;
      }
    }
...
// MPI I/O routines
void ParaviewPostProcessingWriter::writeParticles_Paraview_MPIIO_binary(
    list<Particle *> const *particles, string const &partFilename,
    bool const &forceForAllTag, bool const &processwrites) {
  GrainsMPIWrapper *wrapper = GrainsExec::getComm();
  MPI_Comm MPI_COMM_activeProc = wrapper->get_active_procs_comm();
  ...
  compress_segment_binary(CURRENT_LENGTH, "writeParticles_Paraview/NormU");
  ...
  compress_segment_binary(CURRENT_LENGTH, "writeParticles_Paraview/NormOm");
  ...
  compress_segment_binary(CURRENT_LENGTH, "writeParticles_Paraview/CoordNumb");
  total_offset = OFFSET;
  ...
  for (rank = 1; rank < m_nprocs; rank++)
    cumul_binary_offset_per_proc[rank] =
        cumul_binary_offset_per_proc[rank - 1] +
  ...
  oss << "<DataArray type=\"Float32\" NumberOfComponents=\"3\" ";
  oss << "offset=\""
      << cumul_binary_offset_per_proc[m_rank] + point_binary_offset
  ...
  mpifile_offsets[0] = 0;
  for (rank = 1; rank < m_nprocs; rank++)
    mpifile_offsets[rank] =
        mpifile_offsets[rank - 1] + header_per_proc[rank - 1] * sizeof(char);
  ...
  for (rank = 0; rank < m_nprocs; rank++)
    mpifile_offsets[rank] =
        starting_point + cumul_binary_offset_per_proc[rank] * sizeof(char);
  MPI_File_write_at_all(file, mpifile_offsets[m_rank], BUFFER, OFFSET, MPI_CHAR,
                        &status);
  delete[] BUFFER;
  BUFFER = 0;
  ALLOCATED = 0;
...
void ParaviewPostProcessingWriter::writeParticlesAsGlyph_Paraview_MPIIO_text(
    list<Particle *> const *particles, string const &partFilename,
    bool const &forceForAllTag, bool const &processwrites) {
  GrainsMPIWrapper *wrapper = GrainsExec::getComm();
  MPI_Comm MPI_COMM_activeProc = wrapper->get_active_procs_comm();
  ...
  list<Particle *>::const_iterator particle;
  const char fmt[8] = "%12.5e ";
  const char endfmt[8] = "%12.5e\n";
  const int charspernum = 13;
  size_t counter = 0;
  Vector3 const *PPTranslation =
      GrainsExec::m_translationParaviewPostProcessing;
  ...
  for (particle = particles->begin(); particle != particles->end(); particle++)
    if ((*particle)->getActivity() == COMPUTE &&
      ...
      if (PPTranslation)
        gc += *PPTranslation;
      snprintf(&coord[3 * counter * charspernum], charspernum + 1, fmt, gc[X]);
      snprintf(&coord[(3 * counter + 1) * charspernum], charspernum + 1, fmt,
               gc[Y]);
      snprintf(&coord[(3 * counter + 2) * charspernum], charspernum + 1, endfmt,
               gc[Z]);
      ++counter;
    }
...
// mode with MPI I/O routines
void ParaviewPostProcessingWriter::writeParticlesAsGlyph_Paraview_MPIIO_binary(
    list<Particle *> const *particles, string const &partFilename,
    bool const &forceForAllTag, bool const &processwrites) {
  GrainsMPIWrapper *wrapper = GrainsExec::getComm();
  MPI_Comm MPI_COMM_activeProc = wrapper->get_active_procs_comm();
  ...
  Vector3 const *PPTranslation =
      GrainsExec::m_translationParaviewPostProcessing;
  double normU, normOm, coordNum;
  int point_binary_offset = 0, orientation_binary_offset, normU_binary_offset,
      normOm_binary_offset, coord_binary_offset, total_offset;
  Point3 gc;
  Vector3 vec;
  Quaternion qrot;
  ...
  compress_segment_binary(
      CURRENT_LENGTH, "writeParticlesAsGlyph_Paraview_MPIIO_binary/CoordNumb");
  total_offset = OFFSET;
  ...
  oss << "<DataArray Name=\"Quaternion\" NumberOfComponents=\"4\" "
      << "type=\"Float32\" offset=\""
      << cumul_binary_offset_per_proc[m_rank] + orientation_binary_offset
      << "\" format=\"appended\"></DataArray>\n";
  oss << "<DataArray type=\"Float32\" Name=\"NormU\" offset=\""
      << cumul_binary_offset_per_proc[m_rank] + normU_binary_offset
      << "\" format=\"appended\"></DataArray>\n";
...
void ParaviewPostProcessingWriter::
  ...
  for (particle = particles->begin(); particle != particles->end(); particle++)
    if ((*particle)->getActivity() == COMPUTE &&
      ...
      snprintf(&coord[3 * counter * charspernum], charspernum + 1, fmt,
               (*vec)[X]);
      snprintf(&coord[(3 * counter + 1) * charspernum], charspernum + 1, fmt,
               (*vec)[Y]);
      snprintf(&coord[(3 * counter + 2) * charspernum], charspernum + 1, endfmt,
...
void ParaviewPostProcessingWriter::
  ...
  oss << "<DataArray Name=\"U\" NumberOfComponents=\"3\" "
      << "type=\"Float32\" offset=\""
      << cumul_binary_offset_per_proc[m_rank] + translational_binary_offset
      << "\" format=\"appended\">"
      << "</DataArray>\n";
  oss << "<DataArray Name=\"Omega\" NumberOfComponents=\"3\" "
      << "type=\"Float32\" offset=\""
      << cumul_binary_offset_per_proc[m_rank] + angular_binary_offset
      << "\" format=\"appended\">"
      << "</DataArray>\n";
  oss << "</PointData>\n";
  oss << "</Piece>\n";
...
// routines
void ParaviewPostProcessingWriter::writeContactForceVectors_Paraview_MPIIO_text(
    list<Particle *> const *particles, LinkedCell const *LC,
    string const &partFilename, double const &time, bool const &processwrites) {
  GrainsMPIWrapper *wrapper = GrainsExec::getComm();
  MPI_Comm MPI_COMM_activeProc = wrapper->get_active_procs_comm();
  MPI_File file;
  MPI_Status status;
  ...
  if (nbpts > 0) {
    for (i = 0; i < nbpts; ++i) {
      ...
      if (PPTranslation)
        pt += *PPTranslation;
      snprintf(&coord[3 * counter * charspernum], charspernum + 1, fmt, pt[X]);
      snprintf(&coord[(3 * counter + 1) * charspernum], charspernum + 1, fmt,
               pt[Y]);
      snprintf(&coord[(3 * counter + 2) * charspernum], charspernum + 1, endfmt,
               pt[Z]);
...
void ParaviewPostProcessingWriter::
    writeContactForceVectors_Paraview_MPIIO_binary(
        list<Particle *> const *particles, LinkedCell const *LC,
        string const &partFilename, double const &time,
  ...
  compress_segment_binary(
      CURRENT_LENGTH, "writeContactForceVectors_Paraview_MPIIO_binary/Points");
  ...
  compress_segment_binary(
      CURRENT_LENGTH, "writeContactForceVectors_Paraview_MPIIO_binary/Force");
  total_offset = OFFSET;
  ...
  oss << "<DataArray Name=\"Force\" NumberOfComponents=\"3\" "
      << "type=\"Float32\" offset=\""
      << cumul_binary_offset_per_proc[m_rank] + force_binary_offset
      << "\" format=\"appended\">"
      << "</DataArray>\n";
  oss << "</PointData>\n";
  oss << "</Piece>\n";
...
// MPI I/O routines
void ParaviewPostProcessingWriter::writeContactForceChains_Paraview_MPIIO_text(
    list<Particle *> const *particles, LinkedCell const *LC,
    string const &filename, double const &time, bool const &processwrites) {
  GrainsMPIWrapper *wrapper = GrainsExec::getComm();
  MPI_Comm MPI_COMM_activeProc = wrapper->get_active_procs_comm();
  MPI_File file;
  MPI_Status status;
  ...
  // Open the file
  MPI_File_open(MPI_COMM_activeProc,
                (m_ParaviewFilename_dir + "/" + filename).c_str(),
                MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &file);
  ...
  oss << "<VTKFile type=\"PolyData\" version=\"0.1\" "
      << "byte_order=\"LittleEndian\">\n";
  oss << "<PolyData>\n";
  oss << "<Piece NumberOfPoints=\"" << total_nbpts << "\" NumberOfVerts=\"" << 0
      << "\" NumberOfLines=\"" << total_nbpts / 2 << "\" NumberOfStrips=\"" << 0
      << "\" NumberOfPolys=\"" << 0 << "\">\n";
  oss << "<Points>\n";
  oss << "<DataArray type=\"Float32\" NumberOfComponents=\"3\" ";
  oss << "format=\"ascii\">\n";
  ...
  for (i = 0; i < nPPF; ++i) {
    ...
    snprintf(&normF[counter * charspernum], charspernum + 1, fmt, norm);
    snprintf(&normF[(counter + 1) * charspernum], charspernum + 1, fmt, norm);
    counter += 2;
  }
...
void ParaviewPostProcessingWriter::
  ...
  int i, j, rank, point_binary_offset = 0, force_binary_offset,
                  connectivity_binary_offset, offsets_binary_offset,
                  total_offset, nPPF = int(LC->getNbPPForces()), nbpts, comp;
  Point3 pt0, pt1;
  double norm = 0.;
  ...
  for (i = 0; i < nPPF; ++i) {
    ...
    if (PPTranslation)
      pt1 += *PPTranslation;
    for (comp = 0; comp < 3; ++comp)
      write_double_binary(pt1[comp]);
  }
  compress_segment_binary(
      CURRENT_LENGTH, "writeContactForceChains_Paraview_MPIIO_binary/Points");
  ...
  for (j = 0; j < nPPF; j++)
    write_int_binary(2 * j);
  compress_segment_binary(
      CURRENT_LENGTH, "writeContactForceChains_Paraview_MPIIO_binary/offsets");
  ...
  compress_segment_binary(
      CURRENT_LENGTH, "writeContactForceChains_Paraview_MPIIO_binary/Force");
  total_offset = OFFSET;
  ...
  oss << "<Piece NumberOfPoints=\"" << nbpts << "\" NumberOfVerts=\"" << 0
      << "\" NumberOfLines=\"" << nPPF << "\" NumberOfStrips=\"" << 0
      << "\" NumberOfPolys=\"" << 0 << "\">\n";
  oss << "<Points>\n";
  oss << "<DataArray type=\"Float32\" NumberOfComponents=\"3\" ";
  oss << "offset=\""
