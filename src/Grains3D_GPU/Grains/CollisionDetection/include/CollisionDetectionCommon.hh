...
//@{
/** @brief Detects collisions between components (world-frame vec/quat).
    @param pairList list of contact pairs
    @param rigidBody rigid body array
    @param position world positions of the components
    @param quaternion world quaternions of the components
...
__HOSTDEVICE__ static INLINE void
    detectCollisionsComponents_common(const uint2*               pairList,
                                      ...
                                      const uint                 pairID,
                                      const uint                 writeIdx = ~0u)
{
    const uint           wIdx  = (writeIdx == ~0u) ? pairID : writeIdx;
...
// -------------------------------------------------------------------------------------------------
/** @brief Detects collisions between components (world-frame Transform3).
    @param pairList list of contact pairs
    @param rigidBody rigid body array
    @param transform world-frame transforms of the components
    @param contactInfo output contact information
    @param pairID ID of the pair */
template <typename T,
          GJKType            GJKVARIANT = GJKType::JOHNSON,
...
template <typename T,
          ...
          BoundingVolumeType BVType     = BoundingVolumeType::OFF>
__HOSTDEVICE__ static INLINE void
    detectCollisionsComponents_common(const uint2* __RESTRICT__         pairList,
