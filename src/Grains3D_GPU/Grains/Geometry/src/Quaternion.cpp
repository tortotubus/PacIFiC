...
// -------------------------------------------------------------------------------------------------
// Atomically add quaternion components to this quaternion
template <typename T>
__DEVICE__ void Quaternion<T>::atomicAdd(const Quaternion<T>& other) noexcept
{
    m_vqt.atomicAdd(other.m_vqt);
    ::atomicAdd(&m_w, other.m_w);
}
