                                          ...
                                          uint                             numParticles)
{
    ...
    const uint denseIndex = (*cells)->computeDenseIndex(positions[tID], false);
    mortonCodes[tID]      = (denseIndex == UINT_MAX)
                                ? ~uint64_t(0)
                                : static_cast<uint64_t>((*cells)->mortonKeyFromLinearIndex(denseIndex));
}
