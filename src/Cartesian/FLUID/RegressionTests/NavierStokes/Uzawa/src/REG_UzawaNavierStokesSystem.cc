...
REG_UzawaNavierStokesSystem:: create( MAC_Object* a_owner,
...
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: create" ) ;
   MAC_CHECK_PRE( exp != 0 ) ;
   MAC_CHECK_PRE( mac_uu != 0 ) ;
   MAC_CHECK_PRE( mac_pp != 0 ) ;     

   REG_UzawaNavierStokesSystem* result = 
         new REG_UzawaNavierStokesSystem( a_owner, exp, mac_uu, mac_pp,
...
//----------------------------------------------------------------------
REG_UzawaNavierStokesSystem:: REG_UzawaNavierStokesSystem(
	MAC_Object* a_owner,
	MAC_ModuleExplorer const* exp,
	FV_DiscreteField* mac_uu,
	FV_DiscreteField* mac_pp,
...
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: REG_UzawaNavierStokesSystem" ) ;
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem:: build_system( MAC_ModuleExplorer const* exp )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: build_system" ) ;
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem:: re_initialize( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: re_initialize" ) ;
...
//----------------------------------------------------------------------
REG_UzawaNavierStokesSystem:: ~REG_UzawaNavierStokesSystem( void )
//----------------------------------------------------------------------
{}
...
//----------------------------------------------------------------------
double 
REG_UzawaNavierStokesSystem:: get_Stokes_convergence_criterion( void ) const
//----------------------------------------------------------------------
{
   MAC_LABEL( 
     "REG_UzawaNavierStokesSystem:: get_Stokes_convergence_criterion" ) ;
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem::at_each_time_step( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: at_each_time_step" ) ;
...
//----------------------------------------------------------------------
double 
REG_UzawaNavierStokesSystem:: compute_velocity_change( void )
//----------------------------------------------------------------------
{ 	
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: compute_velocity_change" ) ;
...
//----------------------------------------------------------------------
double
REG_UzawaNavierStokesSystem:: compute_velocity_divergence_norm( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( 
   	"REG_UzawaNavierStokesSystem:: compute_velocity_divergence_norm" ) ;
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem::nullify_velocity_advection_rhs( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: nullify_velocity_advection_rhs" ) ;
...
//----------------------------------------------------------------------
bool 
REG_UzawaNavierStokesSystem:: Uzawa_solver( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: Uzawa_solver" ) ;   
...
//----------------------------------------------------------------------
bool 
REG_UzawaNavierStokesSystem:: PreconditionedStokes_Uzawa_solver( 
	const double &mupr, const double &gamma )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: Preconditioned_Uzawa_solver" ) ;
...
//----------------------------------------------------------------------
LA_SeqVector const*
REG_UzawaNavierStokesSystem:: get_solution_velocity( void ) const
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: get_solution_U" ) ;
...
//----------------------------------------------------------------------
LA_SeqVector const*
REG_UzawaNavierStokesSystem:: get_solution_pressure( void ) const
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: get_solution_P" ) ;
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem::initialize_velocity( void )
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem::initialize_pressure( void )
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem:: finalize_constant_matrices( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: finalize_constant_matrices" ) ;
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem:: compute_velocityAdvectionDiffusion_rhs( 
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem:: Uzawa_NavierStokes_solver( 
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem:: assemble_velocity_viscous_matrix_rhs( 
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem:: assemble_velocity_unsteady_matrix( 
	double const& coef )
//----------------------------------------------------------------------
{
   MAC_LABEL( 
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem:: assemble_pdivv_matrix_rhs( double const& coef )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: assemble_pdivv_matrix_rhs" ) ;
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem:: assemble_velocity_advection( 
	string const& AdvectionScheme,
      	size_t advecting_level, double const& coef, 
	size_t advected_level  )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: assemble_velocity_advection" ) ;
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem:: assemble_pressure_laplacian_matrix_rhs( 
	double const& coef_lap )
//----------------------------------------------------------------------
{
   MAC_LABEL( 
...
//----------------------------------------------------------------------
void 
REG_UzawaNavierStokesSystem:: pressure_laplacian_correction( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: pressure_laplacian_correction" ) ;
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem::do_additional_savings(
	string const& rootfilename_ugunm2 )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: do_additional_savings" ) ; 

// NOT USED ANYMORE but kept for archival purposes
// ---------------------------------------------------------------------
//    MAC_Communicator const* macCOMM = MAC_Exec::communicator();  
//    double xx = 0. ;   
//    
...
//    {
//      ostringstream oss;
//      oss << rootfilename_ugunm2 ;
//      if ( macCOMM->nb_ranks() > 1 ) oss << "_" << macCOMM->rank() ; 
//      oss << ".mac.bin";
//      ofstream fileOUTbin( oss.str().c_str(), ios::out | ios::binary );
//      
//      UU_NUM->scatter()->get( VEC_rhs_VelocityAdvection_Nm2, U_LOC ) ;
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem:: initialize_ugradu_Nm2( 
	 bool const& restart, string const& rootfilename_ugunm2 )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: initialize_ugradu_Nm2" ) ;
   
// NOT USED ANYMORE but kept for archival purposes
// ---------------------------------------------------------------------
//    // If u.grad(u)^(n-2) has been saved in a separate file
//    // Check that this file exists and read values from it in case of restart
//    if ( restart )
//    {
//      MAC_Communicator const* macCOMM = MAC_Exec::communicator();
//      ostringstream oss;
//      size_t my_rank = macCOMM->rank();
//      
//      oss << rootfilename_ugunm2 ;
//      if ( macCOMM->nb_ranks() > 1 ) oss << "_" << my_rank ; 
//      oss << ".mac.bin";
//      
...
//        fileINbin.close();     
//       
//        UU_NUM->scatter()->set( U_LOC, VEC_rhs_VelocityAdvection_Nm2 ) ;
//        VEC_rhs_VelocityAdvection_Nm2->synchronize() ;       
//      }
//      else       
//        MAC::out() << "Warning : u.grad(u)^(n-2) restart file " <<
//        	oss.str().c_str() << " does not exist on proc " << my_rank << endl;
//      	
//      macCOMM->barrier();
//      if ( my_rank == 0 ) MAC::out() << endl;	
//    }   
// -----------------------------------------------------------------------      
}
...
//----------------------------------------------------------------------
void
REG_UzawaNavierStokesSystem::add_storable_objects(
	MAC_ListIdentity* list ) const
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_UzawaNavierStokesSystem:: add_storable_objects" ) ; 
