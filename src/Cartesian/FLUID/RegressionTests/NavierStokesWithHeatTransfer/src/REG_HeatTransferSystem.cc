...
//----------------------------------------------------------------------
REG_HeatTransferSystem*
REG_HeatTransferSystem:: create( MAC_Object* a_owner,
...
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_HeatTransferSystem:: create" ) ;
   MAC_CHECK_PRE( exp != 0 ) ;
   MAC_CHECK_PRE( mac_uu != 0 ) ;
   MAC_CHECK_PRE( mac_tt != 0 ) ;     

   REG_HeatTransferSystem* result = 
         new REG_HeatTransferSystem( a_owner, exp, mac_tt, mac_uu,
...
//----------------------------------------------------------------------
REG_HeatTransferSystem:: REG_HeatTransferSystem(
	MAC_Object* a_owner,
	MAC_ModuleExplorer const* exp,
        FV_DiscreteField* mac_tt,
        FV_DiscreteField const* mac_uu,
...
//----------------------------------------------------------------------
void
REG_HeatTransferSystem:: build_system( MAC_ModuleExplorer const* exp )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_HeatTransferSystem:: build_system" ) ;
   ...
   VEC_rhs_TemperatureSourceTerm = MAT_A_TemperatureUnsteady->create_vector( 
   	this ) ;
...
//----------------------------------------------------------------------
void
REG_HeatTransferSystem:: re_initialize( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_HeatTransferSystem:: re_initialize" ) ;
   ...
   // Temperature source term rhs
   VEC_rhs_TemperatureSourceTerm->re_initialize( t_glob ) ;     
...
//----------------------------------------------------------------------
REG_HeatTransferSystem:: ~REG_HeatTransferSystem( void )
//----------------------------------------------------------------------
{}
...
//----------------------------------------------------------------------
void
REG_HeatTransferSystem::at_each_time_step( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_HeatTransferSystem:: at_each_time_step" ) ;
...
//----------------------------------------------------------------------
double 
REG_HeatTransferSystem:: compute_temperature_change( void )
//----------------------------------------------------------------------
{ 	
   MAC_LABEL( "REG_HeatTransferSystem:: compute_temperature_change" ) ;
...
//----------------------------------------------------------------------
void
REG_HeatTransferSystem::nullify_temperature_advection_rhs( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( 
   	"REG_HeatTransferSystem:: nullify_temperature_advection_rhs" ) ;
...
//----------------------------------------------------------------------
LA_SeqVector const*
REG_HeatTransferSystem:: get_solution_temperature( void ) const
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_HeatTransferSystem:: get_solution_temperature" ) ;
...
//----------------------------------------------------------------------
void
REG_HeatTransferSystem::initialize_temperature( void )
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void
REG_HeatTransferSystem:: assemble_temperature_advection( 
...
//----------------------------------------------------------------------
void
REG_HeatTransferSystem:: finalize_constant_matrices( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( 
...
//----------------------------------------------------------------------
void
REG_HeatTransferSystem:: compute_temperatureAdvectionDiffusion_rhs( 
	bool const& b_restart,
	size_t const& iteration_number, 
	bool const& b_with_advection )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_HeatTransferSystem:: "
...
//----------------------------------------------------------------------
void
REG_HeatTransferSystem:: assemble_temperature_diffusion_matrix_rhs( 
...
//----------------------------------------------------------------------
void
REG_HeatTransferSystem:: assemble_temperature_unsteady_matrix( 
	double const& coef )
//----------------------------------------------------------------------
{
   MAC_LABEL( 
...
//----------------------------------------------------------------------
void
REG_HeatTransferSystem::add_storable_objects(
	MAC_ListIdentity* list )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_HeatTransferSystem:: add_storable_objects" ) ; 
...
//----------------------------------------------------------------------
void
REG_HeatTransferSystem::store_ugradT_Nm2( 
	size_t const& n_advection_subtimesteps )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_HeatTransferSystem:: store_ugradT_Nm2" ) ; 
...
//----------------------------------------------------------------------
bool
REG_HeatTransferSystem::TemperatureDiffusion_solver( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_HeatTransferSystem:: TemperatureDiffusion_solver" ) ;

   // Copy the advection-diffusion rhs to VEC_rhs_A_Temperature
   VEC_rhs_A_Temperature->set( VEC_rhs_TemperatureAdvectionDiffusion );   

   // Add the source term 
   VEC_rhs_A_Temperature->sum( VEC_rhs_TemperatureSourceTerm, 1. ) ;  
...
//----------------------------------------------------------------------
bool
REG_HeatTransferSystem:: TemperatureAdvection_solver( void  )
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_HeatTransferSystem:: TemperatureAdvection_solver" ) ;
...
//----------------------------------------------------------------------
LA_Vector*
REG_HeatTransferSystem::get_sourceTerm_vector(
	void ) 
//----------------------------------------------------------------------
{
   MAC_LABEL( "REG_HeatTransferSystem:: get_sourceTerm_vector" ) ; 

   return ( VEC_rhs_TemperatureSourceTerm ) ;
