...
//----------------------------------------------------------------------
VPH_ViscoplasticSystem*
VPH_ViscoplasticSystem:: create( MAC_Object* a_owner,
	MAC_ModuleExplorer const* exp,
	FV_DiscreteField* mac_uu,
	FV_DiscreteField* mac_pp,
	FV_DiscreteField* mac_dd,
	const string &VPsolver,	
	...
	bool const& b_with_buoyancy_ )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: create" ) ;
   MAC_CHECK_PRE( exp != 0 ) ;
   MAC_CHECK_PRE( mac_uu != 0 ) ;
   MAC_CHECK_PRE( mac_pp != 0 ) ;     

   VPH_ViscoplasticSystem* result = 
         new VPH_ViscoplasticSystem( a_owner, exp, mac_uu, mac_pp,
	 	mac_dd, VPsolver,
...
//----------------------------------------------------------------------
VPH_ViscoplasticSystem:: VPH_ViscoplasticSystem(
	MAC_Object* a_owner,
	MAC_ModuleExplorer const* exp,
	FV_DiscreteField* mac_uu,
	FV_DiscreteField* mac_pp,
...
//----------------------------------------------------------------------
   ...
   , MAT_T_StressTensorDivergence( 0 )
   , VEC_TauhatMinus1overLd( 0 )
   , VEC_LambdaMinusrd( 0 )
   ...
   , b_with_buoyancy( b_with_buoyancy_ )
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: VPH_ViscoplasticSystem" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem:: build_system( MAC_ModuleExplorer const* exp,
	const string &VPsolver )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: build_system" ) ;
   ...
   // Buoyancy rhs
   if ( b_with_buoyancy )
     VEC_rhs_VelocityBuoyancy = MAT_A_VelocityUnsteady->create_vector( this );
   ...
   VEC_rhs_FullViscoplastic = MAT_A_VelocityUnsteady
     	->create_vector(this) ;
   MAT_T_StressTensorDivergence = LA_Matrix::make( this,
           exp->create_subexplorer( this, 
	   "MAT_T_StressTensorDivergence" )  ) ;
   if ( VPsolver == "ALG2" )
     VEC_LambdaMinusrd = MAT_A_VelocityUnsteady->create_vector( this ) ;     
   else // VPsolver == "FISTA"
     VEC_TauhatMinus1overLd = MAT_A_VelocityUnsteady->create_vector( this ) ;   
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem:: re_initialize( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: re_initialize" ) ;
   ...
   // Buoyancy rhs
   if ( b_with_buoyancy )
     VEC_rhs_VelocityBuoyancy->re_initialize( u_glob ) ;
   ...
   MAT_T_StressTensorDivergence->re_initialize( u_glob, dd_global ) ;
   if ( VEC_LambdaMinusrd )
     VEC_LambdaMinusrd->re_initialize( dd_global ) ;
   if ( VEC_TauhatMinus1overLd ) 
     VEC_TauhatMinus1overLd->re_initialize( dd_global ) ;
   ...
   if ( VEC_LambdaMinusrd )
     ...
     DD_NUM->define_scatter( VEC_TauhatMinus1overLd ) ;
...
//----------------------------------------------------------------------
VPH_ViscoplasticSystem:: ~VPH_ViscoplasticSystem( void )
//----------------------------------------------------------------------
{}
...
//----------------------------------------------------------------------
double 
VPH_ViscoplasticSystem:: get_Stokes_convergence_criterion( void ) const
//----------------------------------------------------------------------
{
   MAC_LABEL( 
     "VPH_ViscoplasticSystem:: get_Stokes_convergence_criterion" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem::at_each_time_step( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: at_each_time_step" ) ;
...
//----------------------------------------------------------------------
double 
VPH_ViscoplasticSystem:: compute_velocity_change( void )
//----------------------------------------------------------------------
{ 	
   MAC_LABEL( "VPH_ViscoplasticSystem:: compute_velocity_change" ) ;
...
//----------------------------------------------------------------------
double 
VPH_ViscoplasticSystem:: compute_velocity_change_iteration( void )
//----------------------------------------------------------------------
{ 	
   MAC_LABEL( "VPH_ViscoplasticSystem:: compute_velocity_change_iteration" ) ;
...
//----------------------------------------------------------------------
double
VPH_ViscoplasticSystem:: compute_velocity_divergence_norm( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( 
   	"VPH_ViscoplasticSystem:: compute_velocity_divergence_norm" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem::nullify_velocity_advection_rhs( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: nullify_velocity_advection_rhs" ) ;
...
//----------------------------------------------------------------------
bool 
VPH_ViscoplasticSystem:: Uzawa_solver( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: Uzawa_solver" ) ;   
...
//----------------------------------------------------------------------
bool 
VPH_ViscoplasticSystem:: PreconditionedStokes_Uzawa_solver( 
	const double &mupr, const double &gamma )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: Preconditioned_Uzawa_solver" ) ;
...
//----------------------------------------------------------------------
LA_SeqVector const*
VPH_ViscoplasticSystem:: get_solution_velocity( void ) const
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: get_solution_U" ) ;
...
//----------------------------------------------------------------------
LA_SeqVector const*
VPH_ViscoplasticSystem:: get_solution_pressure( void ) const
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: get_solution_P" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem::initialize_velocity( void )
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem::initialize_pressure( void )
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem::initialize_tau_hat( size_t tau_hat_level )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: initialize_tau_hat" ) ;
   
   DD->extract_unknown_DOFs_value( tau_hat_level, DD_LOC ) ;
   DD_NUM->scatter()->set( DD_LOC, VEC_TauhatMinus1overLd ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem::initialize_lambda( size_t lambda_level )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: initialize_lambda" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem::initialize_d( size_t d_level )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: initialize_d" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem:: finalize_constant_matrices( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: finalize_constant_matrices" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem:: compute_velocityAdvectionDiffusion_rhs( 
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem:: Uzawa_NavierStokes_solver(
	const string &VPsolver, const double &viscosity, 
...
//----------------------------------------------------------------------
{
   ...
   if ( VPsolver == "ALG2" )
     ...
     MAT_T_StressTensorDivergence->multiply_vec_then_add(
   	VEC_LambdaMinusrd, VEC_rhs_A_Velocity, 1.0, 1.0 ) ;   
   else // VPsolver == "FISTA"
     // Add the div(tau_hat-(1/L).d) term to velocity rhs
     MAT_T_StressTensorDivergence->multiply_vec_then_add(
   	VEC_TauhatMinus1overLd, VEC_rhs_A_Velocity, 1.0, 1.0 ) ;

   // Buoyancy
   if ( b_with_buoyancy )
     VEC_rhs_A_Velocity->sum( VEC_rhs_VelocityBuoyancy, -1.0 );
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem:: assemble_velocity_viscous_matrix_rhs( 
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem:: assemble_velocity_unsteady_matrix( 
	double const& coef )
//----------------------------------------------------------------------
{
   MAC_LABEL( 
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem:: assemble_pdivv_matrix_rhs( double const& coef )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: assemble_pdivv_matrix_rhs" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem:: assemble_velocity_advection( 
	string const& AdvectionScheme,
      	size_t advecting_level, double const& coef, 
	size_t advected_level  )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: assemble_velocity_advection" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem:: assemble_pressure_laplacian_matrix_rhs( 
	double const& coef_lap )
//----------------------------------------------------------------------
{
   MAC_LABEL( 
...
//----------------------------------------------------------------------
void 
VPH_ViscoplasticSystem:: pressure_laplacian_correction( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: pressure_laplacian_correction" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem:: assemble_tauGradv_tensor_divergence_matrix( 
	double const& coef )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: "
   	"assemble_tauGradv_tensor_divergence_matrix" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem::initialize_previous_iteration_velocity( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( 
     "VPH_ViscoplasticSystem:: initialize_previous_iteration_velocity" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem::compute_viscoplastic_TauhatMinus1overLd_vector( 
	double const& FISTA_1overL )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: "
   	"compute_viscoplastic_TauhatMinus1overLd_vector" ) ;
   
   VEC_TauhatMinus1overLd->sum( VEC_TENSOR, - FISTA_1overL ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem::compute_viscoplastic_LambdaMinusrd_vector( 
	double const& ALG2_r )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: "
...
//----------------------------------------------------------------------
LA_Vector*
VPH_ViscoplasticSystem::get_tensor_unknown_vector( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: get_tensor_unknown_vector" ) ;
...
//----------------------------------------------------------------------
LA_SeqVector const*
VPH_ViscoplasticSystem:: get_solution_strainrate_tensor( void ) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
LA_Vector*
VPH_ViscoplasticSystem::get_TauhatMinus1overLd_vector( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: get_TauhatMinus1overLd_vector" ) ;
   
   return ( VEC_TauhatMinus1overLd ) ;
...
//----------------------------------------------------------------------
LA_SeqVector const*
VPH_ViscoplasticSystem:: get_solution_stress_tensor( void ) const
//----------------------------------------------------------------------
{
   MAC_LABEL( "MAC_NavierStokesSystem:: get_solution_stress_tensor" ) ;

   DD_NUM->scatter()->get( VEC_TauhatMinus1overLd, DD_LOC ) ;
...
//----------------------------------------------------------------------
LA_Vector*
VPH_ViscoplasticSystem::get_LambdaMinusrd_vector( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: get_LambdaMinusrd_vector" ) ;
...
//----------------------------------------------------------------------
LA_SeqVector const*
VPH_ViscoplasticSystem:: get_solution_lambda_tensor( void ) const
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: get_solution_lambda_tensor" ) ;
...
//----------------------------------------------------------------------
void
VPH_ViscoplasticSystem::add_storable_objects(
	MAC_ListIdentity* list ) const
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: add_storable_objects" ) ; 
...
//----------------------------------------------------------------------
LA_Vector*
VPH_ViscoplasticSystem::get_buoyancy_vector( void ) 
//----------------------------------------------------------------------
{
   MAC_LABEL( "VPH_ViscoplasticSystem:: get_buoyancy_vector" ) ; 

   return ( VEC_rhs_VelocityBuoyancy ) ;
