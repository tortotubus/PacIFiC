#ifndef VPH_VISCOPLASTIC_SYSTEM_HH
#define VPH_VISCOPLASTIC_SYSTEM_HH
...
class VPH_ViscoplasticSystem : public MAC_Object
{
   ...
   //-- Constructors & Destructor
      ...
      VPH_ViscoplasticSystem( void ) ;

      /** @brief Destructor */       
      virtual ~VPH_ViscoplasticSystem( void ) ;	

      /** @brief Copy constructor */       
      VPH_ViscoplasticSystem( VPH_ViscoplasticSystem const& other ) ;

      /** @brief Operator == 
      @param other the right hand side */   
      VPH_ViscoplasticSystem& operator=( 
      	VPH_ViscoplasticSystem const& other ) ;
      ...
      @param b_with_buoyancy whether to add the buoyancy term to the rhs */      
      VPH_ViscoplasticSystem ( MAC_Object* a_owner,
   ...
   //-- Instance delivery and initialization
      ...
      @param b_pressure_rescaling_ rescale the pressure in case of all 
      	non-Dirichlet pressure BCs
      @param b_with_buoyancy whether to add the buoyancy term to the rhs */	
      static VPH_ViscoplasticSystem* create( MAC_Object* a_owner,
	MAC_ModuleExplorer const* exp,
        FV_DiscreteField* mac_uu,
        FV_DiscreteField* mac_pp,
	FV_DiscreteField* mac_dd,
   ...
   //-- Access
      ...
      /** @brief Return the lambda-r*d vector */  
      LA_Vector* get_LambdaMinusrd_vector( void ) ;
      
      /** @brief Return the lambda tensor solution vector */  
      LA_SeqVector const* get_solution_lambda_tensor( void ) const ;
            
      /** @brief Return the buoyancy vector */  
      LA_Vector* get_buoyancy_vector( void ) ;      
      //@}
   ...
   //-- Basic operations on matrices & vectors
      ...
      /** @brief Initialize the lambda rhs vector with field values */
      void initialize_lambda( size_t lambda_level );
      
      /** @brief Initialize the d rhs vector with field values */
      void initialize_d( size_t d_level );              
	...
	bool const& b_with_advection ) ;
      ...
      /** @brief Compute the viscoplastic vector lambda-r*d, i.e.,
      "Lagrange multiplier lambda minus r dot strain rate tensor d"
      with field values. Note: in fact what this method does is 
      VEC_LambdaMinusrd =- r * VEC_TENSOR, so this assumes that
      VEC_LambdaMinusrd already has lambda and VEC_TENSOR already has d 
      @param ALG2_r ALG2 augmentation parameter */
      void compute_viscoplastic_LambdaMinusrd_vector( double const& ALG2_r );	
      //@}
   ...
   //-- Solver
      ...
      void Uzawa_NavierStokes_solver( const string &VPsolver, 
   ...
   //-- Attributes
      ...
      // Buoyancy rhs
      LA_Vector * VEC_rhs_VelocityBuoyancy ; 
      ...
      // Specific to viscoplastic ALG2 & FISTA
      LA_SeqVector * DD_LOC ;      
