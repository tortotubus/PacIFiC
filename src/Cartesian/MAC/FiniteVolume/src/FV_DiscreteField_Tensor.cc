...
FV_DiscreteField_Tensor const *FV_DiscreteField_Tensor::PROTOTYPE =
    new FV_DiscreteField_Tensor("tensor");

//----------------------------------------------------------------------
FV_DiscreteField_Tensor::FV_DiscreteField_Tensor(std::string const &a_type)
    //----------------------------------------------------------------------
...
//----------------------------------------------------------------------
FV_DiscreteField *FV_DiscreteField_Tensor::create_replica(
    MAC_Object *a_owner, FV_Mesh const *a_primary_mesh,
    std::string const &a_name, std::string const &a_type,
    size_t a_nb_components, size_t a_depth) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Tensor:: create_replica");

  FV_DiscreteField *result = new FV_DiscreteField_Tensor(
      a_owner, a_primary_mesh, a_name, a_type, a_nb_components, a_depth);

  return (result);
}

//----------------------------------------------------------------------
FV_DiscreteField *FV_DiscreteField_Tensor::create_clone_replica(
    MAC_Object *a_owner, FV_Mesh const *a_primary_mesh,
    std::string const &a_name, std::string const &a_type) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Tensor:: create_clone_replica");

  FV_DiscreteField *result =
      new FV_DiscreteField_Tensor(a_owner, a_primary_mesh, a_name, a_type);
...
FV_DiscreteField_Tensor::FV_DiscreteField_Tensor(MAC_Object *a_owner,
                                                 ...
                                                 size_t a_depth)
    //----------------------------------------------------------------------
    : FV_DiscreteField(a_owner, a_primary_mesh, a_name, a_type, a_nb_components,
                       a_depth) {
  MAC_LABEL("FV_DiscreteField_Tensor:: FV_DiscreteField_Tensor");
  ...
  local_min_index_in_global = new vector<size_t_vector>(NB_COMPS, work_size_t);
  local_max_index_in_global = new vector<size_t_vector>(NB_COMPS, work_size_t);
  ...
  max_index_unknown_handled_by_proc =
      new vector<size_t_vector>(NB_COMPS, work_size_t);
  min_index_unknown_on_proc = new vector<size_t_vector>(NB_COMPS, work_size_t);
  for (size_t comp = 0; comp < NB_COMPS; ++comp)
    for (size_t i = 0; i < DIM; ++i)
      (*min_index_unknown_on_proc)[comp](i) = 1000000;
  max_index_unknown_on_proc = new vector<size_t_vector>(NB_COMPS, work_size_t);
      ...
      if ((*PERIODIC)(i)) {
        ...
        for (size_t j = 0; j < np(i) - 1; ++j)
          (*global_main_coordinates)[comp][i](j) =
              0.5 * ((*primary_mesh_global_main_coordinates)[i](j) +
      ...
      if ((*PERIODIC)(i)) {
        ...
        (*local_min_index_in_global)[comp](i) =
            (*primary_mesh_local_min_index_in_global)(i) == 0
                ? 0
                : (*primary_mesh_local_min_index_in_global)(i) + 1;
        (*local_max_index_in_global)[comp](i) =
  ...
  vector<size_t_vector> *vwork_size_t =
      new vector<size_t_vector>(DIM, work_size_t);
  on_current_processor =
      new vector<vector<size_t_vector>>(NB_COMPS, *vwork_size_t);
  delete vwork_size_t;
  // Diagonal components (number of such components = DIM)
  for (size_t comp = 0; comp < DIM; ++comp)
    for (size_t i = 0; i < DIM; ++i) {
      ...
      for (size_t j = 0; j < security_bandwidth; ++j) {
        ...
        if (!(*primary_mesh_handles_node)[i](
          ...
          if ((*local_max_index_in_global)[comp](i) ==
            ...
            (*on_current_processor)[comp][i]((*local_dof_number)[comp](i) - 1 -
                                             j) = 4;
          else
            (*on_current_processor)[comp][i]((*local_dof_number)[comp](i) - 1 -
                                             j) = 2;
      ...
      if (i != dir_ref) {
        ...
        for (size_t j = 0; j < security_bandwidth; ++j) {
          ...
          if (!(*primary_mesh_handles_node)[i](
            ...
            if ((*local_max_index_in_global)[comp](i) ==
                (*global_max_index)[comp](i))
              (*on_current_processor)[comp][i]((*local_dof_number)[comp](i) -
                                               1 - j) = 4;
            else
              (*on_current_processor)[comp][i]((*local_dof_number)[comp](i) -
                                               1 - j) = 2;
      ...
      if (i != dir_ref) {
        ...
        if ((*on_current_processor)[comp][i](index_max) == 2 &&
            (*local_max_index_in_global)[comp](i) !=
                (*global_max_index)[comp](i))
          for (size_t j = 0; j < security_bandwidth + 1; ++j)
            (*on_current_processor)[comp][i](index_max - security_bandwidth -
                                             j) = 1;
      } else {
        if ((*on_current_processor)[comp][i](0) == 2 &&
            (*local_min_index_in_global)[comp](i) != 0)
  ...
  for (size_t comp = 0; comp < DIM; ++comp)
    for (size_t i = 0; i < DIM; ++i) {
      ...
      if ((*local_max_index_in_global)[comp](i) ==
          ...
          (*PERIODIC)(i))
        for (size_t j = 0; j < security_bandwidth; ++j)
          if ((*on_current_processor)[comp][i](index_max - security_bandwidth -
                                               j) == 1)
            (*on_current_processor)[comp][i](index_max - security_bandwidth -
                                             j) = 5;
          else
    ...
    for (size_t i = 0; i < DIM; ++i) {
      if (i != dir_ref) {
        ...
        if ((*local_max_index_in_global)[comp](i) ==
            ...
            (*PERIODIC)(i))
          for (size_t j = 0; j < security_bandwidth + 1; ++j)
            if ((*on_current_processor)[comp][i](index_max -
                                                 security_bandwidth - j) == 1)
              (*on_current_processor)[comp][i](index_max - security_bandwidth -
                                               j) = 5;
            else
              (*on_current_processor)[comp][i](index_max - security_bandwidth -
  ...
  vector<doubleArray3D> *work_values =
      new vector<doubleArray3D>(NB_COMPS, *work_doubleArray3D);
    ...
    for (size_t i = 0; i < DIM; ++i)
      if ((*PERIODIC)(i)) {
        ...
        (*PERIODIC_SHIFT)[comp].index(i) =
            (*global_main_coordinates)[comp][i].size() - 2 * security_bandwidth;
      }
  ...
  vector<doubleVector> const *primary_coordinates =
      PRIMARY_GRID->get_local_main_coordinates();
  size_t shift = 0, start_index = 0, end_index = 0;
  // Diagonal components (number of such components = DIM)
  for (size_t comp = 0; comp < DIM; ++comp)
    for (size_t i = 0; i < DIM; ++i) {
      (*local_cell_size)[comp][i].re_initialize((*local_dof_number)[comp](i),
                                                0.);
      if ((*local_min_index_in_global)[comp](i) == 0 && !(*PERIODIC)(i)) {
        shift = 1;
        start_index = 1;
      ...
      if ((*local_max_index_in_global)[comp](i) ==
        ...
        end_index = (*local_dof_number)[comp](i) - 1;
      ...
      if (i != dir_ref) {
        ...
        if ((*local_min_index_in_global)[comp](i) == 0) {
          ...
          if ((*PERIODIC)(i))
            ...
            (*local_cell_size)[comp][i](0) =
                0.5 * ((*local_main_coordinates)[comp][i](1) -
                       (*local_main_coordinates)[comp][i](0));
        } else
          start_index = 0;
        ...
        if ((*local_max_index_in_global)[comp](i) ==
          ...
          if ((*PERIODIC)(i))
            ...
            (*local_cell_size)[comp][i](end_index + 1) =
                0.5 * ((*local_main_coordinates)[comp][i](end_index + 1) -
                       (*local_main_coordinates)[comp][i](end_index));
        } else
          end_index = (*local_dof_number)[comp](i) - 1;
        ...
        for (size_t j = start_index; j <= end_index; ++j)
          (*local_cell_size)[comp][i](j) =
              0.5 * ((*primary_mesh_global_main_coordinates)[i](
...
//----------------------------------------------------------------------
void FV_DiscreteField_Tensor::print(std::ostream &os, size_t indent_width,
                                    bool b_values) const
//----------------------------------------------------------------------
{
  ...
  for (size_t comp = 0; comp < NB_COMPS; ++comp) {
    ...
    for (size_t i = 0; i < DIM; ++i) {
      ...
      os << space << "Min index = " << 0
         << "  Max index = " << (*global_max_index)[comp](i) << endl;
  ...
  os << space << "Local field on processor " << MAC_Exec::communicator()->rank()
     << endl;
  os << space << "---------------------------------" << endl;
  os << space << "Mesh" << endl;
  os << space << "----" << endl;
...
//----------------------------------------------------------------------
void FV_DiscreteField_Tensor::build_BCs(MAC_ModuleExplorer const *exp,
      ...
      for (size_t j = 0; j < (*local_dof_number)[comp](1); ++j)
        for (size_t k = 0; k < kmax; ++k)
          if ((*DOFcolors)[comp](i, j, k) > 1)
            (*V_BCS)[(*DOFcolors)[comp](i, j, k)]->add_MacTriplet(comp, i, j,
                                                                  k);
  }
...
//----------------------------------------------------------------------
void FV_DiscreteField_Tensor::set_DOF_status_on_Neumann_BC_3D(
    size_t comp, std::string const &mainColor0, std::string const &mainColor1)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
double FV_DiscreteField_Tensor::get_DOF_coordinate(size_t i, size_t component,
...
//----------------------------------------------------------------------
double
FV_DiscreteField_Tensor::get_DOF_coordinate_Assembling(int i, size_t component,
                                                       size_t direction) const
//----------------------------------------------------------------------
{
  ...
  double result = i < 0 ? 0.
                  : i >= int((*local_dof_number)[component](direction))
                      ? 0.
                      : (*local_main_coordinates)[component][direction](i);

  return (result);
}

//----------------------------------------------------------------------
bool FV_DiscreteField_Tensor::is_global_triplet(int i, int j, int k,
...
//----------------------------------------------------------------------
{
  ...
  bool result = (i >= 0 && i <= int((*global_max_index)[component](0)) &&
...
//----------------------------------------------------------------------
bool FV_DiscreteField_Tensor::is_global_triplet_local_DOF(
...
//----------------------------------------------------------------------
{
  ...
  MAC_CHECK_PRE(IMPLIES(DIM == 2, k == 0));
  MAC_CHECK_PRE(IMPLIES(DIM == 3, k <= (*global_max_index)[component](2)));

  bool result = (i >= (*local_min_index_in_global)[component](0) &&
...
//----------------------------------------------------------------------
void FV_DiscreteField_Tensor::set_postprocessing_options(
    std::string const &a_location, std::string const &a_paraview_fname)
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField:: set_post_processing_options");
  MAC_CHECK_PRE(a_location == "at_cell_centers" || a_location == "at_vertices");
  ...
  if (LOCATION != "at_vertices") {
    ostringstream mesg;
    mesg << "*** Unable to save field of name \"" << FNAME
...
//----------------------------------------------------------------------
void FV_DiscreteField_Tensor::write_field(MAC_Module *point_data,
...
//----------------------------------------------------------------------
{
  ...
  // Surement mieux � faire, mais je ne sais pas faire simple avec un pointeur
  // pour �viter la recopie (GVi)
  ...
  size_t nelem2 = DIM == 2 ? 1 : PRIMARY_GRID->get_local_nb_points(2);

  size_t shift_i =
      (*local_min_index_in_global)[0](0) == 0 && !(*PERIODIC)(0) ? 1 : 0;
  size_t shift_j =
      (*local_min_index_in_global)[0](1) == 0 && !(*PERIODIC)(1) ? 1 : 0;
  size_t shift_k = DIM == 2 ? 0
                   : (*local_min_index_in_global)[0](2) == 0 && !(*PERIODIC)(2)
                       ? 1
                       : 0;
  ...
  if (scalar && !target->has_entry("Scalars"))
    target->add_entry("Scalars", MAC_String::create(target, PARAVIEW_FNAME));
  else if (!scalar && !target->has_entry("Vectors"))
    target->add_entry("Vectors", MAC_String::create(target, PARAVIEW_FNAME));

  target->add_entry(PARAVIEW_FNAME, MAC_DoubleArray2D::create(target, X));
}

//----------------------------------------------------------------------
void FV_DiscreteField_Tensor::set_nb_dof_post_processing(void)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
double FV_DiscreteField_Tensor::interpolate_values_at_nodes(
    size_t i, size_t shift_i, size_t j, size_t shift_j, size_t k,
    size_t shift_k, size_t component, size_t level) const
//----------------------------------------------------------------------
{
  ...
  double result = 0.;
  double coeff_bottom_left, coeff_top_left, coeff_bottom_right, coeff_top_right;
  double coeff_behind_bottom_left, coeff_behind_top_left,
      coeff_behind_bottom_right, coeff_behind_top_right,
      coeff_front_bottom_left, coeff_front_top_left, coeff_front_bottom_right,
  ...
  if (DIM == 2) {
    ...
    // left
      ...
      result =
          (coeff_top_left * (*VALUES)[level][component](i, j + shift_j, k) +
           coeff_bottom_left *
    ...
    // right
      ...
      result = (coeff_top_right *
                ...
                coeff_bottom_right * (*VALUES)[level][component](
                                         i + shift_i, j + shift_j - 1, k)) /
    ...
    // bottom
      ...
      result =
          (coeff_bottom_right * (*VALUES)[level][component](i + shift_i, j, k) +
           coeff_bottom_left *
    ...
    // top
      ...
      result = (coeff_top_right *
                    (*VALUES)[level][component](i + shift_i, j + shift_j, k) +
                coeff_top_left * (*VALUES)[level][component](i + shift_i - 1,
                                                             j + shift_j, k)) /
    ...
    // interior
      ...
      result = (coeff_top_right *
                ...
                coeff_bottom_right * (*VALUES)[level][component](
                                         i + shift_i, j + shift_j - 1, k) +
                coeff_bottom_left * (*VALUES)[level][component](
                                        i + shift_i - 1, j + shift_j - 1, k)) /
               (coeff_top_right + coeff_top_left + coeff_bottom_right +
                coeff_bottom_left);
    }
  } else {
    // behind_bottom_left
    ...
    // front_top_left
    else if ((*DOFcolors)[0](i, j + shift_j, k + shift_k) ==
    ...
    // bottom_right
    else if ((*DOFcolors)[0](i + shift_i, j, k + shift_k - 1) ==
             FV_BC_BOTTOM_RIGHT) {
    ...
    // top_left
    else if ((*DOFcolors)[0](i, j + shift_j, k + shift_k - 1) ==
             FV_BC_TOP_LEFT) {
      ...
      result = (coeff_front_top_left *
                ...
                coeff_behind_top_left * (*VALUES)[level][component](
                                            i, j + shift_j, k + shift_k - 1)) /
    ...
    // top_right
      ...
      result =
          (coeff_front_top_right * (*VALUES)[level][component](
                                       i + shift_i, j + shift_j, k + shift_k) +
           coeff_behind_top_right *
               (*VALUES)[level][component](i + shift_i, j + shift_j,
                                           k + shift_k - 1)) /
    ...
    // behind_right
    else if ((*DOFcolors)[0](i + shift_i, j + shift_j - 1, k) ==
             FV_BC_BEHIND_RIGHT) {
    ...
    // front_left
    else if ((*DOFcolors)[0](i, j + shift_j - 1, k + shift_k) ==
             FV_BC_FRONT_LEFT) {
    ...
    // front_right
      ...
      result =
          (coeff_front_top_right * (*VALUES)[level][component](
                                       i + shift_i, j + shift_j, k + shift_k) +
           coeff_front_bottom_right *
               (*VALUES)[level][component](i + shift_i, j + shift_j - 1,
                                           k + shift_k)) /
    ...
    // behind_top
    else if ((*DOFcolors)[0](i + shift_i - 1, j + shift_j, k) ==
             FV_BC_BEHIND_TOP) {
    ...
    // front_top
      ...
      result =
          (coeff_front_top_right * (*VALUES)[level][component](
                                       i + shift_i, j + shift_j, k + shift_k) +
           coeff_front_top_left * (*VALUES)[level][component](i + shift_i - 1,
                                                              j + shift_j,
                                                              k + shift_k)) /
          (coeff_front_top_right + coeff_front_top_left);
    }
    // left
    else if ((*DOFcolors)[0](i, j + shift_j - 1, k + shift_k - 1) ==
             FV_BC_LEFT) {
      coeff_behind_bottom_left = (*local_cell_size)[0][1](j + shift_j - 1) *
      ...
      result =
          (coeff_behind_bottom_left *
           ...
           coeff_front_top_left * (*VALUES)[level][component](
                                      i, j + shift_j - 1, k + shift_k - 1)) /
    ...
    // bottom
    else if ((*DOFcolors)[0](i + shift_i - 1, j, k + shift_k - 1) ==
             FV_BC_BOTTOM) {
      coeff_behind_bottom_left = (*local_cell_size)[0][0](i + shift_i - 1) *
                                 (*local_cell_size)[0][2](k + shift_k - 1);
      coeff_behind_bottom_right = (*local_cell_size)[0][0](i + shift_i) *
      ...
      result =
          (coeff_behind_bottom_left *
           ...
           coeff_front_bottom_left *
               (*VALUES)[level][component](i + shift_i, j, k + shift_k - 1) +
           coeff_front_bottom_right *
               (*VALUES)[level][component](i + shift_i - 1, j,
                                           k + shift_k - 1)) /
    ...
    // top
      ...
      result =
          (coeff_behind_top_left * (*VALUES)[level][component](
                                       i + shift_i, j + shift_j, k + shift_k) +
           coeff_behind_top_right * (*VALUES)[level][component](i + shift_i - 1,
                                                                j + shift_j,
                                                                k + shift_k) +
           coeff_front_top_left * (*VALUES)[level][component](i + shift_i,
                                                              j + shift_j,
                                                              k + shift_k - 1) +
           coeff_front_top_right *
               (*VALUES)[level][component](i + shift_i - 1, j + shift_j,
                                           k + shift_k - 1)) /
          (coeff_behind_top_left + coeff_behind_top_right +
           coeff_front_top_left + coeff_front_top_right);
    }
    // behind
    else if ((*DOFcolors)[0](i + shift_i - 1, j + shift_j - 1, k) ==
             FV_BC_BEHIND) {
      coeff_behind_top_right = (*local_cell_size)[0][0](i + shift_i) *
                               (*local_cell_size)[0][1](j + shift_j);
      coeff_behind_top_left = (*local_cell_size)[0][0](i + shift_i - 1) *
      ...
      result =
          (coeff_behind_top_right *
           ...
           coeff_behind_bottom_right *
               (*VALUES)[level][component](i + shift_i, j + shift_j - 1, k) +
           coeff_behind_bottom_left *
               (*VALUES)[level][component](i + shift_i - 1, j + shift_j - 1,
                                           k)) /
          (coeff_behind_top_right + coeff_behind_top_left +
           coeff_behind_bottom_right + coeff_behind_bottom_left);
    }
...
//----------------------------------------------------------------------
double FV_DiscreteField_Tensor::get_cell_size(size_t i, size_t component,
                                              size_t direction) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
double FV_DiscreteField_Tensor::get_cell_measure(size_t i, size_t j, size_t k,
                                                 size_t component) const
//----------------------------------------------------------------------
{
  ...
  MAC_CHECK_PRE(IMPLIES(DIM == 2, k == 0));
  MAC_CHECK_PRE(
      IMPLIES(DIM == 3, check_invariant_cell_features(k, component, 2)));

  double result =
      (*local_cell_size)[component][0](i) * (*local_cell_size)[component][1](j);
  if (DIM == 3)
...
//----------------------------------------------------------------------
double FV_DiscreteField_Tensor::get_face_perp_to_direction_measure(
    size_t i, size_t j, size_t k, size_t component, size_t direction) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Tensor:: get_face_perp_to_direction_measure");
  MAC_CHECK_PRE(direction < DIM);
  MAC_CHECK_PRE(component < NB_COMPS);
  MAC_CHECK_PRE(check_invariant_cell_features(i, component, 0));
  MAC_CHECK_PRE(check_invariant_cell_features(j, component, 1));
...
//----------------------------------------------------------------------
size_t FV_DiscreteField_Tensor::get_min_index_unknown_handled_by_proc(
    size_t component, size_t direction) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Tensor:: get_min_index_unknown_handled_by_proc");
  MAC_CHECK_PRE(direction < DIM);
  MAC_CHECK_PRE(component < NB_COMPS);
...
//----------------------------------------------------------------------
size_t FV_DiscreteField_Tensor::get_max_index_unknown_handled_by_proc(
    size_t component, size_t direction) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Tensor:: get_max_index_unknown_handled_by_proc");
  MAC_CHECK_PRE(direction < DIM);
  MAC_CHECK_PRE(component < NB_COMPS);
...
//----------------------------------------------------------------------
size_t
FV_DiscreteField_Tensor::get_min_index_unknown_on_proc(size_t component,
                                                       size_t direction) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Tensor:: get_min_index_unknown_on_proc");
  MAC_CHECK_PRE(direction < DIM);
  MAC_CHECK_PRE(component < NB_COMPS);
...
//----------------------------------------------------------------------
size_t
FV_DiscreteField_Tensor::get_max_index_unknown_on_proc(size_t component,
                                                       size_t direction) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Tensor:: get_max_index_unknown_on_proc");
  MAC_CHECK_PRE(direction < DIM);
  MAC_CHECK_PRE(component < NB_COMPS);
...
//----------------------------------------------------------------------
size_t FV_DiscreteField_Tensor::get_local_nb_dof(size_t component,
                                                 size_t direction) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Tensor:: get_local_nb_dof");
  MAC_CHECK_PRE(direction < DIM);
  MAC_CHECK_PRE(component < NB_COMPS);
...
//----------------------------------------------------------------------
bool FV_DiscreteField_Tensor::DOF_has_imposed_Dirichlet_value(
    size_t i, size_t j, size_t k, size_t component) const
//----------------------------------------------------------------------
{
  ...
  MAC_CHECK_PRE(IMPLIES(DIM == 2, k == 0));
  MAC_CHECK_PRE(IMPLIES(DIM == 3, k <= (*local_dof_number)[component](2)));

  bool result = false;
  if ((*DOFcolors)[component](i, j, k) > 1)
    result = get_BC((*DOFcolors)[component](i, j, k))->is_dirichlet(component);

  return (result);
}

//----------------------------------------------------------------------
bool FV_DiscreteField_Tensor::DOF_on_BC(size_t i, size_t j, size_t k,
                                        size_t component) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
bool FV_DiscreteField_Tensor::DOF_is_unknown_handled_by_proc(
    size_t i, size_t j, size_t k, size_t component) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
FV_SHIFT_TRIPLET
FV_DiscreteField_Tensor::shift_staggeredToTensor(size_t component) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
FV_SHIFT_TRIPLET
FV_DiscreteField_Tensor::shift_tensorToTensor(size_t component) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
bool FV_DiscreteField_Tensor::check_invariant_cell_features(
    size_t i, size_t component, size_t direction) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void FV_DiscreteField_Tensor::compute_normLinf(double time) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void FV_DiscreteField_Tensor::create_transproj_interpolation(void)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void FV_DiscreteField_Tensor::translation_projection(
    size_t const &level, size_t const &temporary_level, bool translate_mesh,
...
//----------------------------------------------------------------------
{
  ...
  if (MAC_Exec::communicator()->rank() == 0)
    FV::out()
        << "Field " << FNAME << " is of the FV_DiscreteField_Tensor "
...
//----------------------------------------------------------------------
void FV_DiscreteField_Tensor::restore_translated_field_mesh(void)
//----------------------------------------------------------------------
{
  ...
  if (fabs(trans_dist) > 1e-12) {
    if (MAC_Exec::communicator()->rank() == 0)
      FV::out() << "      Restore translated mesh of field " << FNAME << endl;
    for (size_t comp = 0; comp < NB_COMPS; ++comp) {
      ncoor = (*global_main_coordinates)[comp][trans_dir].size();
      for (size_t i = 0; i < ncoor; ++i)
...
//----------------------------------------------------------------------
void FV_DiscreteField_Tensor::translate_field_mesh(const size_t &trans_dir,
...
//----------------------------------------------------------------------
double FV_DiscreteField_Tensor::interpolateOneCompOnAnotherComp(
    size_t i, size_t j, size_t comp1, size_t comp2, size_t level,
  ...
  if (comp2 < DIM) {
    ...
    // Interpolation of Dxy on Dxx and Dyy
      ...
      if (DOF_color(i, j, k, comp2) == FV_BC_INTERIOR)
        value = 0.25 *
                (DOF_value(i + shift.i, j + shift.j, k, comp1, level) +
                 DOF_value(i + shift.i, j + shift.j - 1, k, comp1, level) +
                 DOF_value(i + shift.i - 1, j + shift.j, k, comp1, level) +
                 DOF_value(i + shift.i - 1, j + shift.j - 1, k, comp1, level));

      else if (DOF_color(i, j, k, comp2) == FV_BC_LEFT)
        value = 0.5 * (DOF_value(i, j + shift.j, k, comp1, level) +
                       DOF_value(i, j + shift.j - 1, k, comp1, level));
      else if (DOF_color(i, j, k, comp2) == FV_BC_RIGHT)
        value = 0.5 *
                (DOF_value(i + shift.i - 1, j + shift.j, k, comp1, level) +
                 DOF_value(i + shift.i - 1, j + shift.j - 1, k, comp1, level));
      else if (DOF_color(i, j, k, comp2) == FV_BC_BOTTOM)
        value = 0.5 * (DOF_value(i + shift.i, j, k, comp1, level) +
                       DOF_value(i + shift.i - 1, j, k, comp1, level));
    ...
    if (comp1 < DIM) {
      if (DOF_color(i, j, k, comp2) == FV_BC_INTERIOR) {
        ...
        value = (coeff_top_right *
                 ...
                 coeff_bottom_right *
                     DOF_value(i + shift.i, j + shift.j - 1, k, comp1, level) +
                 coeff_bottom_left * DOF_value(i + shift.i - 1, j + shift.j - 1,
                                               k, comp1, level)) /
                (coeff_top_right + coeff_top_left + coeff_bottom_right +
                 coeff_bottom_left);
      } else if (DOF_color(i, j, k, comp2) == FV_BC_LEFT) {
        ...
        value = (coeff_top_left * DOF_value(i, j + shift.j, k, comp1, level) +
                 coeff_bottom_left *
                     DOF_value(i, j + shift.j - 1, k, comp1, level)) /
                (coeff_top_left + coeff_bottom_left);
      } else if (DOF_color(i, j, k, comp2) == FV_BC_RIGHT) {
        ...
        coeff_bottom_left = get_cell_size(i + shift.i - 1, comp1, 0);
        value =
            (coeff_bottom_right * DOF_value(i + shift.i, j, k, comp1, level) +
             coeff_bottom_left *
                 DOF_value(i + shift.i - 1, j, k, comp1, level)) /
            (coeff_bottom_right + coeff_bottom_left);
      } else if (DOF_color(i, j, k, comp2) == FV_BC_TOP) {
...
//----------------------------------------------------------------------
double FV_DiscreteField_Tensor::interpolateOneCompOnAnotherComp(
    size_t i, size_t j, size_t k, size_t comp1, size_t comp2, size_t level,
    FV_SHIFT_TRIPLET shift) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Tensor:: interpolateOneCompOnAnotherComp");
  ...
  double value = 0.;
  double coeff_bottom_left, coeff_top_left, coeff_bottom_right, coeff_top_right,
      coeff_behind_left, coeff_front_left, coeff_behind_right,
      coeff_front_right, coeff_behind_bottom, coeff_front_bottom,
  ...
  if (comp2 < DIM) {
    ...
    // Interpolation of Dxz on Dxx, Dyy and Dzz
      ...
      if (DOF_color(i, j, k, comp2) == FV_BC_INTERIOR ||
        ...
        value = 0.25 *
                (DOF_value(i + shift.i, j, k + shift.k, comp1, level) +
                 DOF_value(i + shift.i, j, k + shift.k - 1, comp1, level) +
                 DOF_value(i + shift.i - 1, j, k + shift.k, comp1, level) +
                 DOF_value(i + shift.i - 1, j, k + shift.k - 1, comp1, level));
      else if (DOF_color(i, j, k, comp2) == FV_BC_LEFT ||
               DOF_color(i, j, k, comp2) == FV_BC_BOTTOM_LEFT ||
               DOF_color(i, j, k, comp2) == FV_BC_TOP_LEFT)
        value = 0.5 * (DOF_value(i, j, k + shift.k, comp1, level) +
                       DOF_value(i, j, k + shift.k - 1, comp1, level));
      else if (DOF_color(i, j, k, comp2) == FV_BC_RIGHT ||
    ...
    // Interpolation of Dyz on Dxx, Dyy and Dzz
      ...
      if (DOF_color(i, j, k, comp2) == FV_BC_INTERIOR ||
        ...
        value = 0.25 *
                (DOF_value(i, j + shift.j, k + shift.k, comp1, level) +
                 DOF_value(i, j + shift.j, k + shift.k - 1, comp1, level) +
                 DOF_value(i, j + shift.j - 1, k + shift.k, comp1, level) +
                 DOF_value(i, j + shift.j - 1, k + shift.k - 1, comp1, level));
      else if (DOF_color(i, j, k, comp2) == FV_BC_BEHIND ||
               DOF_color(i, j, k, comp2) == FV_BC_BEHIND_LEFT ||
               DOF_color(i, j, k, comp2) == FV_BC_BEHIND_RIGHT)
        value = 0.5 * (DOF_value(i, j + shift.j, k, comp1, level) +
      ...
      if (DOF_color(i, j, k, comp2) == FV_BC_INTERIOR ||
        ...
        value =
            (coeff_front_top *
             ...
             coeff_behind_bottom *
                 DOF_value(i, j + shift.j - 1, k + shift.k - 1, comp1, level)) /
            (coeff_front_top + coeff_front_bottom + coeff_behind_top +
             coeff_behind_bottom);
      } else if (DOF_color(i, j, k, comp2) == FV_BC_BEHIND ||
                 DOF_color(i, j, k, comp2) == FV_BC_BEHIND_LEFT ||
                 DOF_color(i, j, k, comp2) == FV_BC_BEHIND_RIGHT) {
        coeff_behind_top = get_cell_size(j + shift.j, comp1, 1);
        coeff_behind_bottom = get_cell_size(j + shift.j - 1, comp1, 1);
        value = (coeff_behind_top * DOF_value(i, j + shift.j, k, comp1, level) +
                 coeff_behind_bottom *
                     DOF_value(i, j + shift.j - 1, k, comp1, level)) /
        ...
        value = 0.5 * (DOF_value(i, j, k + shift.k, comp1, level) +
               ...
               DOF_color(i, j, k, comp2) == FV_BC_TOP_RIGHT)
        value =
            0.5 * (DOF_value(i, j + shift.j, k + shift.k, comp1, level) +
                   DOF_value(i, j + shift.j, k + shift.k - 1, comp1, level));
      else if (DOF_color(i, j, k, comp2) == FV_BC_BEHIND_BOTTOM ||
               DOF_color(i, j, k, comp2) == FV_BC_BEHIND_BOTTOM_LEFT ||
               DOF_color(i, j, k, comp2) == FV_BC_BEHIND_BOTTOM_RIGHT)
    ...
    // Interpolation of Dyz on Dxy
      ...
      if (DOF_color(i, j, k, comp2) == FV_BC_INTERIOR ||
        ...
        value = (coeff_front_right *
                 ...
                 coeff_behind_right *
                     DOF_value(i + shift.i, j, k + shift.k - 1, comp1, level) +
                 coeff_behind_left * DOF_value(i + shift.i - 1, j,
                                               k + shift.k - 1, comp1, level)) /
                (coeff_front_right + coeff_front_left + coeff_behind_right +
                 coeff_behind_left);
      } else if (DOF_color(i, j, k, comp2) == FV_BC_LEFT ||
                 DOF_color(i, j, k, comp2) == FV_BC_BOTTOM_LEFT ||
                 DOF_color(i, j, k, comp2) == FV_BC_TOP_LEFT)
        value = 0.5 * (DOF_value(i, j, k + shift.k, comp1, level) +
               ...
               DOF_color(i, j, k, comp2) == FV_BC_TOP_RIGHT)
        value =
            0.5 * (DOF_value(i + shift.i, j, k + shift.k, comp1, level) +
                   DOF_value(i + shift.i, j, k + shift.k - 1, comp1, level));
      else if (DOF_color(i, j, k, comp2) == FV_BC_BEHIND ||
               DOF_color(i, j, k, comp2) == FV_BC_BEHIND_BOTTOM ||
               DOF_color(i, j, k, comp2) == FV_BC_BEHIND_TOP) {
        ...
        coeff_behind_left = get_cell_size(i + shift.i - 1, comp1, 0);
        value =
            (coeff_behind_right * DOF_value(i + shift.i, j, k, comp1, level) +
             coeff_behind_left *
                 DOF_value(i + shift.i - 1, j, k, comp1, level)) /
        ...
        value = (coeff_front_right *
                     DOF_value(i + shift.i, j, k + shift.k - 1, comp1, level) +
                 coeff_front_left * DOF_value(i + shift.i - 1, j,
                                              k + shift.k - 1, comp1, level)) /
  ...
  // Interpolation on Dxz
    ...
    if (comp1 < DIM) {
      if (DOF_color(i, j, k, comp2) == FV_BC_INTERIOR ||
        ...
        value = (coeff_front_left * DOF_value(i, j, k + shift.k, comp1, level) +
                 coeff_behind_left *
                     DOF_value(i, j, k + shift.k - 1, comp1, level)) /
    ...
    // Interpolation of Dxy on Dxz
      ...
      if (DOF_color(i, j, k, comp2) == FV_BC_INTERIOR ||
        ...
        value =
            (coeff_front_bottom * DOF_value(i, j, k + shift.k, comp1, level) +
             coeff_behind_bottom *
                 DOF_value(i, j, k + shift.k - 1, comp1, level)) /
        ...
        value = (coeff_front_top *
                     DOF_value(i, j + shift.j - 1, k + shift.k, comp1, level) +
                 coeff_behind_top * DOF_value(i, j + shift.j - 1,
                                              k + shift.k - 1, comp1, level)) /
    ...
    // Interpolation of Dyz on Dxz
      ...
      if (DOF_color(i, j, k, comp2) == FV_BC_INTERIOR ||
        ...
        value = 0.5 * (DOF_value(i, j + shift.j, k, comp1, level) +
               ...
               DOF_color(i, j, k, comp2) == FV_BC_FRONT_RIGHT)
        value =
            0.5 * (DOF_value(i + shift.i, j + shift.j, k, comp1, level) +
                   DOF_value(i + shift.i, j + shift.j - 1, k, comp1, level));
      else if (DOF_color(i, j, k, comp2) == FV_BC_BOTTOM ||
               DOF_color(i, j, k, comp2) == FV_BC_BEHIND_BOTTOM ||
               DOF_color(i, j, k, comp2) == FV_BC_FRONT_BOTTOM) {
        ...
        value = (coeff_top_right *
                     DOF_value(i + shift.i, j + shift.j - 1, k, comp1, level) +
                 coeff_top_left * DOF_value(i + shift.i - 1, j + shift.j - 1, k,
                                            comp1, level)) /
                (coeff_top_right + coeff_top_left);
      } else if (DOF_color(i, j, k, comp2) == FV_BC_BOTTOM_LEFT ||
                 DOF_color(i, j, k, comp2) == FV_BC_BEHIND_BOTTOM_LEFT ||
...
bool FV_DiscreteField_Tensor::DOF_offset(int &i, int &j, int &k,
                                         size_t_vector center,
                                         size_t_vector stencil,
...
//----------------------------------------------------------------------
{
  cout << "!!!WARNING!!! DOF_offset SOULD NOT BE \n"
          "CALLED IN FV_DiscreteField_Tensor "
       << endl;
