...
FV_DiscreteField_Vertex const *FV_DiscreteField_Vertex::PROTOTYPE =
    new FV_DiscreteField_Vertex("vertex");

//----------------------------------------------------------------------
FV_DiscreteField_Vertex::FV_DiscreteField_Vertex(std::string const &a_type)
    //----------------------------------------------------------------------
...
//----------------------------------------------------------------------
FV_DiscreteField *FV_DiscreteField_Vertex::create_replica(
...
//----------------------------------------------------------------------
{
  ...
  FV_DiscreteField *result = new FV_DiscreteField_Vertex(
...
//----------------------------------------------------------------------
FV_DiscreteField *FV_DiscreteField_Vertex::create_clone_replica(
    MAC_Object *a_owner, FV_Mesh const *a_primary_mesh,
...
//----------------------------------------------------------------------
{
  ...
  FV_DiscreteField *result =
      new FV_DiscreteField_Vertex(a_owner, a_primary_mesh, a_name, a_type);
...
FV_DiscreteField_Vertex::FV_DiscreteField_Vertex(MAC_Object *a_owner,
  ...
  min_index_unknown_handled_by_proc = new vector<size_t_vector>(1, work_size_t);
  for (size_t i = 0; i < DIM; ++i)
    (*min_index_unknown_handled_by_proc)[0](i) = 1000000;
  max_index_unknown_handled_by_proc = new vector<size_t_vector>(1, work_size_t);
  min_index_unknown_on_proc = new vector<size_t_vector>(1, work_size_t);
  for (size_t i = 0; i < DIM; ++i)
    (*min_index_unknown_on_proc)[0](i) = 1000000;
  max_index_unknown_on_proc = new vector<size_t_vector>(1, work_size_t);
  ...
  for (size_t i = 0; i < DIM; ++i) {
    (*local_main_coordinates)[0][i] = (*primary_mesh_local_main_coordinates)[i];
  ...
  on_current_processor = new vector<vector<size_t_vector>>(1, *vwork_size_t);
  delete vwork_size_t;
  for (size_t i = 0; i < DIM; ++i) {
    (*on_current_processor)[0][i].re_initialize((*local_dof_number)[0](i), 0);
    ...
    if ((*local_max_index_in_global)[0](i) != (*global_max_index)[0](i))
      for (size_t j = 0; j < security_bandwidth; ++j)
        (*on_current_processor)[0][i]((*on_current_processor)[0][i].size() - j -
                                      1) = 2;
  ...
  doubleArray3D *work_doubleArray3D =
      new doubleArray3D((*local_main_coordinates)[0][0].size(),
  ...
  work_values->clear();
  delete work_values;
  intArray3D *work_intArray3D =
      new intArray3D((*local_main_coordinates)[0][0].size(),
                     (*local_main_coordinates)[0][1].size(),
                     DIM == 2 ? 1 : (*local_main_coordinates)[0][2].size());
  ...
  for (size_t i = 0; i < DIM; ++i)
    if ((*PERIODIC)(i)) {
      ...
      (*PERIODIC_SHIFT)[0].index(i) =
          (*global_main_coordinates)[0][i].size() - 2 * security_bandwidth - 1;
    }
  set_PERIODIC_default(0, periodic_depth);
  if (periodic_depth)
    delete periodic_depth;
  ...
  for (size_t i = 0; i < DIM; ++i) {
    ...
    if ((*local_min_index_in_global)[0](i) == 0) {
      ...
      if ((*PERIODIC)(i))
        ...
        (*local_cell_size)[0][i](0) =
            0.5 * ((*local_main_coordinates)[0][i](1) -
                   (*local_main_coordinates)[0][i](0));
    } else
      start_index = 0;
    ...
    if ((*local_max_index_in_global)[0](i) == (*global_max_index)[0](i)) {
      ...
      if ((*PERIODIC)(i))
        ...
        (*local_cell_size)[0][i](end_index + 1) =
            0.5 * ((*local_main_coordinates)[0][i](end_index + 1) -
                   (*local_main_coordinates)[0][i](end_index));
    } else
      end_index = (*local_dof_number)[0](i) - 1;
...
//----------------------------------------------------------------------
void FV_DiscreteField_Vertex::print(std::ostream &os, size_t indent_width,
                                    bool b_values) const
//----------------------------------------------------------------------
{
  ...
  for (size_t i = 0; i < DIM; ++i) {
    ...
    os << space << "Min index = " << 0
       << "  Max index = " << (*global_max_index)[0](i) << endl;
  ...
  for (size_t i = 0; i < DIM; ++i) {
    ...
    os << space << "Min index = " << (*local_min_index_in_global)[0](i)
       << "  Max index = " << (*local_max_index_in_global)[0](i) << endl;
    os << space << "Coordinate values = ";
...
//----------------------------------------------------------------------
void FV_DiscreteField_Vertex::build_BCs(MAC_ModuleExplorer const *exp,
    ...
    for (size_t j = 0; j < (*local_dof_number)[0](1); ++j)
      for (size_t k = 0; k < kmax; ++k)
        if ((*DOFcolors)[0](i, j, k) > 1)
          (*V_BCS)[(*DOFcolors)[0](i, j, k)]->add_MacTriplet(NB_COMPS, i, j, k);
...
//----------------------------------------------------------------------
double FV_DiscreteField_Vertex::get_DOF_coordinate(size_t i, size_t component,
...
//----------------------------------------------------------------------
double
FV_DiscreteField_Vertex::get_DOF_coordinate_Assembling(int i, size_t component,
                                                       size_t direction) const
//----------------------------------------------------------------------
{
  ...
  double result = i < 0 ? 0.
                  : i >= int((*local_dof_number)[0](direction))
                      ? 0.
                      : (*local_main_coordinates)[0][direction](i);

  return (result);
}

//----------------------------------------------------------------------
bool FV_DiscreteField_Vertex::is_global_triplet(int i, int j, int k,
...
//----------------------------------------------------------------------
{
  ...
  bool result = (i >= 0 && i <= int((*global_max_index)[0](0)) && j >= 0 &&
                 j <= int((*global_max_index)[0](1)));
  if (DIM == 3 && result)
...
//----------------------------------------------------------------------
bool FV_DiscreteField_Vertex::is_global_triplet_local_DOF(
...
//----------------------------------------------------------------------
{
  ...
  bool result = (i >= (*local_min_index_in_global)[0](0) &&
...
//----------------------------------------------------------------------
void FV_DiscreteField_Vertex::set_postprocessing_options(
...
//----------------------------------------------------------------------
void FV_DiscreteField_Vertex::write_field(MAC_Module *point_data,
        ...
        for (size_t j = 0; j < nelem1; ++j)
          for (size_t i = 0; i < nelem0; ++i) {
            if ((*primary_mesh_handles_node)[0](i) &&
              ...
              X(comp, iter) =
                  interpolate_values_at_nodes(i, 0, j, 0, k, 0, comp, 0);
              iter++;
            }
          }
    }
...
//----------------------------------------------------------------------
void FV_DiscreteField_Vertex::set_nb_dof_post_processing(void)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
double FV_DiscreteField_Vertex::interpolate_values_at_nodes(
...
//----------------------------------------------------------------------
double FV_DiscreteField_Vertex::get_cell_size(size_t i, size_t component,
                                              size_t direction) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
double FV_DiscreteField_Vertex::get_cell_measure(size_t i, size_t j, size_t k,
                                                 size_t component) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
double FV_DiscreteField_Vertex::get_face_perp_to_direction_measure(
    size_t i, size_t j, size_t k, size_t component, size_t direction) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Vertex:: get_face_perp_to_direction_measure");
  MAC_CHECK_PRE(direction < DIM);
  MAC_CHECK_PRE(component < NB_COMPS);
  MAC_CHECK_PRE(check_invariant_cell_features(i, 0));
  MAC_CHECK_PRE(check_invariant_cell_features(j, 1));
...
//----------------------------------------------------------------------
size_t FV_DiscreteField_Vertex::get_min_index_unknown_handled_by_proc(
    size_t component, size_t direction) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Vertex:: get_min_index_unknown_handled_by_proc");
  MAC_CHECK_PRE(direction < DIM);
  MAC_CHECK_PRE(component < NB_COMPS);
...
//----------------------------------------------------------------------
size_t FV_DiscreteField_Vertex::get_max_index_unknown_handled_by_proc(
    size_t component, size_t direction) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Vertex:: get_max_index_unknown_handled_by_proc");
  MAC_CHECK_PRE(direction < DIM);
  MAC_CHECK_PRE(component < NB_COMPS);
...
//----------------------------------------------------------------------
size_t
FV_DiscreteField_Vertex::get_min_index_unknown_on_proc(size_t component,
                                                       size_t direction) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Vertex:: get_min_index_unknown_on_proc");
  MAC_CHECK_PRE(direction < DIM);
  MAC_CHECK_PRE(component < NB_COMPS);
...
//----------------------------------------------------------------------
size_t
FV_DiscreteField_Vertex::get_max_index_unknown_on_proc(size_t component,
                                                       size_t direction) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Vertex:: get_max_index_unknown_on_proc");
  MAC_CHECK_PRE(direction < DIM);
  MAC_CHECK_PRE(component < NB_COMPS);
...
//----------------------------------------------------------------------
size_t FV_DiscreteField_Vertex::get_local_nb_dof(size_t component,
                                                 size_t direction) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Vertex:: get_local_nb_dof");
  MAC_CHECK_PRE(direction < DIM);
  MAC_CHECK_PRE(component < NB_COMPS);
...
//----------------------------------------------------------------------
bool FV_DiscreteField_Vertex::DOF_has_imposed_Dirichlet_value(
    size_t i, size_t j, size_t k, size_t component) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
bool FV_DiscreteField_Vertex::DOF_on_BC(size_t i, size_t j, size_t k,
                                        size_t component) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
bool FV_DiscreteField_Vertex::DOF_is_unknown_handled_by_proc(
    size_t i, size_t j, size_t k, size_t component) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
bool FV_DiscreteField_Vertex::check_invariant_cell_features(
    size_t i, size_t direction) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void FV_DiscreteField_Vertex::compute_normLinf(double time) const
...
//----------------------------------------------------------------------
void FV_DiscreteField_Vertex::create_transproj_interpolation(void)
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField_Vertex:: create_transproj_interpolation");

  if (STO_DEPTH < 2)
    FV_DiscreteField_ERROR::n1(FNAME);
  ...
  for (size_t i = 0; i < ncoor; ++i)
    if (FV_Mesh::between(&(*local_main_coordinates)[0][trans_dir],
      ...
      (*transproj_interpolation)[0](i) = OUT_OF_DOMAIN_PROJTRANS;
}

//----------------------------------------------------------------------
void FV_DiscreteField_Vertex::translation_projection(
...
//----------------------------------------------------------------------
{
  ...
  if (!transproj_interpolation)
    FV_DiscreteField_ERROR::n2(FNAME);
...
//----------------------------------------------------------------------
void FV_DiscreteField_Vertex::restore_translated_field_mesh(void)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void FV_DiscreteField_Vertex::translate_field_mesh(const size_t &trans_dir,
...
//----------------------------------------------------------------------
{
  cout << "!!!WARNING!!! DOF_offset SHOULD NOT BE \n"
          "CALLED IN FV_DiscreteField_Vertex "
       << endl;
