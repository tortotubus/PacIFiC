...
void FV_DiscreteField::assemble_constantcoef_laplacian_matrix(
    double const &coef_lap, LA_Matrix *MAT, LA_Vector *VEC_rhs,
...
//----------------------------------------------------------------------------
{
  ...
  double xC, xR, xL, yC, yT, yB, dxr, dxl, dyt, dyb, dxC, dyC, zC, zF, zB, dzf,
      dzb, dzC;
      ...
      for (size_t j = min_unknown_index(1); j <= max_unknown_index(1); ++j) {
        ...
        if (DIM == 2) {
          ...
          if (center_pos_in_matrix || !rescale) {
            ...
            arx = one_DOF_laplacian(MAT, VEC_rhs, rescale, i + 1, j, k,
                                    component, center_unknown_on_BC,
                                    center_pos_in_matrix, arx);

            // Left (X)
            alx = coef_lap * dyC / dxl;
            alx = one_DOF_laplacian(MAT, VEC_rhs, rescale, i - 1, j, k,
                                    component, center_unknown_on_BC,
                                    center_pos_in_matrix, alx);

            // Top (Y)
            aty = coef_lap * dxC / dyt;
            aty = one_DOF_laplacian(MAT, VEC_rhs, rescale, i, j + 1, k,
                                    component, center_unknown_on_BC,
                                    center_pos_in_matrix, aty);

            // Bottom (Y)
            aby = coef_lap * dxC / dyb;
            aby = one_DOF_laplacian(MAT, VEC_rhs, rescale, i, j - 1, k,
                                    component, center_unknown_on_BC,
        ...
        } else // if dim==3
        {
          for (size_t k = min_unknown_index(2); k <= max_unknown_index(2);
            ...
            if (center_pos_in_matrix || !rescale) {
              ...
              afz = one_DOF_laplacian(MAT, VEC_rhs, rescale, i, j, k + 1,
                                      component, center_unknown_on_BC,
                                      center_pos_in_matrix, afz);

              // Behind (Z)
              abz = coef_lap * dxC * dyC / dzb;
              abz = one_DOF_laplacian(MAT, VEC_rhs, rescale, i, j, k - 1,
                                      component, center_unknown_on_BC,
...
//---------------------------------------------------------------------------
double FV_DiscreteField::one_DOF_laplacian(LA_Matrix *MAT, LA_Vector *VEC_rhs,
                                           bool const &rescale, int i, int j,
                                           int k, size_t component,
                                           bool center_unknown_on_BC,
...
//---------------------------------------------------------------------------
void FV_DiscreteField::assemble_mass_matrix(double const &coef, LA_Matrix *MAT,
                                            double const &power_index) const
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void FV_DiscreteField::assemble_mass_vector(double const &coef, LA_Vector *VEC,
                                            double const &power_index) const
//---------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void FV_DiscreteField::assemble_pDivv_matrix(FV_DiscreteField const *UU,
                                             double const &coef, LA_Matrix *MAT,
...
//---------------------------------------------------------------------------
double FV_DiscreteField::SuperBee_phi(double const &theta)
//---------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
double FV_DiscreteField::compute_CFL(FV_TimeIterator const *t_it,
                                     size_t level) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void FV_DiscreteField::assemble_advection_Upwind(
    FV_DiscreteField const *AdvectingField, size_t advecting_level,
    double const &coef, size_t advected_level, LA_Vector *VEC_rhs) const
//----------------------------------------------------------------------
{
  MAC_LABEL("FV_DiscreteField:: assemble_advection_Upwind");

  ostringstream mesg;
  mesg << "Field " << FNAME << " is of the " << FDISCRETIZATION << " type; "
       << "method \"assemble_advection_Upwind\" is not "
       << "implemented for fields of type " << FDISCRETIZATION << " !!" << endl;
  MAC_Error::object()->raise_plain(mesg.str());
}

//----------------------------------------------------------------------
void FV_DiscreteField::assemble_advection_TVD(
    FV_DiscreteField const *AdvectingField, size_t advecting_level,
    double const &coef, size_t advected_level, LA_Vector *VEC_rhs) const
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void FV_DiscreteField::assemble_tauGradv_tensor_divergence_matrix(
    FV_DiscreteField const *DD, double const &coef, LA_Matrix *MAT) const
//----------------------------------------------------------------------
{
