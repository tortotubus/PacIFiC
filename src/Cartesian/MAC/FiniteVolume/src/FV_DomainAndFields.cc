...
//-------------------------------------------------------------------------
FV_DomainAndFields *FV_DomainAndFields::create(MAC_Object *a_owner,
...
FV_DomainAndFields::FV_DomainAndFields(MAC_Object *a_owner,
                                       ...
                                       MAC_Communicator const *com)
    //-------------------------------------------------------------------------
    : MAC_Object(a_owner), EXP(exp->create_clone(this)), BUILDER_FV(0),
      COM(com), BUILDER_WRITER(0) {
  MAC_LABEL("FV_DomainAndFields:: FV_DomainAndFields");

  std::string my_name = "unnamed";
  if (exp->has_entry("name"))
    my_name = exp->string_data("name");
...
//-------------------------------------------------------------------------
std::string const &FV_DomainAndFields::name(void) const
//-------------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------
void FV_DomainAndFields::duplicate_field(std::string const &model_name,
                                         std::string const &name_of_new_field)
//-------------------------------------------------------------------------
{
  ...
  if (!found)
    ...
    FV_DiscreteField *newField =
        modelField->create_clone(BUILDER_FV, name_of_new_field);
    FVFIELDS.push_back(newField);
  }
}

//----------------------------------------------------------------------
void FV_DomainAndFields::append_field(FV_DiscreteField *newField)
//----------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------
bool FV_DomainAndFields::is_distributed(void) const
//-------------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------
size_t FV_DomainAndFields::nb_space_dimensions(void) const
//-------------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------
FV_PostProcessingWriter *FV_DomainAndFields::post_processing_writer(void) const
//-------------------------------------------------------------------------
{
  ...
  if (BUILDER_WRITER == 0) {
    MAC_ModuleExplorer *sexp = EXP->create_subexplorer(0, "FV_ResultSaver");

    if (sexp->has_entry("postprocessingwriter"))
      postprocessingwriter = sexp->string_data("postprocessingwriter");
    else
      postprocessingwriter = "paraview"; // by default

    if (postprocessingwriter != "paraview" &&
        postprocessingwriter != "matlab" &&
        postprocessingwriter != "paraview_vtkhdf") {
      string error_message =
          "  - paraview\n  - matlab\n  - paraview_vtkhdf\n ";
      MAC_Error::object()->raise_bad_data_value(sexp, "postprocessingwriter",
                                                error_message);
    }
    ...
    if ((writing_mode != "text") && (writing_mode != "binary")) {
      ...
      MAC_Error::object()->raise_bad_data_value(sexp, "writing_mode",
                                                error_message);
    }
    ...
    for (; sexp->is_valid_module(); sexp->go_next_module()) {
      ...
      ssexp->test_data_in("where_to_save", "at_vertices,"
                                           "at_cell_centers");
      if (ssexp->has_entry("field")) {
        ...
        if (field) {
          ...
          MAC_Error::object()->raise_plain("Bad syntax in module " +
                                           ssexp->name());
      } else {
        MAC_Error::object()->raise_plain("Bad syntax in module " +
                                         ssexp->name());
    ...
    BUILDER_WRITER = FV_PostProcessingWriter::make(
        const_cast<FV_DomainAndFields *>(this), postprocessingwriter, sexp, COM,
        constFields, primary_grid(), isBinary);
...
//----------------------------------------------------------------------
void FV_DomainAndFields::append_post_processing_field(
...
//-------------------------------------------------------------------------
void FV_DomainAndFields::save_state(MAC_ObjectWriter *writer) const
//-------------------------------------------------------------------------
{
  ...
  // Fields storing :
  for (list<FV_DiscreteField *>::const_iterator il = FVFIELDS.begin();
       il != FVFIELDS.end(); il++)
...
//-------------------------------------------------------------------------
void FV_DomainAndFields::restore_state(MAC_ObjectReader *reader)
//-------------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------
bool FV_DomainAndFields::invariant(void) const
//-------------------------------------------------------------------------
{
  MAC_LABEL("FV_DomainAndFields:: invariant");
  MAC_ASSERT(MAC_Object::invariant());
  MAC_ASSERT(nb_space_dimensions() == 2 || nb_space_dimensions() == 3);
  return (true);
}

//-------------------------------------------------------------------------
void FV_DomainAndFields::print_grid(std::ostream &os, size_t indent_width) const
//-------------------------------------------------------------------------
{
...
//-------------------------------------------------------------------------
bool FV_DomainAndFields::has_discrete_field(std::string const &field_name) const
//-------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
FV_Mesh const *FV_DomainAndFields::primary_grid(void) const
//----------------------------------------------------------------------
{
...
// internal--------------------------------------------------------------
void FV_DomainAndFields_ERROR::n1(std::string const &model_name,
                                  std::string const &name_of_new_field)
// internal--------------------------------------------------------------
{
  ostringstream mesg;
  mesg << "Cannot duplicate field \"" << model_name << "\" in "
       << "field \"" << name_of_new_field << "\" because field \"" << model_name
       << "\" does not exist in FV_DomainAndFields !! " << endl;
