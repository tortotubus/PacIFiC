...
//---------------------------------------------------------------------------
MAC_Error *MAC_Error::object(void)
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void MAC_Error::raise_internal(std::string message)
//---------------------------------------------------------------------------
{
  begin();
...
//---------------------------------------------------------------------------
void MAC_Error::raise_plain(std::string message)
//---------------------------------------------------------------------------
{
  begin();
  os << message << endl << endl;
...
//---------------------------------------------------------------------------
void MAC_Error::display_info(std::string message)
//---------------------------------------------------------------------------
{
  os << endl << hline() << endl;
  os << message << endl;
...
//---------------------------------------------------------------------------
void MAC_Error::raise_not_tested(std::string file, int line, std::string test)
//---------------------------------------------------------------------------
{
  begin();
...
//---------------------------------------------------------------------------
void MAC_Error::raise_not_implemented(MAC_Object const *oo, std::string method)
//---------------------------------------------------------------------------
{
  begin();
  os << "Call to not implemented method : " << method << endl;
  os << "         for an object of type : " << oo->type_name() << endl << endl;
  print_invocated_methods();
  end();
  exit_with_internal_error();
}
...
//---------------------------------------------------------------------------
void MAC_Error::raise_bad_types(MAC_Object const *oo, std::string method,
...
//---------------------------------------------------------------------------
void MAC_Error::raise_read_syntax_error(std::string const &file, int line,
...
//---------------------------------------------------------------------------
{
  ...
  os << nature << endl << endl;
  os << "Last line number " << line << " read in file " << file
     << " was :" << endl;
  os << ">> " << last << " <<" << endl;
  end();
  exit();
...
//---------------------------------------------------------------------------
void MAC_Error::raise_missing_keyword(MAC_ModuleExplorer const *exp,
                                      std::string keyword)
//---------------------------------------------------------------------------
{
  begin();
...
//---------------------------------------------------------------------------
void MAC_Error::raise_missing_module(MAC_ModuleExplorer const *exp,
...
//---------------------------------------------------------------------------
void MAC_Error::raise_module_error(MAC_ModuleExplorer const *exp,
                                   std::string error)
//---------------------------------------------------------------------------
{
  begin();
...
//---------------------------------------------------------------------------
void MAC_Error::raise_bad_data_type(MAC_ModuleExplorer const *exp,
                                    std::string keyword,
...
//---------------------------------------------------------------------------
{
  ...
  os << "should be of type : " << MAC_Data::type_name(query_kind) << endl;
  end();
  exit();
}

//---------------------------------------------------------------------------
void MAC_Error::raise_bad_file(MAC_ModuleExplorer const *exp,
...
//---------------------------------------------------------------------------
void MAC_Error::raise_not_evaluable(MAC_ModuleExplorer const *exp,
                                    std::string keyword,
...
//---------------------------------------------------------------------------
void MAC_Error::raise_bad_data_value(MAC_ModuleExplorer const *exp,
                                     std::string keyword,
...
//---------------------------------------------------------------------------
void MAC_Error::raise_data_error(MAC_ModuleExplorer const *exp,
                                 std::string keyword, std::string error)
//---------------------------------------------------------------------------
{
  begin();
  os << "In module: " << endl << "   " << exp->absolute_path_name() << endl;
...
//---------------------------------------------------------------------------
void MAC_Error::raise_bad_object(std::string message,
...
//---------------------------------------------------------------------------
void MAC_Error::raise_precondition_violation(std::string test)
//---------------------------------------------------------------------------
{
  begin();
  ...
  os << endl;
  os << "HINTS : " << endl;
  os << "   - Meeting a PRECONDITION is the CLIENT\'s responsibility." << endl;
...
//---------------------------------------------------------------------------
void MAC_Error::raise_postcondition_violation(std::string file, int line,
                                              std::string test)
//---------------------------------------------------------------------------
{
  begin();
...
//---------------------------------------------------------------------------
void MAC_Error::raise_invariant_violation(std::string file, int line,
                                          std::string test)
//---------------------------------------------------------------------------
{
  begin();
...
//---------------------------------------------------------------------------
void MAC_Error::raise_assertion_violation(std::string file, int line,
                                          std::string test)
//---------------------------------------------------------------------------
{
  begin();
...
//---------------------------------------------------------------------------
void MAC_Error::raise_file_handling(std::string file, std::string operation)
//---------------------------------------------------------------------------
{
  begin();
...
//---------------------------------------------------------------------------
void MAC_Error::print_invocated_methods(void)
//---------------------------------------------------------------------------
{
  if (MAC_Marker::nb_labels() > 0) {
    os << "Stack of invocated methods :" << endl;
    for (size_t i = MAC_Marker::nb_labels() - 1; i < MAC_Marker::nb_labels();
         i--) {
    ...
    os << "(none of them have been labelled with \'MAC_LABEL\')." << endl;
  }
}

//---------------------------------------------------------------------------
void MAC_Error::print_client_and_server(void)
//---------------------------------------------------------------------------
{
  if (MAC_Marker::nb_labels() > 1) {
    os << "CLIENT   : " << MAC_Marker::label(MAC_Marker::nb_labels() - 2)
       << endl;
  }
  if (MAC_Marker::nb_labels() > 0) {
    os << "SUPPLIER : " << MAC_Marker::label(MAC_Marker::nb_labels() - 1)
       << endl;
    os << "(inferred from the above stack of invocated methods)." << endl;
  }
}

//---------------------------------------------------------------------------
std::string const &MAC_Error::hline(void) const
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void MAC_Error::trace(std::string const &event)
//---------------------------------------------------------------------------
{
  os << endl << hline() << endl;
  os << " EVENT TRACKING ";
...
//-------------------------------------------------------------------------
void MAC_Error::display_new_syntax(
    MAC_ModuleExplorer const *older_module,
...
//---------------------------------------------------------------------------
void MAC_Error::notify(MAC_Module *list, std::string const &message,
                       std::string const &where, stringVector const &choices)
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
MAC_Module *MAC_Error::notify(MAC_Module *list, std::string const &message,
...
//---------------------------------------------------------------------------
void MAC_Error::display_data_checking(MAC_ModuleExplorer *report) const
//---------------------------------------------------------------------------
{
  if (!report->is_empty()) {
    ...
    for (report->start_module_iterator(); report->is_valid_module();
         report->go_next_module()) {
      MAC_ModuleExplorer const *error = report->create_subexplorer(0);
...
//---------------------------------------------------------------------------
void MAC_Error::exit_with_internal_error(void)
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void MAC_Error::exit(size_t status)
//---------------------------------------------------------------------------
{
