...
public: //----------------------------------------------------------------
  ...
  void raise_read_syntax_error(std::string const &file, int line,
  ...
  void raise_not_implemented(MAC_Object const *oo, std::string method);

  //-- Simulation of virtual methods with respect to many objects

  void raise_bad_types(MAC_Object const *oo, std::string method,
  ...
  void raise_module_error(MAC_ModuleExplorer const *exp, std::string error);

  void raise_bad_data_type(MAC_ModuleExplorer const *exp, std::string keyword,
                           MAC_Data::Type query_kind);

  void raise_bad_file(MAC_ModuleExplorer const *exp,
                      std::string const &filename, std::string const &access);

  void raise_not_evaluable(MAC_ModuleExplorer const *exp, std::string keyword,
                           stringVector const &undefined_variables);

  void raise_bad_data_value(MAC_ModuleExplorer const *exp, std::string keyword,
                            std::string allowed_values);

  void raise_data_error(MAC_ModuleExplorer const *exp, std::string keyword,
                        std::string error);
  ...
  void raise_postcondition_violation(std::string file, int line,
                                     std::string test);
  void raise_invariant_violation(std::string file, int line, std::string test);
  void raise_assertion_violation(std::string file, int line, std::string test);

  void raise_bad_object(std::string message, MAC_Object const *a_object);

  //-- File manipulation

  void raise_file_handling(std::string file, std::string operation);
  ...
  static void notify(MAC_Module *list, std::string const &message,
                     std::string const &where, stringVector const &choices);

  static MAC_Module *notify(MAC_Module *list, std::string const &message,
