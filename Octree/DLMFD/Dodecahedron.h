...
//----------------------------------------------------------------------------
bool is_in_Dodecahedron( const double x, const double y, const double z, 
  ...
  *nb = 0;
  ...
  while ( *nb == 0 )
  {
	...
	*lN = floor( lengthedge / ( INTERBPCOEF * Delta ) );
        ...
        /* The numberof points on a dodecahedron edge is the number of 
	intervals + 1 */
        *lN += 1;
      }

      /* number of points required for the 30 edges of the dodecahedron and 
      12*5 inner edges */
      *nb += (*lN-2)*(30 + 12*5);
...
//----------------------------------------------------------------------------
void distribute_points_edge_Dodecahedron( coord const corner1, 
...
//----------------------------------------------------------------------------
{
  ...
  for (int i = 0; i < nfaces; i++) 
  {
    ...
    for(int k = 0; k < npoints; k++)
    {
      ...
      distribute_points_edge_Dodecahedron( refcorner, dir1, dlm_bd, lN, isb );
      isb += lN - 2;    
  ...
  for (int i = 0; i < nfaces; i++) 
  {
	...
	if ( allindextable[jm1][j1] == 0 ) 
	{
	  ...
	  distribute_points_edge_Dodecahedron( c1, c2, dlm_bd, lN, isb );
...
//----------------------------------------------------------------------------
void create_FD_Interior_Dodecahedron( particle* p, vector Index_lambda, 
	vector pshift ) 
//----------------------------------------------------------------------------
{
  Cache* c;
  ...
  foreach() 
  {
	...
	if ( ( z > mincoord.z ) && ( z < maxcoord.z ) )
	  ...
	  /* If yes: check if it is inside the Dodecahedron now */
    	  if ( is_in_Dodecahedron( checkpt.x, checkpt.y, checkpt.z, gp ) ) 
	  {
