/**
# Set of functions for a Trancoctahedron
*/


//----------------------------------------------------------------------------
int determ_posi_plane_Trancoctahedron( coord* pointOne, coord* pointTwo, 
...
/** Tests whether a point lies inside the Trancoctahedron */
//----------------------------------------------------------------------------
bool is_in_Trancoctahedron(const double x, const double y, const double z,
...
//----------------------------------------------------------------------------
{
  ...
  for (int i = 0; i < nfaces; i++) 
  {
    ...
    // and cornerThree
    position[i] = determ_posi_plane_Trancoctahedron( &refcorner, &cornerTwo, 
	        &cornerThree, &checkpt );
  }
...
/** Computes the number of boundary points on the surface of the 
trancoctahedron */
//----------------------------------------------------------------------------
void compute_nboundary_Trancoctahedron( GeomParameter* gcp, int* nb, int* lN )
//----------------------------------------------------------------------------
{
  Cache poscache = {0};
  Point lpoint;
  *nb = 0;
  ...
  while ( *nb == 0 )
  {
    ...
    if ( lpoint.level > -1 )
    {
      ...
      /* Grains sends the trancoctahedron circumscribed radius, so to get the 
      trancoctahedron edge length we multiply by 2./(sqrt(10.)) */
      double lengthedge = gcp->radius*2./(sqrt(10.));

      /* We compute the number of intervals on the trancoctahedron  edge */
      foreach_cache (poscache)
      {
	*lN = floor( lengthedge / ( INTERBPCOEF * Delta ) );

        /* The number of points on a trancoctahedron edge is the number of 
	intervals + 1 */
        *lN += 1;
      }

      /* number of points required for the 36 edges of the trancoctahedron and 
      8*6 inner edges for hexagones*/
      *nb += (*lN-2)*(36 + 8*6);
      /* number of points required for the 6 squares and 8 hexatones for the 
      trancoctahedron */
      *nb += 6.*(*lN-2)*(*lN-2) + 8.*(*lN-2)*(*lN-3)/2.*6.;
      /* number of points required for the 24 corners and 8 central points of 
      each surface*/
      *nb += 24 + 8;
...
/** Distributes points on an edge of the Trancoctahedron */
//----------------------------------------------------------------------------
void distribute_points_edge_Trancoctahedron( coord const corner1, 
...
/** Creates boundary points on the surface of the Trancoctahedron */
//----------------------------------------------------------------------------
void create_FD_Boundary_Trancoctahedron( GeomParameter* gcp, 
...
//----------------------------------------------------------------------------
{
  ...
  for (int i = 0; i < nfaces; i++)  // Assuming now working on 8 hexagones
  {
    npoints = gcp->pgp->numPointsOnFaces[i];
	...
	orig_z += gcp->pgp->cornersCoord[iref][2]/npoints;	
      ...
      // Adding central points on each surface of the 8 hexagones
      dlm_bd->x[isb] = refcorner.x;
      dlm_bd->y[isb] = refcorner.y;
      dlm_bd->z[isb] = refcorner.z;
      isb++;
	...
	distribute_points_edge_Trancoctahedron(refcorner, dir1, dlm_bd, lN, isb);
	isb +=lN-2;    
  ...
  // We have 24 corner points for trancoctahedron
  int allindextable[25][25] = {{0}};
  int j1, jm1;
  ...
  for (int i = 0; i < nfaces; i++) 
  {
	...
	if ( allindextable[jm1][j1] == 0 ) 
	{
	  ...
	  distribute_points_edge_Trancoctahedron( c1, c2, dlm_bd, lN, isb );
  ...
  /* Add the final 24 corners points */
...
/** Finds cells lying inside the Trancoctahedron */
//----------------------------------------------------------------------------
void create_FD_Interior_Trancoctahedron( particle* p, vector Index_lambda, 
	vector pshift ) 
//----------------------------------------------------------------------------
{
  Cache* c;
  
  /** Create the cache of the interior points for a Trancoctahedron*/
  c = &(p->Interior);
  ...
  foreach() 
  {
	...
	if ( ( z > mincoord.z ) && ( z < maxcoord.z ) )
	  ...
	  /* If yes: check if it is inside the Trancoctahedron now */
    	  if ( is_in_Trancoctahedron( checkpt.x, checkpt.y, checkpt.z, gp ) ) 
	  {
...
//----------------------------------------------------------------------------
// Reads geometric parameters of the Trancoctahedron
//----------------------------------------------------------------------------
void update_Trancoctahedron( GeomParameter* gcp)
//----------------------------------------------------------------------------
{
  char* token = NULL;

  // Read number of corners, check that it is 24
  size_t nc = 0;
  token = strtok(NULL, " " );
  sscanf( token, "%lu", &nc );
  if ( nc != 24 )
    printf ("Error in number of corners in update_Trancoctahedron: \n");
  ...
  // Read number of faces, check that it is 14
  size_t nf = 0;
  token = strtok(NULL, " " );
  sscanf( token, "%lu", &nf );
  if ( nf != 14 )
    printf ("Error in number of faces in update_Trancoctahedron\n");
  gcp->pgp->allFaces = nf;
  ...
  {
    // Read the number of points/corners on the face, check that it is 6
    token = strtok(NULL, " " );
    sscanf( token, "%ld", &nppf );
    if ( nppf != 6 )
      printf ("Error in number of corners per face in "
      	"update_Trancoctahedron: \n");
    ...
    // Read the point/corner indices : attention 6 points on a face
  ...
  for (size_t i=8;i<nf;++i)
  {
    ...
    // Read the point/corner indices : attention 4 points on a face
...
// Frees the geometric parameters of the Trancoctahedron
//----------------------------------------------------------------------------
void free_Trancoctahedron( GeomParameter* gcp )
//----------------------------------------------------------------------------
{
