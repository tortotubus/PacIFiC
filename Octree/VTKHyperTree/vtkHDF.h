 ...
 *
 * This is intended to be "subclassed" for various vtkHDF files; very little is neccesarily in common between different
 * grid types in the VTKHDF schemas, other than the requirement that we put our stuff into a VTKHDF group, which we
 * create here.
 * 
 */
typedef struct {
...
/**
 * @brief Function called on any error which immediately closes the file to avoid corruptions
 * 
 * @memberof vtkHDF
 */
...
vtkHDF vtk_HDF_init(const char *fname) {
    ...
    vtkHDF vtk_hdf = {.file_id = H5I_INVALID_HID, .fapl = H5I_INVALID_HID, .grp_vtkhdf_id = H5I_INVALID_HID};
...
#if MPI_SINGLE_FILE
    // If we use MPI_SINGLE_FILE, we set all procs to have access and write into the same file
    ...
    vtk_hdf.grp_vtkhdf_id = H5Gcreate2(vtk_hdf.file_id, "/VTKHDF", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    if (vtk_hdf.grp_vtkhdf_id < 0)
        vtk_HDF_error(&vtk_hdf);
#else
