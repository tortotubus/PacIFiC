#ifndef DLMFD_NavierStokes_HH
#define DLMFD_NavierStokes_HH
...
class LA_Vector ;
class LA_SeqVector ;
class DLMFD_NavierStokesSystem ;
class LA_SeqMatrix ;
...
struct DLMFD2NS
{
  ...
  DLMFD_AllRigidBodies* allrigidbodies_ ;
  double critical_distance_translation_ ;
};

/** @brief The Class DLMFD_NavierStokes.
...
class DLMFD_NavierStokes : public MAC_Object, public PAC_ComputingTime,
	public PAC_SolverComputingTime
{
   ...
   //-- Substeps of the step by step progression
      ...
      /** @brief Create and initialize an instance of DLMFD_NavierStokes
      @param a_owner the MAC-based object
      @param exp to read the data file
      @param fromDLMFD structure containing input data from the DLMFD framework */
      static DLMFD_NavierStokes* create(
                  MAC_Object* a_owner,
   ...
   //-- Constructors & Destructor
      ...
      //@{
      /** @brief Destructor */
      virtual ~DLMFD_NavierStokes( void ) ;

      /** @brief Copy constructor */
      DLMFD_NavierStokes( DLMFD_NavierStokes const& other ) ;

      /** @brief Operator ==
      @param other the right hand side */
      DLMFD_NavierStokes& operator=( DLMFD_NavierStokes const& other ) ;

      /** @brief Constructor with arguments
      @param a_owner the MAC-based object
      @param exp to read the data file */
      DLMFD_NavierStokes( MAC_Object* a_owner,
                       MAC_ModuleExplorer const* exp,
                       struct DLMFD2NS const& fromNS );

      /** @brief Constructor without argument */
      DLMFD_NavierStokes( void ) ;
   ...
   //-- Basic discrete system building
      ...
      void assemble_DLMFD_un_at_rhs (FV_TimeIterator const* t_it
                                , double const& gamma);
      ...
      void DLMFD_interface_unknown_solver( LA_SeqVector* rhs
                                      , size_t const& comp
                                      , size_t const& dir
                                      , size_t const& field
                                      , size_t const& r_index ) ;
      ...
      double compute_DLMFD_velocity_change( void );
      //@}


   //-- Direction splitting communicators

      /** @name Fictitious Domain communicators */
      /** @brief Create the sub-communicators */
      void create_DLMFD_subcommunicators ( void ) ;
      ...
      void free_DLMFD_subcommunicators ( void ) ;
   ...
   //-- Attributes
      ...
      DLMFD_NavierStokesSystem* GLOBAL_EQ ;
      DLMFD_PID* controller;
