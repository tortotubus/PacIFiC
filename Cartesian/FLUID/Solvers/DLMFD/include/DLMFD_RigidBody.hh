...
class DLMFD_RigidBody
{
  public: //-----------------------------------------------------------------
    ...
    DLMFD_RigidBody(FS_RigidBody *pgrb, const bool &are_particles_fixed,
    ...
    virtual void set_all_MAC(FV_DiscreteField *pField,
                             double critical_distance) = 0;

    /** @brief Set DLMFD boundary and interior points
    @param critical_distance Critical distance
    @param pField Constrained field */
    virtual void set_all_points(FV_DiscreteField *pField,
                                double critical_distance) = 0;

    /** @brief Set one DLMFD boundary point in the list
    @param point Geometric point to set
    @param bp Boundary point iterator */
    void
    set_boundary_point(const geomVector &point,
                       list<DLMFD_BoundaryMultiplierPoint *>::iterator &bp);

    /** @brief Set DLMFD halozone boundary point in the list
    @param point Geometric point to set
    @param bp Halozone boundary point iterator */
    void set_halozone_boundary_point(
        const geomVector &point,
        list<DLMFD_BoundaryMultiplierPoint *>::iterator &bp);

    /** @brief Set DLMFD boundary point in the list of boundary point or in the
    list of halozone boundary point
    @param point Geometric point to set
    @param primary_grid Mesh
    @param bp Boundary point iterator
    @param bphz Halozone boundary point iterator*/
    void setBndPoint(geomVector const &point, FV_Mesh const *primary_grid,
                     list<DLMFD_BoundaryMultiplierPoint *>::iterator &bp,
                     list<DLMFD_BoundaryMultiplierPoint *>::iterator &bphz);

    /** @brief Set boundary points located on a segment in the boundary_points
    list */
    ...
    virtual void
    erase_critical_interior_points_PerProc(double critical_distance);

    /** @brief Remove critical boundary points close from physical boundaries,
    including periodic conditions if needed
    @param critical_distance Critical distance
    @param GeoBoundaries Boundaries object */
    void erase_critical_boundary_points_ptb(
        const double &critical_distance,
        const DLMFD_AllGeomBoundaries *GeoBoundaries);

    /** @brief Remove critical boundary points close from physical boundaries
    @param critical_distance Critical distance
    @param lb List of boundaries */
    void erase_critical_boundary_points_ptb_oneGC(
    ...
    @param critical_distance Critical_distance */
    void erase_critical_boundary_points_ptp_PerProc(
        DLMFD_RigidBody *second_component, const double &critical_distance);

    /** @brief Remove critical boundary points too close from an other particle,
    @param second_component Other particle
    @param critical_distance Critical_distance */
    void erase_critical_boundary_points_ptp_PerProc_oneGC(
        DLMFD_RigidBody *second_component, const double &critical_distance);
    ...
    vectors with a default length that is predicted before filling
    @param nbIPdefp Default number of interior points
    @param nbIPdefp Default number of boundary points
    @param nbIPdefp Default number of halozone interior points
    @param nbIPdefp Default number of halozone boundary points */
    ...
    /** @brief Update the RB attributes */
    virtual void update();
    ...
    virtual bool proximityQuery(DLMFD_RigidBody const *second_component,
                                const double &distance) const;

    /** @brief Fill the list of boundary points and halozone boundary points
    that are inside the contact region, i.e. that are inside the sphere of
    center refPoint and of radius distance_contactRegion */
    void DLMFDPoints_in_ContactRegion(
    ...
    void print_partPointsCoordinates(
    ...
    /** @brief Compute the float abs(gravity_center(direction) - coordinate)
    @param coordinate Coordinate
    @param direction Direction */
    double compute_distance_to_bottom(const double &coordinate,
                                      const size_t &direction) const;

    /** @brief Compute the orientation vector */
    ...
    // -- Methods to use in the Uzawa solver for DLM/FD problem
    /** @name Methods to be use in the Uzawa solver for DLM/FD problem
    //@{
    ...
    */
    void
    correctQvectorsAndInitUzawa_Velocity(const double &rho_f,
    ...
    void calculateParticleVelocities(double const &rho_f,
                                     double const &timestep);

    /** @brief Set U = t_tran*alpha+U*beta and omega = t_rot*alpha+omega*beta */
    void updateSolutionVectors_Velocity(const double &alpha,
                                        const double &beta);
    ...
    /** @brief Append the momentum equations right hand side <w,v>_P
    in the global matrix system, where w is the Lagrange multipliers field and
    v the test function for the velocity field
    ...
    /** @brief Update lambda and r i.e. do: lambda -= alpha * w and r -= alpha *
     * x */
    ...
    <lambda,v>_P in the global matrix system, where lambda is the Lagrange
    multipliers field at previous time step and v the test function for the
    velocity field
    @param GLOBAL_EQ global velocity-pressure matrix system
    @param bulk true when both boundary and interior points are considered,
    ...
    @param pField Constrained field
    @param OnePointInfos Multiplier point infos
    @param SecondOrderBP Use a second order interpolation for boundary points */
    ...
    // -- DLMFD interpolation methods
    /** @name DLMFD interpolation methods */
    //@{

    /** @brief Compute 1-D interpolation at different orders
    @param x coordinate in 1-D transformed space
    @param i weighting point number
    @param order order of interpolation */
    double compute_weight_generic(double const &x, size_t const &i,
                                  size_t const &order);

    /** @brief Compute Q2 weighting
    @param i test function number
    @param x x ccordinate in transformed space
    @param y y coordinate in transformed space
    @param z z coordinate in transformed space */
    double Q2weighting(size_t const &i, double const &x, double const &y,
                       double const &z);
 ...
 points list is extended by in case of optimized memory handling */
    ...
    size_t ntotal_fieldunk; /**< total number of field unknowns for this solid
                               component on the proc */
    size_t ntotal_fieldunk_previous; /**< total number of allocated field
                                    unknowns at previous time for this solid
                                    component on the proc */

    list<DLMFD_BoundaryMultiplierPoint *>
        boundary_points; /* List of pointers to boundary points */
    size_t nBP;          /**< number of boundary points on proc */
    list<DLMFD_InteriorMultiplierPoint *>
        interior_points; /* List of pointers to interior points */
    size_t nIP;          /**< number of interior points on proc */
    list<DLMFD_BoundaryMultiplierPoint *>
        halozone_boundary_points; /**< list of boundary points in the halo zone
                                     of this process */
    size_t nBPHZ;                 /**< number of boundary points in halozone */
    list<DLMFD_InteriorMultiplierPoint *>
        halozone_interior_points; /**< list of interior points in the halo zone
                                     of this process */
    ...
    bool b_exactAllocation_done; /**< in case of particle treated as fixed
                                obstacle, exact allocation of point infos and
                                vectors must be done only once; as this boolean
                                is true, the method
    ...
    vector<vector<double>>
        inversedInertia_3D; /**< inverse of the inertia tensor */

    vector<geomVector>
        *periodic_directions; /**< periodic clones whose position is
                                 gravity_center + (*periodic_directions)[i] */
