...
DLMFD_GeneralPolyhedron::DLMFD_GeneralPolyhedron(
    FS_RigidBody *pgrb, const bool &are_particles_fixed,
    FV_DiscreteField *pField_, double const critical_distance_)
    : DLMFD_RigidBody(pgrb, are_particles_fixed, pField_), g2(NULL)
//---------------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_GeneralPolyhedron:: DLMFD_GeneralPolyhedron");
...
//---------------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_GeneralPolyhedron::set_ptr_FS_GeneralPolyhedron_"
              "Additional_Param");
...
//---------------------------------------------------------------------------
void DLMFD_GeneralPolyhedron::set_all_MAC(FV_DiscreteField *pField,
...
//---------------------------------------------------------------------------
{
    ...
    if (component_type == "O" || component_type == "PO")
    {
        if (g2 == NULL)
            g2 = new geomVector(3);
        (*g2)(0) =
            gravity_center(0) + 1e-4 * radius * random() / double(INT_MAX);
        (*g2)(1) =
            gravity_center(1) + 1e-4 * radius * random() / double(INT_MAX);
        (*g2)(2) =
            gravity_center(2) + 1e-4 * radius * random() / double(INT_MAX);
    }
    ...
    {
        size_t i, j, ncorners = corners.size(),
                     nper = periodic_directions->size();
        ...
        for (i = 0; i < nper; ++i)
        {
                    ...
                    radius))
            {
                ...
                if (interior_points.empty())
                    allocate_default_listOfPointsAndVectors_GeneralPolyhedron(
...
//---------------------------------------------------------------------------
void DLMFD_GeneralPolyhedron::set_all_points(FV_DiscreteField *pField,
                                             double critical_distance)
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void DLMFD_GeneralPolyhedron::set_boundary_points_list(FV_DiscreteField *pField,
                                                       double critical_distance)
//---------------------------------------------------------------------------
{
    ...
    double edge_length = 0.,
           spacing = DLMFD_FictitiousDomain::BoundaryPointsSpacing_coef *
                     critical_distance;
    ...
    for (faceIter = facesVec.begin(); faceIter != facesVec.end(); ++faceIter)
    {
        ...
        for (itrLocNodes = faceIter->begin() + 1;
             itrLocNodes != faceIter->end(); ++itrLocNodes)
        {
            if (*(itrLocNodes - 1) > *itrLocNodes)
                pairToAdd =
                    pair<size_t, size_t>(*itrLocNodes, *(itrLocNodes - 1));
            else
                pairToAdd =
                    pair<size_t, size_t>(*(itrLocNodes - 1), *itrLocNodes);

            if (facesBnd.insert(pairToAdd).second)
            {
                setPtsOnEdge(corners[*(itrLocNodes - 1)], corners[*itrLocNodes],
                             spacing, false, primary_grid, bp, bphz);

                double el =
                    corners[*(itrLocNodes - 1)].calcDist(corners[*itrLocNodes]);
                if (!edge_length)
                    edge_length = el;
        ...
        if (faceIter->front() > faceIter->back())
            pairToAdd =
                pair<size_t, size_t>(faceIter->back(), faceIter->front());
        else
            pairToAdd =
                pair<size_t, size_t>(faceIter->front(), faceIter->back());

        if (facesBnd.insert(pairToAdd).second)
        {
            setPtsOnEdge(corners[faceIter->front()], corners[faceIter->back()],
                         spacing, false, primary_grid, bp, bphz);

            double el =
                corners[faceIter->front()].calcDist(corners[faceIter->back()]);
    ...
    if (b_isometric && facesVec.size() == 4)
        setBndPts_IsometricTetrahedron(corners, spacing, primary_grid, bp,
                                       bphz);
...
//---------------------------------------------------------------------------
void DLMFD_GeneralPolyhedron::set_interior_points_list(FV_DiscreteField *pField,
                                                       double critical_distance)
//---------------------------------------------------------------------------
{
    ...
    for (size_t m = 0; m < dim; ++m)
        ...
        {
            coor_min(m) =
                (*cornIter)(m) < coor_min(m) ? (*cornIter)(m) : coor_min(m);
            coor_max(m) =
                (*cornIter)(m) > coor_max(m) ? (*cornIter)(m) : coor_max(m);
        }
...
//---------------------------------------------------------------------------
void DLMFD_GeneralPolyhedron::setBndPts_IsometricCube(
...
//---------------------------------------------------------------------------
{
    ...
    size_t nbptsLocal =
        int(corners_[faceIter->front()].calcDist(corners_[faceIter->back()]) /
            spacing);

    if (nbptsLocal > 1)
    {
        for (faceIter = facesVec.begin(); faceIter != facesVec.end();
             ++faceIter)
        {
            RefCorner = corners_[faceIter->front()];
            dir0 = (corners_[*(faceIter->begin() + 1)] - RefCorner) *
                   (1. / nbptsLocal);
            ...
            for (size_t i0 = 1; i0 <= nbptsLocal - 1; ++i0)
                ...
                {
                    newPoint =
                        RefCorner + dir0 * double(i0) + dir1 * double(i1);
...
//---------------------------------------------------------------------------
void DLMFD_GeneralPolyhedron::setBndPts_IsometricTetrahedron(
    vector<geomVector> const &corners_, double spacing,
    FV_Mesh const *primary_grid,
    list<DLMFD_BoundaryMultiplierPoint *>::iterator &bp,
    list<DLMFD_BoundaryMultiplierPoint *>::iterator &bphz)
...
DLMFD_GeneralPolyhedron::get_ptr_FS_GeneralPolyhedron_Additional_Param()
//---------------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_GeneralPolyhedron::get_ptr_FS_GeneralPolyhedron_"
              "Additional_Param");

    return (dynamic_cast<FS_GeneralPolyhedron *>(ptr_FSrigidbody)
...
//---------------------------------------------------------------------------
void DLMFD_GeneralPolyhedron::
    allocate_default_listOfPointsAndVectors_GeneralPolyhedron(
        const double &critical_distance, FV_DiscreteField *pField)
//---------------------------------------------------------------------------
{
    MAC_LABEL("DS_GeneralPolyhedron::allocate_default_listOfPointsAndVectors_"
              "GeneralPolyhedron()");
    ...
    else
    {
        ...
        double spacing = DLMFD_FictitiousDomain::BoundaryPointsSpacing_coef *
                         critical_distance;
...
//---------------------------------------------------------------------------
void DLMFD_GeneralPolyhedron::erase_critical_interior_points_PerProc(
    double critical_distance)
//---------------------------------------------------------------------------
{
        ...
        for (size_t j = 0; j < nIP; ++j, ivi++)
            if ((*ivi)->isValid())
            {
                ...
                for (size_t m = 0; m < nfaces && !test; ++m)
                    if (fabs((face_normals[m], (*ivi)->get_coordinates() -
                                                   corners[facesVec[m][0]])) <
                        critical_distance)
                        test = true;
                ...
                if (periodic_directions && !test)
                {
                    ...
                    for (size_t i = 0; i < nper && !test; ++i)
                        for (size_t m = 0; m < nfaces && !test; ++m)
                            if (fabs((face_normals[m],
                                      (*ivi)->get_coordinates() -
                                          corners[facesVec[m][0]] -
                                          (*periodic_directions)[i])) <
                                critical_distance)
                                test = true;
                }
                ...
                if (test)
                {
                    ...
                    if (!boundary_points.empty())
                    {
                        ...
                        for (size_t i = 0; i < nBP && (*ivi)->isValid();
                             ++i, imp++)
                            if ((*imp)->isValid())
                    ...
                    if (!halozone_boundary_points.empty() && (*ivi)->isValid())
                    {
                        ...
                        for (size_t i = 0; i < nBPHZ && (*ivi)->isValid();
                             ++i, ihp++)
                            if ((*ihp)->isValid())
...
//---------------------------------------------------------------------------
double DLMFD_GeneralPolyhedron::calcPointDeterm4by4(
    const geomVector &pointOne, const geomVector &pointTwo,
    const geomVector &pointThree, const geomVector &pointFour) const
//---------------------------------------------------------------------------
{
    ...
    double retVal =
        pointOne(0) *
            (y2 * z3 + y3 * z4 + y4 * z2 - y4 * z3 - y3 * z2 - y2 * z4) -
        pointOne(1) *
            (x2 * z3 + x3 * z4 + x4 * z2 - x4 * z3 - x3 * z2 - x2 * z4) +
        pointOne(2) *
            (x2 * y3 + x3 * y4 + x4 * y2 - x4 * y3 - x3 * y2 - x2 * y4) -
        (x2 * y3 * z4 + x3 * y4 * z2 + x4 * y2 * z3 - x4 * y3 * z2 -
         x3 * y2 * z4 - x2 * y4 * z3);
...
//---------------------------------------------------------------------------
bool DLMFD_GeneralPolyhedron::checkPointInTetrahedron(
    const geomVector &pointOne, const geomVector &pointTwo,
    const geomVector &pointThree, const geomVector &pointFour,
...
//---------------------------------------------------------------------------
{
    ...
    double detTot =
        calcPointDeterm4by4(pointOne, pointTwo, pointThree, pointFour);

    double detOne =
        calcPointDeterm4by4(pointToCheck, pointTwo, pointThree, pointFour);

    double detTwo =
        calcPointDeterm4by4(pointOne, pointToCheck, pointThree, pointFour);

    double detThree =
        calcPointDeterm4by4(pointOne, pointTwo, pointToCheck, pointFour);

    double detFour =
        calcPointDeterm4by4(pointOne, pointTwo, pointThree, pointToCheck);
    ...
    if ((detTot * detOne >= -0.) && (detTot * detTwo >= -0.) &&
        (detTot * detThree >= -0.) && (detTot * detFour >= -0.))
        in = true;
...
//---------------------------------------------------------------------------
{
    ...
    if (corners.size() == 4 && facesVec.size() == 4)
        inSolid = checkPointInTetrahedron(corners[0], corners[1], corners[2],
                                          corners[3], point);
             ...
             faceIter != facesVec.end() && !inSolid; ++faceIter)
        {
            ...
            if (noPntsFace == 3) // face is triangle
            {
                if (checkPointInTetrahedron(
                        corners[(*faceIter)[0]], corners[(*faceIter)[1]],
                        corners[(*faceIter)[2]],
                        is_an_obstacle ? *g2 : gravity_center, point))
                    inSolid = true;
            }
            else
            {
                for (size_t idxPts = 2; idxPts < noPntsFace && !inSolid;
                     ++idxPts)
                    if (checkPointInTetrahedron(
                            corners[(*faceIter)[0]],
