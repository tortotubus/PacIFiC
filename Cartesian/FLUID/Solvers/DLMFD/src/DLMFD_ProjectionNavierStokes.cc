...
DLMFD_ProjectionNavierStokes::DLMFD_ProjectionNavierStokes(void)
    //--------------------------------------------------------------------------
    : FV_OneStepIteration("DLMFD_ProjectionNavierStokes"), PAC_ComputingTime("Solver")
{
      ...
      b_pressure_rescaling(false)
{
  ...
  if (AdvectionScheme == "TVD" && UU->primary_grid()->get_security_bandwidth() < 2)
  {
  ...
  {
    MAC::out() << endl
               << "*** Navier & Stokes algorithm" << endl
               << endl;
    MAC::out() << "   Name = Projection" << endl;
    MAC::out() << "   Viscous term time accuracy = " << ViscousTimeAccuracy << endl;
    MAC::out() << "   Advection term time accuracy = " << AdvectionTimeAccuracy << endl;
    MAC::out() << "   Imposed CFL = " << imposed_CFL << endl;
    MAC::out() << "   Explicit Pressure Gradient = " << (b_ExplicitPressureGradient ? "true" : "false") << endl;
...
//---------------------------------------------------------------------------
{
  ...
  double velocity_time_change = GLOBAL_EQ->compute_velocity_change() / t_it->time_step();
  if (my_rank == is_master)
    MAC::out() << "         Velocity change = " << MAC::doubleToString(ios::scientific, 14, velocity_time_change)
               << endl;
...
//---------------------------------------------------------------------------
void DLMFD_ProjectionNavierStokes::do_additional_savings(FV_TimeIterator const *t_it,
...
//---------------------------------------------------------------------------
void DLMFD_ProjectionNavierStokes::NavierStokes_AdvectionDiffusion_PredictionStep(
    FV_TimeIterator const *t_it)
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void DLMFD_ProjectionNavierStokes::assemble_pressure_DirichletBC_in_momentumEquation(
    LA_Vector *VEC_rhs)
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void DLMFD_ProjectionNavierStokes::assemble_unitary_periodic_pressure_gradient_rhs(
    LA_Vector *VEC_rhs)
//---------------------------------------------------------------------------
{
  MAC_LABEL("DLMFD_ProjectionNavierStokes:: "
...
//---------------------------------------------------------------------------
{
  ...
  for (i = min_unknown_index(0); i <= max_unknown_index(0); ++i)
  {
    ...
    for (j = min_unknown_index(1); j <= max_unknown_index(1); ++j)
    {
      ...
      if (dim == 2)
      {
        ...
        dux = UU->DOF_value(shift.i + i, j, k, 0, 0) - UU->DOF_value(shift.i + i - 1, j, k, 0, 0);

        // Divergence of u (y component)
        duy = UU->DOF_value(i, shift.j + j, k, 1, 0) - UU->DOF_value(i, shift.j + j - 1, k, 1, 0);
      ...
      else
      {
        for (k = min_unknown_index(2); k <= max_unknown_index(2); ++k)
        {
          ...
          duz = UU->DOF_value(i, j, shift.k + k, 2, 0) - UU->DOF_value(i, j, shift.k + k - 1, 2, 0);
  ...
  FV_Mesh const *primary_mesh = UU->primary_grid();
  double domain_measure = dim == 2 ? primary_mesh->get_main_domain_boundary_perp_to_direction_measure(0) * primary_mesh->get_main_domain_boundary_perp_to_direction_measure(1) : primary_mesh->get_main_domain_boundary_perp_to_direction_measure(0) * (primary_mesh->get_main_domain_max_coordinate(2) - primary_mesh->get_main_domain_min_coordinate(2));
