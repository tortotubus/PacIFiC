...
DLMFD_DirectionSplitting const *DLMFD_DirectionSplitting::PROTOTYPE =
    new DLMFD_DirectionSplitting();
...
DLMFD_DirectionSplitting::DLMFD_DirectionSplitting(
    ...
    //---------------------------------------------------------------------------
    : FV_OneStepIteration(a_owner, dom, exp), ComputingTime("Solver"),
      UF(dom->discrete_field("velocity")), PF(dom->discrete_field("pressure")),
      GLOBAL_EQ(0), peclet(1.), mu(1.), kai(1.), AdvectionScheme("TVD"),
      resultsDirectory("Res"), AdvectionTimeAccuracy(1), rho(1.),
      ...
      translation_direction(0), bottom_coordinate(0.), translated_distance(0.)
{
    ...
    MAC_ModuleExplorer *se =
        exp->create_subexplorer(0, "DLMFD_DirectionSplittingSystem");
    ...
    if (is_solids)
        dlmfd_solver =
            DLMFD_FictitiousDomain::create(a_owner, dom, exp, transfert);
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::do_one_inner_iteration(
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::do_before_time_stepping(
...
//---------------------------------------------------------------------------
{
    ...
    if (is_solids)
        if (b_projection_translation)
        {
            ...
            geomVector vt(dim);
            vt(translation_direction) =
                primary_grid->get_translation_distance();
            dlmfd_solver->setParaviewPostProcessingTranslationVector(
                -vt(0), -vt(1), -vt(2));
...
//---------------------------------------------------------------------------
{
    ...
    start_total_timer(
        "DLMFD_DirectionSplitting:: do_before_inner_iterations_stage");
...
//---------------------------------------------------------------------------
{
    ...
    start_total_timer(
        "DLMFD_DirectionSplitting:: do_after_inner_iterations_stage");
    ...
    double velocity_time_change =
        GLOBAL_EQ->compute_DS_velocity_change() / t_it->time_step();
    ...
    if (is_solids)
        ...
        if (b_projection_translation)
        {
            double distance_to_bottom =
                dlmfd_solver->Compute_distance_to_bottom(bottom_coordinate,
                                                         translation_direction);
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::do_additional_savings(
...
//---------------------------------------------------------------------------
{
    ...
    if (is_solids)
        ...
        dlmfd_solver->do_additional_savings(
            cycleNumber, t_it, translated_distance, translation_direction);
...
//---------------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_DirectionSplitting:: error_with_analytical_solution");
    // Parameters
    double x, y, z;
    size_t cpp;
    ...
    height = (UF->primary_grid()->get_main_domain_max_coordinate(1) -
    ...
    for (size_t comp = 0; comp < nb_comps[1]; ++comp)
    {
            ...
            for (size_t j = 0; j < local_dof_number(1); ++j)
            {
                ...
                if (dim == 2)
                {
                    ...
                    if (UF->DOF_is_unknown_handled_by_proc(i, j, k, comp))
                        error_L2 +=
                            MAC::sqr(computed_field - analytical_solution) *
                ...
                else
                {
                    for (size_t k = 0; k < local_dof_number(2); ++k)
                    {
                        ...
                        if (comp == cpp)
                        {
                            analytical_solution = (bodyterm / (2. * mu)) *
                                                  (y * y - height * height);
                        }
                        if (UF->DOF_is_unknown_handled_by_proc(i, j, k, comp))
                            error_L2 +=
        ...
        if (my_rank == 0)
            cout << "L2 Error with analytical solution field " << UF->name()
                 << ", component " << comp << " = " << std::fixed
                 << std::setprecision(16) << error_L2 << endl;
    }
}

//---------------------------------------------------------------------------
size_t DLMFD_DirectionSplitting::return_row_index(FV_DiscreteField const *FF,
                                                  size_t const &comp,
                                                  size_t const &dir,
                                                  size_t const &j,
                                                  size_t const &k)
...
//---------------------------------------------------------------------------
double DLMFD_DirectionSplitting::assemble_field_matrix(
    FV_DiscreteField const *FF, FV_TimeIterator const *t_it,
    double const &gamma, size_t const &comp, size_t const &dir,
    size_t const &field, size_t const &j, size_t const &k,
...
//---------------------------------------------------------------------------
{
    ...
    for (m = 0, i = min_unknown_index(dir); i <= max_unknown_index(dir);
         ++i, ++m)
    {
        ...
        bool r_bound = false;
        bool l_bound = false;
        // All the proc will have open right bound, except last proc for non
        // periodic systems
        if ((is_periodic[field][dir] != 1) &&
            (rank_in_i[dir] == nb_ranks_comm_i[dir] - 1))
            r_bound = true;
        // All the proc will have open left bound, except first proc for non
        // periodic systems
        if ((is_periodic[field][dir] != 1) && (rank_in_i[dir] == 0))
            l_bound = true;

        // Since, this function is used in all directions;
        // ii, jj, and kk are used to convert the passed arguments corresponding
        ...
        if (i == min_unknown_index(dir) && l_bound)
        {
            ...
            if (FF->DOF_in_domain(ii, jj, kk, comp) &&
                FF->DOF_has_imposed_Dirichlet_value(ii, jj, kk, comp))
            {
                value = center;
            }
            else
        ...
        if ((!l_bound) && (i == min_unknown_index(dir)))
        {
            ...
            if (rank_in_i[dir] == 0)
            {
                A[dir].ie[comp][r_index]->set_item(m, nb_ranks_comm_i[dir] - 1,
                                                   left);
                A[dir].ei[comp][r_index]->set_item(nb_ranks_comm_i[dir] - 1, m,
                                                   right);
            }
            else
            {
                A[dir].ie[comp][r_index]->set_item(m, rank_in_i[dir] - 1, left);
                A[dir].ei[comp][r_index]->set_item(rank_in_i[dir] - 1, m,
                                                   right);
            }
        }
        ...
        if ((!r_bound) && (i == max_unknown_index(dir)))
        {
            ...
            // For last index, Aee comes from this proc as it is interface
            // unknown wrt this proc
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::assemble_field_schur_matrix(
    struct TDMatrix *A, size_t const &comp, size_t const &dir,
    double const &Aee_diagcoef, size_t const &field, size_t const &r_index)
//---------------------------------------------------------------------------
{
    ...
    if (nb_ranks_comm_i[dir] > 1)
    {
        ...
        if (rank_in_i[dir] == 0)
        {
            ...
            for (size_t i = 1; i < nb_ranks_comm_i[dir]; ++i)
            {
                ...
                for (int k = 0; k < nbrows; k++)
                {
                    ...
                    {
                        // Assemble the global product matrix by adding
                        // contributions from all the procs
                        receive_matrix->add_to_item(
                            k, j, received_data[k * (nbrows) + j]);
                    }
                }
                ...
                if (is_periodic[field][dir] == 0)
                {
                    if (i < nb_ranks_comm_i[dir] - 1)
                    {
                        ...
                        // No periodic condition in x. So no fe contribution
                        // from last proc
                        A[dir].ee[comp][r_index]->set_item(
                            i, i, received_data[nb_received_data - 1]);
                    }
                }
                else
                {
                    // Assemble the global Aee matrix
                    // Periodic condition in x. So there is fe contribution from
                    // last proc
                    A[dir].ee[comp][r_index]->set_item(
                        i, i, received_data[nb_received_data - 1]);
                }
        ...
        else
        {
            ...
            // Fill the last element of packed data with the diagonal
            // coefficient Aee
            packed_data[nb_send_data - 1] = Aee_diagcoef;

            // Send the data
            MPI_Send(packed_data, nb_send_data, MPI_DOUBLE, 0, 0,
                     DDS_Comm_i[dir]);
        ...
        if (rank_in_i[dir] == 0)
        {
            ...
            for (int p = 0; p < nb_row; p++)
            {
                ...
                if (p < nb_row - 1)
                    Schur[dir].ii_super[comp][r_index]->set_item(
                        p, -receive_matrix->item(p, p + 1));
                if (p > 0)
                    Schur[dir].ii_sub[comp][r_index]->set_item(
                        p - 1, -receive_matrix->item(p, p - 1));
                // In case of periodic and multi-processor, there will be a
                // variant of Tridiagonal matrix instead of normal format
                if (is_periodic[field][dir] == 1)
                {
                    Schur[dir].ie[comp][r_index]->set_item(
                        p, 0, -receive_matrix->item(p, nb_row));
                    Schur[dir].ei[comp][r_index]->set_item(
                        0, p, -receive_matrix->item(nb_row, p));
                }
            }
            ...
            // of Tridiagonal matrix instead of normal format So, Schur
            // complement of Schur complement is calculated
            if (is_periodic[field][dir] == 1)
            {
                Schur[dir].ee[comp][r_index]->set_item(
                    0, 0,
                    A[dir].ee[comp][r_index]->item(nb_row, nb_row) -
                        receive_matrix->item(nb_row, nb_row));

                ProdMatrix *SchurP = GLOBAL_EQ->get_SchurP(field);
                GLOBAL_EQ->compute_product_matrix_interior(Schur, SchurP, comp,
                                                           0, dir, r_index);

                TDMatrix *DoubleSchur = GLOBAL_EQ->get_DoubleSchur(field);
                size_t nb_row =
                    DoubleSchur[dir].ii_main[comp][r_index]->nb_rows();
                DoubleSchur[dir].ii_main[comp][r_index]->set_item(
                    0, Schur[dir].ee[comp][r_index]->item(0, 0) -
    ...
    {
        // Condition for single processor in any direction with periodic
        // boundary conditions
        ProdMatrix *Ap = GLOBAL_EQ->get_Ap(field);
        GLOBAL_EQ->compute_product_matrix(A, Ap, comp, dir, field, r_index);
                     ...
                     j <= max_unknown_index(dir_j); ++j)
                {
                    for (size_t k = local_min_k; k <= local_max_k; ++k)
                    {
                        ...
                        if (field == 0)
                        {
                            ...
                            Aee_diagcoef = assemble_field_matrix(
                                PF, t_it, gamma, comp, dir, 0, j, k, r_index);
                        }
                        else if (field == 1)
                        {
                            r_index = return_row_index(UF, comp, dir, j, k);
                            Aee_diagcoef = assemble_field_matrix(
                                UF, t_it, gamma, comp, dir, 1, j, k, r_index);
                        }
                        TDMatrix *A = GLOBAL_EQ->get_A(field);
                        assemble_field_schur_matrix(A, comp, dir, Aee_diagcoef,
                                                    field, r_index);
                    }
                }
            }
...
//---------------------------------------------------------------------------
double DLMFD_DirectionSplitting::compute_un_component(
...
//---------------------------------------------------------------------------
{
    ...
    if (dir == 0)
    {
        ...
        if (UF->DOF_in_domain(i - 1, j, k, comp) &&
            UF->DOF_in_domain(i + 1, j, k, comp))
    ...
    else if (dir == 1)
    {
        ...
        if (UF->DOF_in_domain(i, j - 1, k, comp) &&
            UF->DOF_in_domain(i, j + 1, k, comp))
    ...
    else if (dir == 2)
    {
        ...
        if (UF->DOF_in_domain(i, j, k - 1, comp) &&
            UF->DOF_in_domain(i, j, k + 1, comp))
...
//---------------------------------------------------------------------------
double DLMFD_DirectionSplitting::velocity_local_rhs(
...
//---------------------------------------------------------------------------
{
    ...
    for (i = min_unknown_index(dir); i <= max_unknown_index(dir); ++i)
    {
        ...
        hl = UF->get_DOF_coordinate(i, comp, dir) -
             UF->get_DOF_coordinate(i - 1, comp, dir);
        hr = UF->get_DOF_coordinate(i + 1, comp, dir) -
             UF->get_DOF_coordinate(i, comp, dir);
        ...
        if (dir == 0)
        {
            temp_val = (UF->DOF_value(i, j, k, comp, 0) * dC * rho) /
                           (t_it->time_step()) -
                       gamma * value;
        }
        else if (dir == 1)
        {
            temp_val = (UF->DOF_value(j, i, k, comp, 3) * dC * rho) /
                           (t_it->time_step()) -
                       gamma * value;
        }
        else if (dir == 2)
        {
            temp_val = (UF->DOF_value(j, k, i, comp, 4) * dC * rho) /
                           (t_it->time_step()) -
                       gamma * value;
        }
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::unpack_compute_ue_pack(size_t const &comp,
                                                      size_t const &dir,
                                                      size_t const &p,
                                                      size_t const &field)
//---------------------------------------------------------------------------
{
    ...
    if (is_periodic[field][dir])
        VEC[dir].T[comp]->set_item(
            nb_ranks_comm_i[dir] - 1,
            first_pass[field][dir].send[comp][rank_in_i[dir]][3 * p]);
    VEC[dir].T[comp]->set_item(
        0, first_pass[field][dir].send[comp][rank_in_i[dir]][3 * p + 1]);
    VEC[dir].interface_T[comp]->set_item(
        0, first_pass[field][dir].send[comp][rank_in_i[dir]][3 * p + 2]);
    ...
    for (size_t i = 1; i < nb_ranks_comm_i[dir]; i++)
    {
        if (i != nb_ranks_comm_i[dir] - 1)
        {
            ...
            VEC[dir].interface_T[comp]->set_item(
                i, first_pass[field][dir]
                       .receive[comp][i]
                               [3 * p + 2]); // Assemble the interface rhs fe
        }
        else
        {
            if (is_periodic[field][dir] == 0)
    ...
    for (size_t i = 0; i < nb_interface_unknowns; i++)
    {
        VEC[dir].interface_T[comp]->set_item(
            ...
            VEC[dir].interface_T[comp]->item(i) -
                VEC[dir].T[comp]->item(i)); // Get fe - Aei*xi to solve for ue
    }
...
//----------------------------------------------------------------------
void DLMFD_DirectionSplitting::DS_interface_unknown_solver(
...
//----------------------------------------------------------------------
{
    ...
    // Condition for variant of Tridiagonal Schur complement in Perioidic
    // direction with multi-processor
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::unpack_ue(size_t const &comp,
                                         double *received_data,
                                         size_t const &dir, int const &p,
                                         size_t const &field)
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::solve_interface_unknowns(
...
//---------------------------------------------------------------------------
{
    ...
    // first_pass[field][dir_i].send[comp][rank_in_i[dir_i]],
    // first_pass[field][dir_i].size[comp],
    ...
    if (rank_in_i[dir] == 0)
    {
        if (nb_ranks_comm_i[dir] != 1)
        {
            for (i = 1; i < nb_ranks_comm_i[dir]; ++i)
            {
                ...
                MPI_Recv(first_pass[field][dir].receive[comp][i],
                         first_pass[field][dir].size[comp], MPI_DOUBLE, i, 0,
                         DDS_Comm_i[dir], &status);
            }
        }
        ...
        if (dim == 2)
        {
            for (j = local_min_j; j <= local_max_j; j++)
            {
                ...
                // Need to have the original rhs function assembled for
                // corrosponding j,k pair
                double fe =
                ...
                GLOBAL_EQ->DS_NavierStokes_solver(
                    FF, j, k, min_unknown_index(dir), comp, dir, field, p);
            }
        }
        else
        {
    ...
    else
    {
        ...
        MPI_Send(first_pass[field][dir].send[comp][rank_in_i[dir]],
                 first_pass[field][dir].size[comp], MPI_DOUBLE, 0, 0,
                 DDS_Comm_i[dir]);
    }
        ...
        if (rank_in_i[dir] == 0)
        {
            for (i = 1; i < nb_ranks_comm_i[dir]; ++i)
            {
                MPI_Send(second_pass[field][dir].send[comp][i],
                         second_pass[field][dir].size[comp], MPI_DOUBLE, i, 0,
                         DDS_Comm_i[dir]);
            }
        }
        else
        {
            ...
            if (dim == 2)
            {
                for (j = local_min_j; j <= local_max_j; j++)
                {
                    ...
                    unpack_ue(
                        comp,
                        second_pass[field][dir].send[comp][rank_in_i[dir]], dir,
                        p, field);
            ...
            else
            {
                for (k = local_min_k; k <= local_max_k; k++)
                {
                    ...
                    {
                        p = (j - local_min_j) +
                            local_length_j * (k - local_min_k);

                        unpack_ue(
                            comp,
                            second_pass[field][dir].send[comp][rank_in_i[dir]],
                            dir, p, field);
                        ...
                        // Solve ui and transfer solution into distributed
                        // vector
                        GLOBAL_EQ->DS_NavierStokes_solver(
                            FF, j, k, min_unknown_index(dir), comp, dir, field,
                            p);
                    }
                }
            }
...
//---------------------------------------------------------------------------
double DLMFD_DirectionSplitting::compute_p_component(size_t const &comp,
                                                     size_t const &i,
                                                     size_t const &j,
                                                     size_t const &k)
//---------------------------------------------------------------------------
{
    ...
    if (dim == 2)
    {
        if (comp == 0)
        {
            value = (PF->DOF_value(shift.i + i, j, 0, 0, 1) -
                     PF->DOF_value(shift.i + i - 1, j, 0, 0, 1)) *
                    dyC;
        }
        else
        {

            value = (PF->DOF_value(i, shift.j + j, 0, 0, 1) -
                     PF->DOF_value(i, shift.j + j - 1, 0, 0, 1)) *
                    dxC;
        }
    }
    else if (dim == 3)
...
//---------------------------------------------------------------------------
double DLMFD_DirectionSplitting::compute_adv_component(size_t const &comp,
                                                       size_t const &i,
                                                       size_t const &j,
                                                       size_t const &k)
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
{
    ...
    double pvalue = 0., xvalue = 0., yvalue = 0., zvalue = 0., rhs = 0.,
           bodyterm = 0., adv_value = 0.;
    int cpp = -1;
        ...
        for (i = min_unknown_index(0); i <= max_unknown_index(0); ++i)
        {
            ...
            for (j = min_unknown_index(1); j <= max_unknown_index(1); ++j)
            {
                ...
                if (dim == 2)
                {
                    ...
                    rhs = gamma * (xvalue * dyC + yvalue * dxC) - pvalue -
                          adv_value +
                          (UF->DOF_value(i, j, k, comp, 1) * dxC * dyC * rho) /
                              (t_it->time_step());
                ...
                else
                {
                         ...
                         ++k)
                    {
                        ...
                        rhs = gamma * (xvalue * dyC * dzC + yvalue * dxC * dzC +
                                       zvalue * dxC * dyC) -
                              pvalue - adv_value +
                              (UF->DOF_value(i, j, k, comp, 1) * dxC * dyC *
                               dzC * rho) /
                                  (t_it->time_step());
                        ...
                        //     global_numbering_index = UF->DOF_global_number(i,
                        //     j, k, comp); dlmfd_explicit_value =
                        //     GLOBAL_EQ->get_explicit_DLMFD_at_index(global_numbering_index);
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::Solve_i_in_jk(
                 ...
                 j <= max_unknown_index(dir_j); ++j)
            {
                for (size_t k = local_min_k; k <= local_max_k; ++k)
                {
                    ...
                    double fe = assemble_local_rhs(j, k, gamma, t_it, comp,
                                                   dir_i, field);
                    GLOBAL_EQ->DS_NavierStokes_solver(
                        FF, j, k, min_unknown_index(dir_i), comp, dir_i, field,
                        r_index);
                }
            }
        }
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::update_velocity_DLMFD_explicit(
    FV_TimeIterator const *t_it)
//---------------------------------------------------------------------------
{
            ...
            for (j = min_unknown_index(1); j <= max_unknown_index(1); ++j)
            {
                ...
                else
                {
                         ...
                         ++k)
                    {
                        ...
                        rhs = (UF->DOF_value(i, j, k, comp, 0) * dxC * dyC *
                               dzC * rho) /
                              (t_it->time_step());

                        global_numbering_index =
                            UF->DOF_global_number(i, j, k, comp);
                        dlmfd_explicit_value =
                            GLOBAL_EQ->get_explicit_DLMFD_at_index(
                                global_numbering_index);
                        rhs += dlmfd_explicit_value * dxC * dyC * dzC;
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::data_packing(FV_DiscreteField const *FF,
...
//---------------------------------------------------------------------------
{
    ...
    if (dir == 0)
    {
        ...
        {
            vec_pos = (j - min_unknown_index(1)) +
                      (max_unknown_index(1) - min_unknown_index(1) + 1) *
                          (k - min_unknown_index(2));
        }
    }
    else if (dir == 1)
    {
        ...
        {
            vec_pos = (j - min_unknown_index(0)) +
                      (max_unknown_index(0) - min_unknown_index(0) + 1) *
                          (k - min_unknown_index(2));
        }
    }
    else if (dir == 2)
    ...
    else if (rank_in_i[dir] == nb_ranks_comm_i[dir] - 1)
    {
        ...
        // If it is, we need to pack two elements apart from fe
        if (is_periodic[field][dir])
            packed_data[3 * vec_pos + 1] =
                VEC[dir].T[comp]->item(rank_in_i[dir]);
        else
            packed_data[3 * vec_pos + 1] = 0;

        packed_data[3 * vec_pos + 0] =
            VEC[dir].T[comp]->item(rank_in_i[dir] - 1);
    }
    else
    {
        packed_data[3 * vec_pos + 0] =
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::compute_Aei_ui(struct TDMatrix *arr,
...
//---------------------------------------------------------------------------
double DLMFD_DirectionSplitting::assemble_local_rhs(
    size_t const &j, size_t const &k, double const &gamma,
    FV_TimeIterator const *t_it, size_t const &comp, size_t const &dir,
    size_t const &field)
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
double DLMFD_DirectionSplitting::pressure_local_rhs(size_t const &j,
                                                    size_t const &k,
                                                    FV_TimeIterator const *t_it,
                                                    size_t const &dir)
//---------------------------------------------------------------------------
{
    ...
    for (i = min_unknown_index(dir); i <= max_unknown_index(dir); ++i)
    {
        ...
        {
            // Dxx for un
            xhr = UF->get_DOF_coordinate(shift.i + i, 0, 0) -
                  UF->get_DOF_coordinate(shift.i + i - 1, 0, 0);
            xright = UF->DOF_value(shift.i + i, j, k, 0, 0) -
                     UF->DOF_value(shift.i + i - 1, j, k, 0, 0);
            xvalue = xright / xhr;

            // Dyy for un
            yhr = UF->get_DOF_coordinate(shift.j + j, 1, 1) -
                  UF->get_DOF_coordinate(shift.j + j - 1, 1, 1);
            yright = UF->DOF_value(i, shift.j + j, k, 1, 0) -
            ...
            {
                // Dzz for un
                zhr = UF->get_DOF_coordinate(shift.k + k, 2, 2) -
                      UF->get_DOF_coordinate(shift.k + k - 1, 2, 2);
                zright = UF->DOF_value(i, j, shift.k + k, 2, 0) -
            ...
            {
                value = -(rho * (xvalue + yvalue + zvalue) * dx) /
                        (t_it->time_step());
            }
        }
        else if (dir == 1)
...
//---------------------------------------------------------------------------
{
    ...
    // Get local min and max indices
    // When we are running in parallel, the unknowns in the overlapping region
    // are not solved. So we need to include them here by calling
    // get_min_index_unknown_on_proc() instead of
    // get_min_index_unknown_handled_by_proc().
    ...
    for (i = min_unknown_index(0); i <= max_unknown_index(0); ++i)
    {
        for (j = min_unknown_index(1); j <= max_unknown_index(1); ++j)
        {
            if (dim == 2)
            {
                ...
                xright = UF->DOF_value(shift.i + i, j, k, 0, 1) -
                         UF->DOF_value(shift.i + i - 1, j, k, 0, 1);
                xvalue2 = xright / xhr;
                ...
                yright = UF->DOF_value(i, shift.j + j, k, 1, 1) -
                         UF->DOF_value(i, shift.j + j - 1, k, 1, 1);
                yvalue2 = yright / yhr;
            ...
            else
            {
                for (k = min_unknown_index(2); k <= max_unknown_index(2); ++k)
                {
                    ...
                    zright = UF->DOF_value(i, j, shift.k + k, 2, 1) -
                             UF->DOF_value(i, j, shift.k + k - 1, 2, 1);
                    zvalue2 = zright / zhr;
                    ...
                    value = PF->DOF_value(i, j, k, 0, 0) +
                            PF->DOF_value(i, j, k, 0, 1) -
                            0.5 * kai * mu * (xvalue + yvalue + zvalue);
                    PF->set_DOF_value(i, j, k, 0, 0, value);
                }
            }
        }
...
//----------------------------------------------------------------------
{
    ...
    fileNameStream << "/home/arun95/NS_2D_lidCavity_results/"
                   << t_it->time_step()
                   << "output_pressure.csv"; // and pass "dice_" here
    string fileName = fileNameStream.str();

    MAC::out() << "The file name is " << fileName << endl;

    pressureFile.open(fileName.c_str(),
...
//----------------------------------------------------------------------
{
    ...
    fileNameStream << "/home/arun95/NS_2D_lidCavity_results/"
                   << t_it->time_step()
                   << "output_velocity.csv"; // and pass "dice_" here
    string fileName = fileNameStream.str();

    velocityFile.open(fileName.c_str(),
                      std::ios_base::out | std::ios_base::trunc);
    size_t i, j, k;
...
//----------------------------------------------------------------------
double DLMFD_DirectionSplitting::get_velocity_divergence(void)
//----------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::processor_splitting(int const &color,
                                                   int const &key,
                                                   size_t const &dir)
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void DLMFD_DirectionSplitting::allocate_mpi_variables(
    FV_DiscreteField const *FF, size_t const &field)
...
//----------------------------------------------------------------------
double DLMFD_DirectionSplitting::assemble_advection_Upwind(
...
//----------------------------------------------------------------------
double DLMFD_DirectionSplitting::assemble_advection_TVD(
    size_t const &advecting_level, double const &coef,
    size_t const &advected_level, size_t const &i, size_t const &j,
    size_t const &k, size_t const &component) const
//----------------------------------------------------------------------
