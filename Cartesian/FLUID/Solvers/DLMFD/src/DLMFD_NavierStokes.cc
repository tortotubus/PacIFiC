...
#include <DLMFD_NavierStokesSystem.hh>
...
//---------------------------------------------------------------------------
DLMFD_NavierStokes*
DLMFD_NavierStokes:: create( MAC_Object* a_owner,
		                    MAC_ModuleExplorer const* exp,
                          struct DMLFD2NS const& transfer )
//---------------------------------------------------------------------------
{
 MAC_LABEL( "DLMFD_NavierStokes:: create" ) ;
 MAC_CHECK_PRE( exp != 0 ) ;

 DLMFD_NavierStokes* result =
                      new DLMFD_NavierStokes( a_owner, exp, transfer ) ;
...
//---------------------------------------------------------------------------
DLMFD_NavierStokes:: DLMFD_NavierStokes( MAC_Object* a_owner,
                              	  MAC_ModuleExplorer const* exp,
                                   struct DLMFD2NS const& fromDLMFD )
//---------------------------------------------------------------------------
   : MAC_Object( a_owner )
   , PAC_ComputingTime("Solver")
   , UF ( fromDLMFD.dom_->discrete_field( "velocity" ) )
   , PF ( fromDLMFD.dom_->discrete_field( "pressure" ) )
   , GLOBAL_EQ( 0 )
   , mu( fromDLMFD.mu_ )
   , kai( fromDLMFD.kai_ )
   , AdvectionScheme( fromDLMFD.AdvectionScheme_ )
   , StencilCorrection( fromDLMFD.StencilCorrection_ )
   , is_CConlyDivergence ( fromDLMFD.is_CConlyDivergence_ )
   , FluxRedistThres( fromDLMFD.FluxRedistThres_ )
   , AdvectionTimeAccuracy( fromDLMFD.AdvectionTimeAccuracy_ )
   , rho( fromDLMFD.rho_ )
   , b_restart ( fromDLMFD.b_restart_ )
   , is_solids( fromDLMFD.is_solids_ )
   , is_stressCal ( fromDLMFD.is_stressCal_ )
   , ViscousStressOrder ( fromDLMFD.ViscousStressOrder_ )
   , PressureStressOrder ( fromDLMFD.PressureStressOrder_ )
   , stressCalFreq ( fromDLMFD.stressCalFreq_ )
   , is_par_motion ( fromDLMFD.is_par_motion_ )
   , allrigidbodies ( fromDLMFD.allrigidbodies_ )
   , b_projection_translation( fromDLMFD.dom_->primary_grid()
   ...
   , turn (false)
{
   MAC_LABEL( "DLMFD_NavierStokes:: DLMFD_NavierStokes" ) ;
   ...
   MAC_ModuleExplorer* se = exp->create_subexplorer( 0,"DLMFD_NavierStokesSystem" ) ;
   GLOBAL_EQ = DLMFD_NavierStokesSystem::create( this, se, UF, PF, inputData ) ;
   se->destroy() ;

	if (UF->primary_grid()->is_periodic_flow_rate()) {
		// controller = DLMFD_PID::create(0.5,1.,0);
		...
		if (!b_restart) {
			...
			if (macCOMM->rank() == 0) {
				string fileName = "./DLMFD_results/flow_pressure_history.csv" ;
				std::ofstream MyFile;
...
//---------------------------------------------------------------------------
DLMFD_NavierStokes:: ~DLMFD_NavierStokes( void )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: ~DLMFD_NavierStokes" ) ;

   free_DLMFD_subcommunicators() ;

}

//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: do_one_inner_iteration( FV_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: do_one_inner_iteration" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: do_before_time_stepping( FV_TimeIterator const* t_it,
      	std::string const& basename )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: do_before_time_stepping" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: do_after_time_stepping( void )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: do_after_time_stepping" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: do_before_inner_iterations_stage(
	FV_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: do_before_inner_iterations_stage" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: do_after_inner_iterations_stage(
	FV_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: do_after_inner_iterations_stage" ) ;
...
/*
   ...
   if ( my_rank == is_master ) {
      string fileName = "./DLMFD_results/max_divergence.csv" ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: do_additional_savings( FV_TimeIterator const* t_it,
      	int const& cycleNumber )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: do_additional_savings" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: ugradu_initialization ( )
//---------------------------------------------------------------------------
{
  MAC_LABEL( "DLMFD_NavierStokes:: ugradu_initialization" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: calculate_row_indexes ( FV_DiscreteField const* FF)
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: calculate_row_indexes" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: assemble_field_matrix ( FV_DiscreteField const* FF
                                        , FV_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: assemble_field_matrix" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: assemble_field_schur_matrix ( FV_DiscreteField const* FF )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: assemble_field_schur_matrix" ) ;
				...
				if (rank_in_i[dir] != 0 ) {
			      ...
			      MPI_Send( data_for_S[field][dir].send[comp][0],
			          (int) data_for_S[field][dir].size[comp],
			          	MPI_DOUBLE, 0, 0, DLMFD_Comm_i[dir] ) ;
				}
				...
				if (rank_in_i[dir] == 0 ) {
					for (size_t i = 1; i < (size_t)nb_ranks_comm_i[dir]; ++i) {
						...
						MPI_Recv( data_for_S[field][dir].receive[comp][0],
							(int) data_for_S[field][dir].size[comp],
							MPI_DOUBLE, (int) i, 0, DLMFD_Comm_i[dir], &status ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: assemble_1D_matrices ( FV_DiscreteField const* FF
                                        , FV_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: assemble_1D_matrices" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: NS_first_step ( FV_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
  MAC_LABEL( "DLMFD_NavierStokes:: NS_first_step" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: assemble_velocity_diffusion_terms ( )
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: assemble_velocity_diffusion_terms" ) ;
...
//---------------------------------------------------------------------------
double
DLMFD_NavierStokes:: compute_un_component ( size_t const& comp,
                                          size_t const& i,
                                          size_t const& j,
                                          size_t const& k,
                                          size_t const& dir,
...
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: compute_un_component" ) ;
...
DLMFD_NavierStokes:: compute_first_derivative ( size_t const& comp,
	                                          size_t const& i,
	                                          size_t const& j,
	                                          size_t const& k,
	                                          size_t const& dir,
...
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: compute_first_derivative" ) ;
...
//---------------------------------------------------------------------------
double
DLMFD_NavierStokes:: compute_un_component_FD ( size_t const& comp,
                                          size_t const& i,
                                          size_t const& j,
                                          size_t const& k,
                                          size_t const& dir,
...
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: compute_un_component_FD" ) ;
...
//---------------------------------------------------------------------------
double
DLMFD_NavierStokes:: compute_un_component_FV ( size_t const& comp,
                                       	  size_t const& i,
                                            size_t const& j,
                                            size_t const& k,
                                            size_t const& dir,
...
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: compute_un_component_FV" ) ;
...
//---------------------------------------------------------------------------
double
DLMFD_NavierStokes:: velocity_local_rhs ( size_t const& j
                                      , size_t const& k
                                      , double const& gamma
                                      , FV_TimeIterator const* t_it
                                      , size_t const& comp
...
//---------------------------------------------------------------------------
{
   ...
   MAC_LABEL("DLMFD_NavierStokes:: velocity_local_rhs" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: unpack_compute_ue_pack(size_t const& comp
                                        , size_t const& dir
                                        , size_t const& p
                                        , size_t const& field)
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: unpack_compute_ue_pack" ) ;
   ...
   DLMFD_interface_unknown_solver(VEC[dir].interface_T[comp],comp,dir,field,p);
...
//----------------------------------------------------------------------
void
DLMFD_NavierStokes::DLMFD_interface_unknown_solver( LA_SeqVector* interface_rhs
                                             , size_t const& comp
                                             , size_t const& dir
                                             , size_t const& field
...
//----------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokesSystem:: DLMFD_interface_unknown_solver" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: unpack_ue(size_t const& comp
...
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: unpack_ue" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: solve_interface_unknowns ( FV_DiscreteField* FF
                                            , double const& gamma
                                            , FV_TimeIterator const* t_it
                                            , size_t const& comp
                                            , size_t const& dir
...
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: solve_interface_unknowns" ) ;
   ...
   if ( rank_in_i[dir] == 0 ) {
      ...
      for (size_t j = local_min_j; j <= local_max_j; j++) {
         for (size_t k = local_min_k; k <= local_max_k; k++) {
            ...
            GLOBAL_EQ->DLMFD_NavierStokes_solver(FF,j,k,min_unknown_index(dir)
                                                            ,comp,dir,p,level);
         }
      }
      ...
      if ( rank_in_i[dir] == 0 ) {
         for (size_t i = 1; i < (size_t)nb_ranks_comm_i[dir]; ++i) {
            MPI_Send( second_pass[field][dir].send[comp][i],
                ...
                MPI_DOUBLE,(int) i, 0, DLMFD_Comm_i[dir] ) ;
         }
      } else {
         // Receive the data
         static MPI_Status status ;
         MPI_Recv( second_pass[field][dir].receive[comp][0],
             (int) second_pass[field][dir].size[comp],
             MPI_DOUBLE, 0, 0, DLMFD_Comm_i[dir], &status ) ;
...
//---------------------------------------------------------------------------
double
DLMFD_NavierStokes:: compute_p_component ( size_t const& comp
                                       , size_t const& i
                                       , size_t const& j
                                       , size_t const& k)
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: compute_p_component" ) ;
...
//---------------------------------------------------------------------------
double
DLMFD_NavierStokes:: compute_adv_component ( FV_TimeIterator const* t_it,
														 size_t const& comp,
                                           size_t const& i,
                                           size_t const& j,
                                           size_t const& k)
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: compute_adv_component" ) ;
...
//----------------------------------------------------------------------
double
DLMFD_NavierStokes:: divergence_of_U_noCorrection( size_t const& i
				                                  , size_t const& j
				                                  , size_t const& k
...
//----------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: divergence_of_U_noCorrection" );
...
//---------------------------------------------------------------------------
double
DLMFD_NavierStokes:: get_current_mean_flow_speed()
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokesSystem:: get_current_mean_flow_speed" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: predicted_pressure_drop (FV_TimeIterator const* t_it)
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: predicted_pressure_drop" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: assemble_DLMFD_un_at_rhs ( FV_TimeIterator const* t_it,
                                           double const& gamma)
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: assemble_DLMFD_un_at_rhs" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: Solve_i_in_jk ( FV_DiscreteField* FF
                                 , FV_TimeIterator const* t_it
                                 , size_t const& dir_i
                                 , size_t const& dir_j
                                 , size_t const& dir_k
...
//---------------------------------------------------------------------------
{
  MAC_LABEL("DLMFD_NavierStokes:: Solve_i_in_jk" ) ;
     ...
     if ((nb_ranks_comm_i[dir_i]>1)||(is_periodic[field][dir_i] == 1)) {
        ...
        for (size_t j=min_unknown_index(dir_j);j<=max_unknown_index(dir_j);++j){
           for (size_t k=local_min_k; k <= local_max_k; ++k) {
              ...
              GLOBAL_EQ->DLMFD_NavierStokes_solver(FF,j,k,min_unknown_index(dir_i)
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: data_packing ( FV_DiscreteField const* FF
                                , size_t const& p
                                , double const& fe
                                , size_t const& comp
                                , size_t const& dir )
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: data_packing" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: compute_Aei_ui (struct TDMatrix* arr
...
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: compute_Aei_ui" ) ;
...
//---------------------------------------------------------------------------
double
DLMFD_NavierStokes:: assemble_local_rhs ( size_t const& j
                                     , size_t const& k
                                     , double const& gamma
                                     , FV_TimeIterator const* t_it
                                     , size_t const& comp
...
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: assemble_local_rhs" ) ;
   double fe = 0.;
   if (field == 0) {
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: NS_velocity_update ( FV_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: NS_velocity_update" ) ;

   double gamma=mu;

   assemble_DLMFD_un_at_rhs(t_it,gamma);
   ...
   if (dim == 3) {
	...
	double velocity_time_change = compute_DLMFD_velocity_change()/t_it->time_step();
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes::initialize_grid_nodes_on_rigidbody( vector<size_t> const& list )
//---------------------------------------------------------------------------
{
  MAC_LABEL( "DLMFD_NavierStokes::initialize_grid_nodes_on_rigidbody" ) ;
...
//---------------------------------------------------------------------------
double
DLMFD_NavierStokes:: assemble_velocity_gradients (class doubleVector& grad
                                              , size_t const& i
                                              , size_t const& j
                                              , size_t const& k
                                              , size_t const& level)
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: assemble_velocity_gradients" ) ;
...
//---------------------------------------------------------------------------
double
DLMFD_NavierStokes:: calculate_velocity_divergence_FD ( size_t const& i,
                                                   size_t const& j,
                                                   size_t const& k,
                                                   size_t const& level)
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: calculate_velocity_divergence" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: assemble_velocity_advection_terms ( FV_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: assemble_velocity_advection_terms" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: compute_velocity_divergence ( FV_DiscreteField const* FF )
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: compute_velocity_divergence" ) ;
...
//---------------------------------------------------------------------------
double
DLMFD_NavierStokes:: pressure_local_rhs ( size_t const& j
                                      , size_t const& k
                                      , FV_TimeIterator const* t_it
                                      , size_t const& dir)
//---------------------------------------------------------------------------
{
   MAC_LABEL("DLMFD_NavierStokes:: pressure_local_rhs" ) ;
   // Get local min and max indices
   size_t_vector min_unknown_index(dim,0);
   size_t_vector max_unknown_index(dim,0);
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: correct_pressure_1st_layer_solid (size_t const& level )
//---------------------------------------------------------------------------
{
  MAC_LABEL( "DLMFD_NavierStokes:: correct_pressure_1st_layer_solid" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: correct_pressure_2nd_layer_solid (size_t const& level )
//---------------------------------------------------------------------------
{
  MAC_LABEL( "DLMFD_NavierStokes:: correct_pressure_2nd_layer_solid" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: correct_mean_pressure (size_t const& level )
//---------------------------------------------------------------------------
{
  MAC_LABEL( "DLMFD_NavierStokes:: correct_mean_pressure" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: NS_pressure_update ( FV_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
  MAC_LABEL( "DLMFD_NavierStokes:: NS_pressure_update" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: NS_final_step ( FV_TimeIterator const* t_it )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: NS_final_step" ) ;
...
//----------------------------------------------------------------------
void
DLMFD_NavierStokes::write_output_field(FV_DiscreteField const* FF)
//----------------------------------------------------------------------
{
  ofstream outputFile ;

  std::ostringstream os2;
  os2 << "./DLMFD_results/intersection_data_" << my_rank << ".csv";
...
//----------------------------------------------------------------------
void
DLMFD_NavierStokes::output_L2norm_divergence( )
//----------------------------------------------------------------------
{
  size_t_vector min_unknown_index(dim,0);
  size_t_vector max_unknown_index(dim,0);
...
//----------------------------------------------------------------------
void
DLMFD_NavierStokes::output_L2norm_pressure( size_t const& level )
//----------------------------------------------------------------------
{
  size_t_vector min_unknown_index(3,0);
  size_t_vector max_unknown_index(3,0);
...
//----------------------------------------------------------------------
double
DLMFD_NavierStokes:: compute_DLMFD_velocity_change( void )
//----------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: compute_DLMFD_velocity_change" ) ;
...
//----------------------------------------------------------------------
void
DLMFD_NavierStokes::output_L2norm_velocity( size_t const& level )
//----------------------------------------------------------------------
{
  size_t_vector min_unknown_index(3,0);
  size_t_vector max_unknown_index(3,0);
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: create_DLMFD_subcommunicators ( void )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: create_DLMFD_subcommunicators" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: processor_splitting ( int const& color
                                       , int const& key
                                       , size_t const& dir )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: processor_splitting" ) ;

   MPI_Comm_split(MPI_COMM_WORLD, color, key, &DLMFD_Comm_i[dir]);
   MPI_Comm_size( DLMFD_Comm_i[dir], &nb_ranks_comm_i[dir] ) ;
   MPI_Comm_rank( DLMFD_Comm_i[dir], &rank_in_i[dir] ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: allocate_mpi_variables (FV_DiscreteField const* FF)
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: deallocate_mpi_variables ()
//---------------------------------------------------------------------------
{
   // Array declarations
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes:: free_DLMFD_subcommunicators ( void )
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: free_DLMFD_subcommunicators" ) ;
}
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes::build_links_translation()
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: build_links_translation" ) ;
...
//---------------------------------------------------------------------------
doubleVector
DLMFD_NavierStokes::compute_outOfDomain_Pressure(size_t const& level)
//---------------------------------------------------------------------------
{
	MAC_LABEL( "DLMFD_NavierStokes:: compute_outOfDomain_Pressure" ) ;
...
//---------------------------------------------------------------------------
void
DLMFD_NavierStokes::fields_projection()
//---------------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: fields_projection" ) ;
...
//----------------------------------------------------------------------
double
DLMFD_NavierStokes:: assemble_advection_Centered( size_t const& advecting_level,
                                                double const& coef,
...
//----------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: assemble_advection_Centered" );
...
//----------------------------------------------------------------------
double
DLMFD_NavierStokes:: assemble_advection_Centered_FD(double const& coef,
													          size_t const& i,
                                                 size_t const& j,
                                                 size_t const& k,
                                                 size_t const& comp,
...
//----------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: assemble_advection_Centered_FD" );
...
//----------------------------------------------------------------------
double
DLMFD_NavierStokes:: assemble_advection_Centered_CutCell(FV_TimeIterator const* t_it,
																		double const& coef,
																		size_t const& i,
                                                	   size_t const& j,
                                                		size_t const& k,
...
//----------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: assemble_advection_Centered_CutCell" );
...
//----------------------------------------------------------------------
double
DLMFD_NavierStokes:: assemble_advection_Upwind(
...
//----------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: assemble_advection_Upwind" );
...
//----------------------------------------------------------------------
double
DLMFD_NavierStokes:: assemble_advection_TVD(
  size_t const& advecting_level, double const& coef, size_t const& advected_level,
         size_t const& i, size_t const& j, size_t const& k, size_t const& component ) const
//----------------------------------------------------------------------
{
   MAC_LABEL( "DLMFD_NavierStokes:: assemble_advection_TVD" );
