...
//---------------------------------------------------------------------------
{
    ...
    list<pair<size_t, double>> gbFromField =
        pField->main_geometric_boundaries_on_proc();
    ...
    for (il = gbFromField.begin(); il != gbFromField.end(); il++)
    {
        ...
        all_geomboundaries.back() =
            new DLMFD_XYZBoxPlan("DLMFD_XYZBoxPlan", il->first, il->second);
    }
}
...
//---------------------------------------------------------------------------
{
    ...
    f << "Number of geometric boundaries = " << G.all_geomboundaries.size()
      << endl;
    list<DLMFD_GeomBoundary *>::const_iterator il;
    for (il = G.all_geomboundaries.begin(); il != G.all_geomboundaries.end();
         il++)
        f << *(*il) << endl;
...
//---------------------------------------------------------------------------
{
    ...
    f << "Number of geometric boundaries = " << all_geomboundaries.size()
      << endl;
    list<DLMFD_GeomBoundary *>::const_iterator il;
    for (il = all_geomboundaries.begin(); il != all_geomboundaries.end(); il++)
    {
