...
//---------------------------------------------------------------------------
DLMFD_AllRigidBodies::DLMFD_AllRigidBodies(size_t &dim, double const &time,
                                           istringstream &in,
                                           bool const &are_particles_fixed_,
                                           FV_DiscreteField *UU,
                                           FV_DiscreteField *PP,
                                           double const critical_distance)
    : are_particles_fixed(are_particles_fixed_), v_AllSharedOnProcs(NULL),
      l_AllSharedOnProcs(NULL), output_x_velocity("yes"),
      output_y_velocity("yes"), output_z_velocity("yes"),
      output_x_position("yes"), output_y_position("yes"),
      output_z_position("yes"), output_x_omega("yes"), output_y_omega("yes"),
      output_z_omega("yes"), output_orientation("no")
//---------------------------------------------------------------------------
{
    ...
    for (size_t i = 0; i < RBs_number; ++i)
    {
        ...
        vec_ptr_DLMFDallrigidbodies[i] = DLMFD_RigidBody_BuilderFactory::create(
            ptr_FSallrigidbodies->get_ptr_rigid_body(i), are_particles_fixed,
            pField, critical_distance);
    }
...
//---------------------------------------------------------------------------
{
    ...
    for (size_t i = 0; i < RBs_number; i++)
        vec_ptr_DLMFDallrigidbodies[i]->set_all_points(pField,
                                                       critical_distance);
}
...
//---------------------------------------------------------------------------
void DLMFD_AllRigidBodies::eraseCriticalDLMFDPoints(const double &time,
...
//---------------------------------------------------------------------------
{
    ...
    for (size_t i = 0; i < RBs_number; i++)
        vec_ptr_DLMFDallrigidbodies[i]->erase_critical_boundary_points_ptb(
            1.0001 * critical_distance / sqrt(double(dim)), GeoBoundaries);
    ...
    if (RBs_number > 1)
    {
        for (size_t i = 0; i < RBs_number; i++)
            for (size_t j = i + 1; j < RBs_number; j++)
                if (i != j)
                    vec_ptr_DLMFDallrigidbodies[i]
                        ->erase_critical_boundary_points_ptp_PerProc(
                            vec_ptr_DLMFDallrigidbodies[j], critical_distance);
    }
    ...
    for (size_t i = 0; i < RBs_number; i++)
        if (vec_ptr_DLMFDallrigidbodies[i]->get_component_type() != "O" ||
            time < 1.e-10)
            vec_ptr_DLMFDallrigidbodies[i]
                ->erase_critical_interior_points_PerProc(coef_cd *
                                                         critical_distance);
}
...
//---------------------------------------------------------------------------
{
    ...
    for (size_t i = 0; i < RBs_number; i++)
    {
        ...
        {
            onProc.push_back(i);
            if (vec_ptr_DLMFDallrigidbodies[i]
                    ->hasDLMFDPoints_inHalozone_OnProc())
    ...
    {
        v_AllSharedOnProcs =
            new vector<size_t_vector>(size_procs, v_SharedOnProc);
...
{
    MAC_LABEL(
        "DLMFD_AllRigidBodies::set_ptr_constrained_field_in_all_particles");
    ...
    if (l_AllSharedOnProcs)
        for (il = l_AllSharedOnProcs->begin(); il != l_AllSharedOnProcs->end();
             il++)
            vec_ptr_DLMFDallrigidbodies[*il]->set_ptr_constrained_field(pField);
}
...
//---------------------------------------------------------------------------
{
    ...
    for (size_t j = 0; j < size_procs; ++j)
    {
        if (j == rank)
        {
            MAC::out() << "Local domain geometric boundaries " << "on process "
                       << rank << endl;
            MAC::out() << "----------------------------------"
                       << "----------------" << endl;
...
//---------------------------------------------------------------------------
void DLMFD_AllRigidBodies::set_coupling_factor(double const &rho_f,
...
//---------------------------------------------------------------------------
{
    ...
    list<int>::iterator il;
    for (il = onProc.begin(); il != onProc.end(); il++)
        vec_ptr_DLMFDallrigidbodies[*il]->set_coupling_factor(
            rho_f, explicit_treatment);
...
{
    MAC_LABEL(
        "DLMFD_AllRigidBodies:: set_mass_and_density_and_volume_and_inertia");
...
//---------------------------------------------------------------------------
void DLMFD_AllRigidBodies::set_translational_velocity(const size_t i,
                                                      const geomVector &vtran)
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void DLMFD_AllRigidBodies::set_angular_velocity_3D(const size_t i,
                                                   const geomVector &vrot)
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
vector<size_t_vector> const *
DLMFD_AllRigidBodies::get_v_AllSharedOnProcs() const
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
geomVector
DLMFD_AllRigidBodies::get_translational_velocity(const size_t i) const
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void DLMFD_AllRigidBodies::particles_velocities_output(
    vector<vector<double>> &vecVel) const
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void DLMFD_AllRigidBodies::output_DLMFDPoints_PARAVIEW(
    const string &filename, geomVector const *translated_distance_vector,
...
//---------------------------------------------------------------------------
{
    ...
    for (il = onProc.begin(); il != onProc.end(); il++)
        output_npts +=
            vec_ptr_DLMFDallrigidbodies[*il]->get_npts_output(withIntPts);
    ...
    for (il = onProc.begin(); il != onProc.end(); il++)
        vec_ptr_DLMFDallrigidbodies[*il]->print_partPointsCoordinates(
            f, translated_distance_vector, "", "", withIntPts);
...
//---------------------------------------------------------------------------
void DLMFD_AllRigidBodies::particles_hydrodynamic_force_output(
    ...
    if (b_output_hydro_forceTorque)
    {
            ...
            if (rank == master)
            {
                ...
                for (i = 0; i < RBs_number; i++)
                {
                    ...
                    for (indexcomp = 0; indexcomp < dim; ++indexcomp)
                    {
                        ...
                        if (vec_ptr_DLMFDallrigidbodies[i]
                                    ->get_component_type() != "O" &&
                            !are_particles_fixed)
                        {
                            hydro_force(indexcomp) +=
                                dlmfd_mass *
                                (vtrans(indexcomp) -
                                 (*translational_velocity_nm1)(i, indexcomp)) /
                                timestep;
                            (*translational_velocity_nm1)(i, indexcomp) =
                                vtrans(indexcomp);
                        }
                    ...
                    density_ratio =
                        rho_f / vec_ptr_DLMFDallrigidbodies[i]->get_density();
                    ...
                    for (indexcomp = 0; indexcomp < dim; ++indexcomp)
                    {
                        ...
                        if (vec_ptr_DLMFDallrigidbodies[i]
                            ...
                            !are_particles_fixed)
                        {
                            hydro_torque(indexcomp) +=
                                density_ratio * (*Iw_Idw)[i][dim + indexcomp] /
                                timestep;
                            (*angular_velocity_3D_nm1)(i, indexcomp) =
                                vrot(indexcomp);
                        }
                    }
                    // Add w x Iw to all torque component
                    if (vec_ptr_DLMFDallrigidbodies[i]->get_component_type() !=
                            "O" &&
                        !are_particles_fixed)
                    {
                        hydro_torque(0) +=
                            density_ratio * (vrot(1) * (*Iw_Idw)[i][2] -
                                             vrot(2) * (*Iw_Idw)[i][1]);
                        hydro_torque(1) +=
                            density_ratio * (vrot(2) * (*Iw_Idw)[i][0] -
                                             vrot(0) * (*Iw_Idw)[i][2]);
                        hydro_torque(2) +=
                            density_ratio * (vrot(0) * (*Iw_Idw)[i][1] -
                                             vrot(1) * (*Iw_Idw)[i][0]);
                    }
        ...
        else
        {
            if (rank == master)
            {
                ...
                {
                    vtrans = vec_ptr_DLMFDallrigidbodies[i]
                                 ->get_translational_velocity();
                    vrot = vec_ptr_DLMFDallrigidbodies[i]
...
//---------------------------------------------------------------------------
void DLMFD_AllRigidBodies::sum_DLM_hydrodynamic_force_output(
    ...
    if (b_output_hydro_forceTorque)
    {
        ...
        if (counter == 0 && restart_counter)
        {
            ...
            else
            {
                ...
                // Do not use compute_all_Qu and compute_all_Qrot that act on
                // particles that are on proc only If a particle is not on
                // master proc, methods SolidComponent::compute_Qu and
                // SolidComponent::compute_Qrot initialize q_tran and q_rot to
                // zero
                for (i = 0; i < no; i++)
                {
                ...
                {
                    doubleVector recvbuf(
                        0); // No need to set the reception buffer size
    ...
    if (counter == 0 && restart_counter)
    {
        ...
        if (output_x_velocity != "no")
        {
            ...
            if (output_x_velocity == "all" || output_x_velocity == "mean")
            {
                ...
                for (i = 0; i < RBs_number; i++)
                {
                    val = vec_ptr_DLMFDallrigidbodies[i]
                              ->get_translational_velocity()(0);
                    mean_value += val;
                ...
                for (i = 0; i < RBs_number; i++)
                    rms += pow(vec_ptr_DLMFDallrigidbodies[i]
                                       ->get_translational_velocity()(0) -
                                   mean_value,
                               2.);
                rms = sqrt(rms / RBs_number);
                xvf << " " << mean_value << " " << min_value << " " << max_value
                    << " " << rms;
            }
            xvf << endl;
            xvf.close();
            ...
            if (output_y_velocity == "all" || output_y_velocity == "mean")
            {
                ...
                for (i = 0; i < RBs_number; i++)
                    rms += pow(vec_ptr_DLMFDallrigidbodies[i]
                               ...
                               2.);
                rms = sqrt(rms / RBs_number);
                yvf << " " << mean_value << " " << min_value << " " << max_value
                    << " " << rms;
            }
            yvf << endl;
            yvf.close();
            ...
            if (output_z_velocity == "all" || output_z_velocity == "mean")
            {
                ...
                for (i = 0; i < RBs_number; i++)
                    rms += pow(vec_ptr_DLMFDallrigidbodies[i]
                               ...
                               2.);
                rms = sqrt(rms / RBs_number);
                zvf << " " << mean_value << " " << min_value << " " << max_value
                    << " " << rms;
            }
            zvf << endl;
            zvf.close();
            ...
            if (output_x_position == "all" || output_x_position == "mean")
            {
                ...
                for (i = 0; i < RBs_number; i++)
                    rms += pow((*vec_ptr_DLMFDallrigidbodies[i]
                               ...
                               2.);
                rms = sqrt(rms / RBs_number);
                xpf << " " << mean_value << " " << min_value << " " << max_value
                    << " " << rms;
            }
            xpf << endl;
            xpf.close();
            ...
            if (output_y_position == "all" || output_y_position == "mean")
            {
                ...
                for (i = 0; i < RBs_number; i++)
                    rms += pow((*vec_ptr_DLMFDallrigidbodies[i]
                               ...
                               2.);
                rms = sqrt(rms / RBs_number);
                ypf << " " << mean_value << " " << min_value << " " << max_value
                    << " " << rms;
            }
            ypf << endl;
            ypf.close();
            ...
            if (output_z_position == "all" || output_z_position == "mean")
            {
                ...
                for (i = 0; i < RBs_number; i++)
                    rms += pow((*vec_ptr_DLMFDallrigidbodies[i]
                               ...
                               2.);
                rms = sqrt(rms / RBs_number);
                zpf << " " << mean_value << " " << min_value << " " << max_value
                    << " " << rms;
            }
            zpf << endl;
            zpf.close();
        ...
        if (output_orientation != "no")
        {
            ...
            for (i = 0; i < RBs_number; i++)
            {
                vorientation = vec_ptr_DLMFDallrigidbodies[i]
                                   ->particle_orientation_vector();
...
//---------------------------------------------------------------------------
void DLMFD_AllRigidBodies::completeFirstUzawaIteration_Velocity(
...
//---------------------------------------------------------------------------
{
    ...
    for (il = entireOnProc.begin(); il != entireOnProc.end(); il++)
    {
        ...
        {
            vec_ptr_DLMFDallrigidbodies[*il]
                ->correctQvectorsAndInitUzawa_Velocity(
                    rho_f, timestep, gravity_vector_split, gravity_vector);
            vec_ptr_DLMFDallrigidbodies[*il]->calculateParticleVelocities(
                rho_f, timestep);
            vec_ptr_DLMFDallrigidbodies[*il]->updateSolutionVectors_Velocity(
                1., 0.);
        }
        else
             ...
             il++)
        {
            if (vec_ptr_DLMFDallrigidbodies[*il]->get_component_type() != "O")
            {
                ...
                vec_ptr_DLMFDallrigidbodies[*il]->calculateParticleVelocities(
                    rho_f, timestep);
                vec_ptr_DLMFDallrigidbodies[*il]
                    ->updateSolutionVectors_Velocity(1., 0.);
            }
            else
                vec_ptr_DLMFDallrigidbodies[*il]->setTVectors_constant();
        }
...
{
    MAC_LABEL(
        "DLMFD_AllRigidBodies::update_ParticlesVelocities_afterBcast_of_T");

    list<int>::iterator il;
    for (il = onProc.begin(); il != onProc.end(); il++)
    {
        vec_ptr_DLMFDallrigidbodies[*il]->updateSolutionVectors_Velocity(1.,
                                                                         0.);
    }
}
...
//---------------------------------------------------------------------------
void DLMFD_AllRigidBodies::solve_Particles_OneUzawaIter_Velocity(
    double const &rho_f, double const &timestep)
//---------------------------------------------------------------------------
{
    ...
    // Particles entire on this process
    for (il = entireOnProc.begin(); il != entireOnProc.end(); il++)
        vec_ptr_DLMFDallrigidbodies[*il]->calculateParticleVelocities(rho_f,
                                                                      timestep);
...
//---------------------------------------------------------------------------
{
    ...
    for (il = onProc.begin(); il != onProc.end(); il++)
    {
        if (vec_ptr_DLMFDallrigidbodies[*il]->get_component_type() == "P" ||
            vec_ptr_DLMFDallrigidbodies[*il]->get_component_type() == "PP")
            vec_ptr_DLMFDallrigidbodies[*il]->updateSolutionVectors_Velocity(
                alpha, 1.);
        else
            vec_ptr_DLMFDallrigidbodies[*il]->updateSolutionVectors_Velocity(
...
//---------------------------------------------------------------------------
{
    ...
    list<int>::iterator il;
    for (il = onProc.begin(); il != onProc.end(); il++)
        vec_ptr_DLMFDallrigidbodies[*il]->compute_fluid_DLMFD_explicit(
            GLOBAL_EQ, bulk, pField);
}
...
{
    MAC_LABEL(
        "DLMFD_AllRigidBodies::allocate_translational_angular_velocity_array");
...
//---------------------------------------------------------------------------
double DLMFD_AllRigidBodies::compute_minimum_distance_to_bottom(
...
//---------------------------------------------------------------------------
{
    ...
    {
        distance = vec_ptr_DLMFDallrigidbodies[j]->compute_distance_to_bottom(
            coordinate, direction);
        minimum_distance =
            distance < minimum_distance ? distance : minimum_distance;
    }
...
//---------------------------------------------------------------------------
void DLMFD_AllRigidBodies::translate_geometricBoundaries(
    const geomVector &translation_vector, const size_t &translation_direction)
//---------------------------------------------------------------------------
{
