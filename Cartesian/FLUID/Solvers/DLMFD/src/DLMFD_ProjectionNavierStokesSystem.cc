...
//----------------------------------------------------------------------
DLMFD_ProjectionNavierStokesSystem *DLMFD_ProjectionNavierStokesSystem::create(
    MAC_Object *a_owner, MAC_ModuleExplorer const *exp,
    FV_DiscreteField *mac_uu, FV_DiscreteField *mac_pp,
    const size_t &NS_Viscous_TimeAccuracy_,
    const size_t &NS_Advection_TimeAccuracy_, const bool &b_pressure_rescaling_,
...
//----------------------------------------------------------------------
{
    ...
    DLMFD_ProjectionNavierStokesSystem *result =
        new DLMFD_ProjectionNavierStokesSystem(
            a_owner, exp, mac_uu, mac_pp, NS_Viscous_TimeAccuracy_,
            NS_Advection_TimeAccuracy_, b_pressure_rescaling_,
            b_ExplicitPressureGradient_, b_HighOrderPressureCorrection_);
...
DLMFD_ProjectionNavierStokesSystem::DLMFD_ProjectionNavierStokesSystem(
    ...
    //----------------------------------------------------------------------
    : MAC_Object(a_owner),
      DLMFD_System(a_owner, exp, mac_uu, mac_pp, NS_Viscous_TimeAccuracy_,
                   NS_Advection_TimeAccuracy_, b_pressure_rescaling_,
                   b_ExplicitPressureGradient_, b_HighOrderPressureCorrection_,
                   false),
      UU(mac_uu), PP(mac_pp), UU_NUM(0), PP_NUM(0), MAT_A_VelocityUnsteady(0),
      VEC_rhs_A_Velocity(0), MAT_A_VelocityUnsteadyPlusViscous(0),
      VEC_rhs_A_VelocityViscous(0), MAT_B_VelocityDivergence(0),
      VEC_rhs_B_VelocityDivergence(0), VEC_rhs_Bt_PressureGradient(0),
      MAT_D_PressureLaplacian(0), VEC_rhs_D_PressureLaplacian(0), U_LOC(0),
      P_LOC(0), T_LOC(0), VEC_U(0), VEC_U_previoustime(0), VEC_U_timechange(0),
      VEC_P(0), VEC_rhs_VelocityAdvection(0),
      VEC_rhs_VelocityAdvectionDiffusion(0), VEC_rhs_VelocityAdvection_Nm2(0),
      VEC_rhs_A_UnitaryPeriodicPressureGradient(0),
      VEC_rhs_A_PeriodicPressure(0), VEC_t(0), VEC_q(0), VEC_r(0), VEC_w(0),
      VEC_mean_pressure(0), VEC_unit_pressure(0), VEC_inverse_mean_pressure(0),
      SOLVER_A_VelocityUnsteadyPlusViscous(0), SOLVER_D_PressureLaplacian(0),
      SOLVER_A_VelocityUnsteady(0), VECS_Storage(0),
      ...
      b_NS_ExplicitDLMFD(false), b_NS_pressure_drop_each_time(false)
{
    MAC_LABEL("DLMFD_ProjectionNavierStokesSystem:: "
              "DLMFD_ProjectionNavierStokesSystem");

    // Pressure drop handled at each time step
    b_NS_pressure_drop_each_time =
        DLMFD_ProjectionNavierStokes::b_pressure_drop_each_time;
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::build_system(
...
//----------------------------------------------------------------------
{
    ...
    MAT_A_VelocityUnsteadyPlusViscous = LA_Matrix::make(
        this,
        exp->create_subexplorer(this, "MAT_A_VelocityUnsteadyPlusViscous"));
    VEC_rhs_A_VelocityViscous = MAT_A_VelocityUnsteady->create_vector(this);

    // Velocity divergence p.Div(v) matrix and rhs
    MAT_B_VelocityDivergence = LA_Matrix::make(
        this, exp->create_subexplorer(this, "MAT_B_VelocityDivergence"));
    VEC_rhs_B_VelocityDivergence =
        MAT_B_VelocityDivergence->create_vector(this);
    ...
    MAT_D_PressureLaplacian = LA_Matrix::make(
        this, exp->create_subexplorer(this, "MAT_D_PressureLaplacian"));
    ...
    // Solver
    SOLVER_A_VelocityUnsteadyPlusViscous = LA_Solver::make(
        this,
        exp->create_subexplorer(this, "SOLVER_A_VelocityUnsteadyPlusViscous"));
    SOLVER_A_VelocityUnsteadyPlusViscous->set_initial_guess_nonzero(true);

    SOLVER_D_PressureLaplacian = LA_Solver::make(
        this, exp->create_subexplorer(this, "SOLVER_D_PressureLaplacian"));
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::re_initialize(void)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::at_each_time_step(void)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
double DLMFD_ProjectionNavierStokesSystem::compute_velocity_change(void)
//----------------------------------------------------------------------
{
...
{
    MAC_LABEL("DLMFD_ProjectionNavierStokesSystem:: "
              "compute_velocity_divergence_norm");
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::nullify_velocity_advection_rhs(void)
//----------------------------------------------------------------------
{
    MAC_LABEL(
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::initialize_velocity(void)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::initialize_pressure(void)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::finalize_constant_matrices(void)
//----------------------------------------------------------------------
{
    MAC_LABEL(
    ...
    if (macCOMM->rank() == 0)
        MAC::out() << "            Velocity unsteady + viscous matrix" << endl;
    ...
    MAT_A_VelocityUnsteadyPlusViscous->add_Mat(MAT_A_VelocityUnsteady, 1.,
                                               same_pattern);
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::compute_velocityAdvectionDiffusion_rhs(
...
//----------------------------------------------------------------------
{
    ...
    if (NS_Viscous_TimeAccuracy == 1)
    {
        ...
        MAT_A_VelocityUnsteady->multiply_vec_then_add(
            VEC_U, VEC_rhs_VelocityAdvectionDiffusion);
    ...
    else
    {
        ...
        // rhs = 2 * (rho/Dt) * u
        MAT_A_VelocityUnsteady->multiply_vec_then_add(
            VEC_U, VEC_rhs_VelocityAdvectionDiffusion, 2.);
        ...
        MAT_A_VelocityUnsteadyPlusViscous->multiply_vec_then_add(
            VEC_U, VEC_rhs_VelocityAdvectionDiffusion, -1., 1.);
    }
    ...
    if (b_with_advection)
    {
            ...
            (iteration_number == 1 && !b_restart))
        {
            // Add u.gradu RHS
            // rhs += rho * (u.gradu)^n-1
            VEC_rhs_VelocityAdvectionDiffusion->sum(VEC_rhs_VelocityAdvection,
                                                    1.);
        }
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::assemble_velocity_viscous_matrix_rhs(
    double const &coef_lap)
//----------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_ProjectionNavierStokesSystem:: "
              "assemble_velocity_viscous_matrix_rhs");

    UU->assemble_constantcoef_laplacian_matrix(
        coef_lap, MAT_A_VelocityUnsteadyPlusViscous, VEC_rhs_A_VelocityViscous);
}
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::assemble_velocity_unsteady_matrix(
    double const &coef)
//----------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_ProjectionNavierStokesSystem:: "
              "assemble_velocity_unsteady_matrix");
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::assemble_pdivv_matrix_rhs(
    double const &coef)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::assemble_velocity_advection(
    string const &AdvectionScheme, size_t advecting_level, double const &coef,
...
//----------------------------------------------------------------------
{
    ...
    if (AdvectionScheme == "Upwind")
        UU->assemble_advection_Upwind(UU, advecting_level, coef, advected_level,
                                      VEC_rhs_VelocityAdvection);
    else
        UU->assemble_advection_TVD(UU, advecting_level, coef, advected_level,
                                   VEC_rhs_VelocityAdvection);
}
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::assemble_pressure_laplacian_matrix_rhs(
    double const &coef_lap)
//----------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_ProjectionNavierStokesSystem:: "
              "assemble_pressure_laplacian_matrix_rhs");

    PP->assemble_constantcoef_laplacian_matrix(
        coef_lap, MAT_D_PressureLaplacian, VEC_rhs_D_PressureLaplacian,
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::pressure_laplacian_correction(void)
//----------------------------------------------------------------------
{
    MAC_LABEL(
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::add_storable_objects(
...
//----------------------------------------------------------------------
bool DLMFD_ProjectionNavierStokesSystem::VelocityDiffusion_solver(void)
//----------------------------------------------------------------------
{
    ...
    {
        // Add - grad(p)
        MAT_B_VelocityDivergence->tr_multiply_vec_then_add(
            VEC_P, VEC_rhs_A_Velocity, -1., 1.);
    ...
    MAC_ASSERT(SOLVER_A_VelocityUnsteadyPlusViscous->solution_is_achieved());
...
//----------------------------------------------------------------------
double DLMFD_ProjectionNavierStokesSystem::VelocityPressure_correction_solver(
    double const &density, double const &viscosity, double const &timestep)
//----------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_ProjectionNavierStokesSystem:: "
              "VelocityPressure_correction_solver");
    ...
    if (MAC_Exec::communicator()->rank() == 0)
        MAC::out() << "Nb iterations sub-linear systems = " << niter_sublin
                   << endl;
...
//----------------------------------------------------------------------
LA_Vector *
DLMFD_ProjectionNavierStokesSystem::get_pressure_DirichletBC_vector(void)
//----------------------------------------------------------------------
{
    MAC_LABEL(
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::set_velocity_unknown(size_t i_row,
                                                              double xx)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::set_pressure_unknown(size_t i_row,
                                                              double xx)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_ProjectionNavierStokesSystem::synchronize_velocity_"
              "unknown_vector");
...
//----------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_ProjectionNavierStokesSystem::synchronize_pressure_"
              "unknown_vector");
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::set_rhs_DLMFD_Nm1(size_t i_row,
                                                           double xx)
//----------------------------------------------------------------------
{
...
{
    MAC_LABEL(
        "DLMFD_ProjectionNavierStokesSystem::synchronize_rhs_DLMFD_Nm1_vector");
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::
    synchronize_rhs_periodic_pressure_vector()
//----------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_ProjectionNavierStokesSystem::synchronize_rhs_periodic_"
              "pressure_vector");
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::set_periodic_pressure_rhs_item(
    size_t i_row, double xx)
//----------------------------------------------------------------------
{
    MAC_LABEL(
        "DLMFD_ProjectionNavierStokesSystem::set_periodic_pressure_rhs_item");
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::store_ugradu_Nm2(
...
//----------------------------------------------------------------------
bool DLMFD_ProjectionNavierStokesSystem::VelocityAdvection_solver(void)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::assemble_inQUvector(double transferVal,
                                                             size_t index,
                                                             double coef)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::solve_FluidVel_DLMFD_Init(
    const double &time)
//----------------------------------------------------------------------
{
...
//----------------------------------------------------------------------
double const
DLMFD_ProjectionNavierStokesSystem::get_DLMFD_convergence_criterion() const
//----------------------------------------------------------------------
{
    MAC_LABEL(
        "DLMFD_ProjectionNavierStokesSystem::get_DLMFD_convergence_criterion");
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::solve_FluidVel_DLMFD_Iter(
    const double &time)
//----------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_ProjectionNavierStokesSystem::initialize_QUvector_with_"
...
//----------------------------------------------------------------------
void DLMFD_ProjectionNavierStokesSystem::update_FluidVel_OneUzawaIter(
    const double &alpha)
//----------------------------------------------------------------------
{
    MAC_LABEL(
        "DLMFD_ProjectionNavierStokesSystem::update_FluidVel_OneUzawaIter");
...
{
    MAC_LABEL(
        "DLMFD_ProjectionNavierStokesSystem::re_initialize_explicit_DLMFD");
