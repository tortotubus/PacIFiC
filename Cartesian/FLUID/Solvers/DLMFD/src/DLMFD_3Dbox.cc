...
//---------------------------------------------------------------------------
DLMFD_3Dbox::DLMFD_3Dbox(FS_RigidBody *pgrb, const bool &are_particles_fixed,
...
//---------------------------------------------------------------------------
void DLMFD_3Dbox::set_all_MAC(FV_DiscreteField *pField,
...
//---------------------------------------------------------------------------
{
            ...
            gravity_center(0), gravity_center(1), gravity_center(2), radius))
    {
        ...
        if (interior_points.empty())
            allocate_default_listOfPointsAndVectors_3Dbox(critical_distance,
                                                          pField);
...
//---------------------------------------------------------------------------
void DLMFD_3Dbox::set_all_points(FV_DiscreteField *pField,
                                 double critical_distance)
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
void DLMFD_3Dbox::set_boundary_points_list(FV_DiscreteField *pField,
                                           double critical_distance)
//---------------------------------------------------------------------------
{
             ...
             itrLocNodes != faceIter->end(); ++itrLocNodes)
        {
            ...
            if (facesBnd.insert(pairToAdd).second)
            {
                setPtsOnEdge(corners[*(itrLocNodes - 1)], corners[*itrLocNodes],
                             critical_distance, false, primary_grid, bp, bphz);
...
//---------------------------------------------------------------------------
void DLMFD_3Dbox::set_interior_points_list(FV_DiscreteField *pField,
                                           double critical_distance)
//---------------------------------------------------------------------------
{
...
//---------------------------------------------------------------------------
FS_3Dbox_Additional_Param const *
DLMFD_3Dbox::get_ptr_FS_3Dbox_Additional_Param()
//---------------------------------------------------------------------------
{
    MAC_LABEL("DLMFD_3Dbox::get_ptr_FS_3Dbox_Additional_Param");

    return (dynamic_cast<FS_3Dbox *>(ptr_FSrigidbody)
...
//---------------------------------------------------------------------------
void DLMFD_3Dbox::allocate_default_listOfPointsAndVectors_3Dbox(
...
//---------------------------------------------------------------------------
{
    ...
    else
    {
        ...
        size_t nxB =
            size_t((coor_max(0) - coor_min(0)) / critical_distance) + 1;
        size_t nyB =
            size_t((coor_max(1) - coor_min(1)) / critical_distance) + 1;
        size_t nzB =
            size_t((coor_max(2) - coor_min(2)) / critical_distance) + 1;

        size_t nbIPdef = 3 * nxI * nyI * nzI;
        size_t nbBPdef = 2 * (nxB * nyB + nxB * nzB + nyB * nzB);
        size_t nbIPHZdef =
            security_bandwidth * (nxI * nyI + nxI * nzI + nyI * nzI);
...
//---------------------------------------------------------------------------
{
    ...
    for (vector<vector<size_t>>::const_iterator faceIter = facesVec.begin();
         faceIter != facesVec.end() && !b_isIn; ++faceIter)
        for (size_t idxPts = 2; idxPts < 4 && !b_isIn; ++idxPts)
            if (checkPointInTetrahedron(
                    corners[(*faceIter)[0]], corners[(*faceIter)[idxPts - 1]],
...
//---------------------------------------------------------------------------
bool DLMFD_3Dbox::proximityQuery(DLMFD_RigidBody const *second_component,
...
//---------------------------------------------------------------------------
{
    ...
    bool condition_one = fabs(0.5 * (coor_min(0) + coor_max(0)) - (*gc2)(0)) <=
                         0.5 * (coor_max(0) - coor_min(0)) + radius2;
    bool condition_two = fabs(0.5 * (coor_min(1) + coor_max(1)) - (*gc2)(1)) <=
                         0.5 * (coor_max(1) - coor_min(1)) + radius2;
    bool condition_three =
        fabs(0.5 * (coor_min(2) + coor_max(2)) - (*gc2)(2)) <=
        0.5 * (coor_max(2) - coor_min(2)) + radius2;
