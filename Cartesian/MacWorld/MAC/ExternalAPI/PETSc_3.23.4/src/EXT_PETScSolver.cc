...
//----------------------------------------------------------------------------
{
   ...
   if( VERB && SUBEXP==0 )
   {
     ...
     PETSc_do( PetscViewerAndFormatCreate( PETSC_VIEWER_STDOUT_WORLD,
...
//             }
	 ...
	 if( exp->has_entry( "pc_factor_shift_nonzero" ) )
	 {
	    ...
	    PETSc_do( PCFactorSetShiftAmount(
         ...
         if( exp->has_entry( "pc_factor_allow_diagonal_fill" ) &&
            ...
            PETSc_do( PCFactorSetAllowDiagonalFill( pc, PETSC_TRUE ) ) ;
	...
	PETSc_do( PCFactorSetMatSolverType( pc, MATSOLVERMUMPS ) );
      }
      else
      {
         MAC_Error::object()->raise_bad_data_value(
