...
void ensure_distributed_spherical_capsule_template(lagMesh* mesh, int cap_id,
  int cap_type, int nln, int nle, int nlt, double cap_es,
  double cap_radius);
#define LAG_DISTRIBUTED_ENSURE_RECEIVED_CAPSULE_TEMPLATE(mesh, cap_id, \
  cap_type, nln, nle, nlt, cap_es, cap_radius) \
  ensure_distributed_spherical_capsule_template(mesh, cap_id, cap_type, \
    nln, nle, nlt, cap_es, cap_radius)
...
void ensure_distributed_spherical_capsule_template(lagMesh* mesh, int cap_id,
  ...
  double cap_radius)
{
  int has_template = mesh->topology != NULL && mesh->ref_geometry != NULL &&
    mesh->nln == nln && mesh->nle == nle;
  #if dimension > 2
    has_template = has_template && mesh->nlt == nlt;
  #else
    (void) nlt;
  #endif
  if (has_template)
  ...
  int cap_level = cap_type == 0 ? LAG_LEVEL_L : LAG_LEVEL_S;
  coord saved_centroid = mesh->centroid;
  double saved_circum_radius = mesh->circum_radius;

  if (mesh->nodes != NULL || mesh->edges != NULL)
    free_lagMesh_current_storage_keep_slot(mesh);

  initialize_empty_capsule(mesh);
  mesh->cap_es = cap_es;
  mesh->cap_radius = cap_radius;
  mesh->cap_id = cap_id;
  mesh->cap_type = cap_type;
  mesh->isactive = true;
  initialize_spherical_capsule((_initialize_circular_capsule){
    .mesh = mesh, .cap_es = cap_es, .cap_radius = cap_radius,
    .level = cap_level, .cap_id = cap_id, .cap_type = cap_type
  });
  attach_shared_lag_topology(mesh);
  #if _ELASTICITY_FT
    store_initial_configuration(mesh);
  #endif
  attach_shared_lag_ref_geometry(mesh);
  free(mesh->nodes);
  free(mesh->edges);
  #if dimension > 2
    free(mesh->triangles);
  ...
  mesh->cap_es = cap_es;
  mesh->cap_radius = cap_radius;
  mesh->centroid = saved_centroid;
  mesh->circum_radius = saved_circum_radius;
  mesh->isactive = false;
}

double initial_capsule_route_radius(double cap_radius)
{
  #if MULT_GRID == 1
  ...
  return cap_radius + LAG_BOUNDING_SPHERE_PADDING_DELTAS*delta;
}
...
bool should_initialize_capsule_on_this_rank(coord centroid, double radius)
{
    ...
    route_mesh.circum_radius = radius;

    int owner_proc = find_capsule_owner_proc(&route_mesh,
      all_proc_min, all_proc_max);
    int intersects_local = periodic_sphere_intersects_box(centroid, radius,
      proc_min, proc_max);

    return owner_proc == pid() || intersects_local;
  #else
    (void) centroid;
    (void) radius;
    return true;
...
event init (i = 0) {
  ...
  else
  {
    ...
    for (size_t k = 0; k < NCAPS; k++) {
      ...
      coord initial_centroid = {
        cap_read[k].x,
        cap_read[k].y,
        cap_read[k].z
      };
      double cap_es = cap_read[k].type == 0 ? E_S_L : E_S_S;
      double cap_radius = cap_read[k].type == 0 ? RADIUS_L : RADIUS_S;
      int cap_level = cap_read[k].type == 0 ? LAG_LEVEL_L : LAG_LEVEL_S;
      double route_radius = initial_capsule_route_radius(cap_radius);
      int initialize_here =
...
event output_and_dump_caps (i+=OUTPUT_FREQ_PV_DUMP) {
    ...
    pv_output_bounding_spheres_ascii(nb_dump);
    #if _MPI && LAG_DISTRIBUTED_OUTPUT_GATHER_RANK0
